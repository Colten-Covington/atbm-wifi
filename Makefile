...
###############################################################################
#
# when release to customer ,the DEBUG_MODE must set to n!!!!!!!!!!!!!
#
###############################################################################
DEBUG_MODE=n
###############################################################################
...
ifeq ($(PLATFORM_KALI),y)
KERDIR:=/lib/modules/5.16.0-kali7-amd64/build/
endif
...
CROSS_COMPILE:=/wifi_prj/staff/mengxuehong/mengxuehong/arm-2014.05/bin/arm-none-linux-gnueabi-
...
#CROSS_COMPILE:=/usr/lchome/zhaoledong/Android6.0/cqa64_android_v6.0/lichee/brandy/armv8_toolchain/gcc-linaro-aarch64-linux-gnu-4.9-2014.09_linux/bin/aarch64-linux-gnu-
else
KERDIR:=/wifi_prj/staff/songningning/cqa64_ubuntu18_64bit_bv3s/linux-3.10
#KERDIR:=/media/sf_E__MyVirtualBox_VirtualBoxShareDir/ToolChainCopy/AllWinner-sun50iw1p1-ubuntu18/linux-3.10-BJ200Server/linux-3.10
CROSS_COMPILE:=/wifi_prj/staff/songningning/cqa64_ubuntu18_64bit_bv3s/out/sun50iw1p1/ubuntu18/common/buildroot/external-toolchain/bin/aarch64-linux-gnu-
#CROSS_COMPILE:=/media/sf_E__MyVirtualBox_VirtualBoxShareDir/ToolChainCopy/AllWinner-sun50iw1p1-ubuntu18/common/buildroot/external-toolchain/bin/aarch64-linux-gnu-
endif
export
...
ATBM_SVN_VERSION:=$(shell ((svnversion | grep -qv exported && echo -n 'Revision: ' && svnversion) || git svn info | sed -e 's/$$$$/M/' | grep '^Revision: ' ) | sed -e 's/^Revision: //'| sed -e 's/M//g')

getsvn:
	@echo "**************************************"
ifeq ($(DEBUG_MODE),y)
	@echo "#define DRIVER_VER " $(ATBM_SVN_VERSION) >hal_apollo/svn_version.h
	@echo "svn version:" $(ATBM_SVN_VERSION)
endif
