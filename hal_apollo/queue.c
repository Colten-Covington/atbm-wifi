...
{
	int i;
	memset(stats, 0, sizeof(struct atbm_queue_stats));
	...
	for (i = 0; i < ATBM_WIFI_MAX_VIFS; i++) {
		stats->link_map_cache[i] = atbm_kzalloc(sizeof(int[50]),
			GFP_KERNEL);//make sure the size is enough
		      ...
		      unsigned long ttl)
{
	...
	memset(queue, 0, sizeof(struct atbm_queue));
	queue->stats = stats;
		     ...
		     struct atbm_txpriv *txpriv)
{
	...
	if (!WARN_ON(list_empty(&queue->free_pool))) {
		...
		spin_unlock_bh(&stats->lock);
		atbm_printk_debug("[QueuePut][%d][%x][%p]\n", queue->num_queued, item->packetID, item->skb);
		/*
...
int atbm_queue_get(struct wsm_tx_encap *wsm_encap)
{
	int ret = -ENOENT;
	struct atbm_queue_item *item;
	struct atbm_queue *queue = wsm_encap->queue;
	struct atbm_queue_stats *stats = queue->stats;
	bool wakeup_stats = false;
	...
	list_for_each_entry(item, &queue->queue, head) {
		if ((item->txpriv.if_id == wsm_encap->priv->if_id) &&
			(wsm_encap->tx_allowed_mask & BIT(item->txpriv.link_id))) {
			ret = 0;
			break;
		}
	}
	...
	if (!WARN_ON(ret)) {
		wsm_encap->skb = item->skb;
		wsm_encap->txpriv = &item->txpriv;
		...
		*/
		...
		wsm_encap->packetID = __cpu_to_le32(item->packetID);
		...
		spin_unlock_bh(&stats->lock);
		atbm_printk_debug("[QueueGet][%x][%p]\n", wsm_encap->packetID, wsm_encap->skb);
#if 0
