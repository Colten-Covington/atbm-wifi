...
static void __atbm_sta_notify(struct atbm_vif *priv,
				...
				int link_id);
static int beacon_rate = 0xff;
module_param(beacon_rate,int,0644);
static int probe_rate  = 0xff;
module_param(probe_rate,int,0644);
/* ******************************************************************** */
/* AP API								*/
static void atbm_sta_supp_rate(struct atbm_common *hw_priv,struct ieee80211_sta *sta,struct wsm_supp_rate *rate)
{
    rate->supp_rates = cpu_to_le32(atbm_rate_mask_to_wsm(hw_priv,sta->supp_rates[hw_priv->channel->band]));
    rate->flags = 0;
    if(hw_priv->channel->band == IEEE80211_BAND_5GHZ){
        rate->flags |= cpu_to_le16(WSM_SUPPORT_RATE_FLAGS__ENABLE_5G);
    }
    if(sta->ht_cap.ht_supported && sta->ht_cap.mcs.rx_mask[0]){
        rate->flags |= cpu_to_le16(WSM_SUPPORT_RATE_FLAGS__ENABLE_HT);
        rate->ht_flags = cpu_to_le32(sta->ht_cap.mcs.rx_mask[0]);
    }
	...
	if((hw_priv != NULL) && (hw_priv->chip_version != OCEANUS_NO_WIFI6))
	{
	    if(sta->vht_cap.vht_supported){
	        ...
	        if(rate->vht_flags){
	            rate->flags |= cpu_to_le16(WSM_SUPPORT_RATE_FLAGS__ENABLE_VHT);
	        }

			if((hw_priv->channel->band == IEEE80211_BAND_2GHZ))
			{
				//For 2G VHT disabled specifically, Override rate flags here
				rate->vht_flags = 0;
				rate->flags &= cpu_to_le16(~WSM_SUPPORT_RATE_FLAGS__ENABLE_VHT);
			}
	    }
    ...
    if(sta->he_cap.has_he){
        switch(atbm_ieee80211_he_mcs_operation_rate(sta->he_cap.he_mcs_nss_supp.rx_mcs_80,1)){
        case 7:
        ...
        if(rate->he_flags){
            ...
            rate->flags |= cpu_to_le16(WSM_SUPPORT_RATE_FLAGS__ENABLE_HE);
        }
    }
    if(ieee80211_sta_support_40w(sta)){
        rate->flags |= cpu_to_le16(WSM_SUPPORT_RATE_FLAGS__ENABLE_40M);
    }

    atbm_printk_init("%s:flags[%x],supp_rate[%x][%x],ht_flags[%x],vht_flags[%x],he_flags[%x]\n",__func__,
            rate->flags,rate->supp_rates,sta->supp_rates[hw_priv->channel->band],rate->ht_flags,rate->vht_flags,rate->he_flags);
}
		   ...
		   struct ieee80211_sta *sta)
{
	...
	if(vif->type == NL80211_IFTYPE_STATION){
		...
		int ret = -1;
		if((bss = ieee80211_atbm_get_bss(hw_priv->hw->wiphy, hw_priv->channel,sta->addr, NULL, 0, 0, 0))){
			     ...
			     u32 changed)
{
	...
	atbm_printk_debug("atbm_bss_info_changed-%x\n", changed);
		...
		if (info->assoc || info->ibss_joined) { /* TODO: ibss_joined */
			...
			if (sta) {
				...
				* mac80211 changes are available */
                ...
                atbm_sta_supp_rate(hw_priv,sta,&priv->association_mode.rate);
			} else {
...
static int atbm_upload_beacon(struct atbm_vif *priv)
{
	...
	u8 *erp_inf, *ies, *ht_info, *ssid_ie;
#ifdef CONFIG_ATBM_AP_CHANNEL_CHANGE_EVENT
	u8 *ds_params;
#endif
	u32 ies_len;
	#define ATBM_HE_TRIM_BCN 1
	...
	if(hw_priv->chip_version == OCEANUS_NO_WIFI6)
	{
		struct sk_buff *skb;
		int offset = 0;
		const u8 *ssidie2 = NULL;
		const u8 *hecapie = NULL;
		const u8 *heopsie = NULL;
		const u8 *hecapie2 = NULL;
		const u8 *heopsie2 = NULL;

		u8 hecapie_len = 24;//21/22
		u8 heopsie_len = 9;//6/7
		u8 NeedTrimHeCapAgain = 0;
		u8 NeedTrimHeOpsAgain = 0;

		skb = frame.skb;
		hecapie = atbm_kmalloc(hecapie_len,GFP_KERNEL);
		heopsie = atbm_kmalloc(heopsie_len,GFP_KERNEL);

		offset = offsetof(struct atbm_ieee80211_mgmt, u.beacon.variable);
		ssidie2 = atbm_ieee80211_find_ie(ATBM_WLAN_EID_SSID, skb->data + offset,
						  skb->len - offset);

		hecapie =(const u8*) ieee80211_find_ext_elem(ATBM_WLAN_EID_EXT_HE_CAPABILITY, skb->data + offset, skb->len - offset);
		if(NULL == hecapie)
		{
			atbm_printk_err("hecapie is null\n");
		}

		heopsie = (const u8*)ieee80211_find_ext_elem(ATBM_WLAN_EID_EXT_HE_OPERATION, skb->data + offset, skb->len - offset);
		if(NULL == heopsie)
		{
			atbm_printk_err("heopsie is null\n");
		}
		...
		if(NULL != hecapie)
		{
			if(*(hecapie+1) == 22)
			{
				memmove(hecapie,heopsie,(ies + ies_len -(hecapie+24)));
		
				hecapie =(const u8*) ieee80211_find_ext_elem(ATBM_WLAN_EID_EXT_HE_CAPABILITY, skb->data + offset, skb->len - offset);
				if(hecapie == NULL)
				{
					atbm_printk_err("no hecap\n");
				}else{
					hecapie2 = hecapie;
					NeedTrimHeCapAgain = 1;
				}
				
				ssid_ie = (u8 *)atbm_ieee80211_find_ie(ATBM_WLAN_EID_SSID, ies, ies_len);
				if((NULL == ssid_ie))
				{
					atbm_printk_err("no ssid_ie\n");
				}else{
					atbm_printk_ap("ssid_ie:%p\n",ssid_ie);
				}

				heopsie = (const u8*)ieee80211_find_ext_elem(ATBM_WLAN_EID_EXT_HE_OPERATION, skb->data + offset, skb->len - offset);
				if((NULL == heopsie))
				{
					atbm_printk_err("no heopsie\n");
				}else{
					atbm_printk_ap("heopsie:%p\n",heopsie);
				}
		...
		if(NULL != heopsie)
		{
			if(*(heopsie+1) == 7)
			{
				ies_len -= 9;
				frame.skb->len -= 9;
				if(NeedTrimHeCapAgain != 1)
				{
					memset(heopsie,0,24+9);
				}
				atbm_printk_ap("skblen-%d\n",frame.skb->len);
			}
		}

		heopsie2 = (const u8 *)ieee80211_find_ext_elem(ATBM_WLAN_EID_EXT_HE_OPERATION, skb->data + offset, skb->len - offset);
		if(NULL != heopsie2)
		{
			atbm_printk_err("still heopsie2:%p\n",heopsie2);
			NeedTrimHeOpsAgain = 1;
		}

		if((NeedTrimHeCapAgain == 1)||(NeedTrimHeOpsAgain == 1))
		{
			if(NeedTrimHeCapAgain == 1)
			{
				memmove(hecapie2,heopsie2,(ies + ies_len -(hecapie2+24)));
				ies_len -= 24;
				frame.skb->len -= 24;
			}
			...
			if(NeedTrimHeOpsAgain == 1)
			{			
				if(*((heopsie+9)) == 221)
				{
					memmove(heopsie,(heopsie+9),(ies + ies_len -(heopsie+9)));
				}else{
					atbm_printk_ap("ignore as no ie\n");
				}
...
//	struct ieee80211_bss_conf *conf = &priv->vif->bss_conf;	
	...
	if(hw_priv->chip_version == OCEANUS_NO_WIFI6)
	{
		...
		if(NULL == hecapie)
		{
			atbm_printk_ap("hecapie is null\n");
		}

		heopsie = (const u8*)ieee80211_find_ext_elem(ATBM_WLAN_EID_EXT_HE_OPERATION, skb->data + offset, skb->len - offset);
		if(NULL == heopsie)
		{
			atbm_printk_ap("heopsie is null\n");
		}
		...
		if(NULL != hecapie)
		{
			if(*(hecapie+1) == 22)
			{
				memmove(hecapie,heopsie,(ies + ies_len -(hecapie+24)));//Need take care "HeApComp=0" case
			
				hecapie =(const u8*) ieee80211_find_ext_elem(ATBM_WLAN_EID_EXT_HE_CAPABILITY, skb->data + offset, skb->len - offset);
				if(hecapie == NULL)
				{
					atbm_printk_ap("prbrsp-no hecap\n");
				}else{
					NeedTrimHeCapAgain = 1;
				}
		...
		if(NULL != heopsie)
		{
			if(*(heopsie+1) == 7)
			{
				if(*((heopsie+9)) == 221)
				{
					...
					atbm_printk_ap("prbrsp-ignore\n");
				}
				
				
				ies_len -= 9;
				frame.skb->len -= 9;
				atbm_printk_debug("prbrsp-skblen-%d\n",frame.skb->len);
			}
		}
	}
...
static int atbm_start_ap(struct atbm_vif *priv)
{
	...
	int offset;
#endif
#if defined(CONFIG_ATBM_RADAR_DETECT) && LINUX_VERSION_IS_GEQ_OR_CPTCFG(3, 9, 0)
	...
	if(sdata->radar_required){
		start.mode |= WSM_FLAG_ENABLE_RADAR;
	}
#endif
	...
	atbm_printk_err("%s:start.channel_type,(%d),channelNumber(%d),rates[%x][%x]\n",__func__,start.channel_type,start.channelNumber,start.basicRateSet,conf->basic_rates);
