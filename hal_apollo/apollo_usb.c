...
#define ATBM_USB_EP0_MAX_SIZE DOWNLOAD_BLOCK_SIZE
...
#ifdef CONFIG_ATBM_USB_AUTO_WAKEUP
#undef RX_URB_NUM
#define RX_URB_NUM    8
#endif
#define ATBM_USB_AUTO_SUSPEND_MAX_DELAY    (30)
struct sbus_urb {
...
struct dvobj_priv{
    ...
    struct usb_anchor xmit_suspend;
    struct usb_anchor recv_suspend;
...
static const struct usb_device_id atbm_usb_ids[] = {
    ...
    {USB_DEVICE(WIFI_USB_VID, 0x6055)},
...
static void atbm_usb_resume_resubmit_recv_urb(struct sbus_priv *self);
...
#ifdef CONFIG_ATBM_USB_AUTO_WAKEUP
            if((rx_urb->test_urb->actual_length)){
                atbm_printk_err("resume err rx urb %d %d\n",rx_urb->test_urb->actual_length,self->suspend);

		    	usb_anchor_urb(err_urb, &self->drvobj->recv_suspend);
                if(self->suspend == 0){
                    atbm_usb_resume_resubmit_recv_urb(self);
                }
            }else {
			    atbm_usb_urb_put(self,self->drvobj->rx_urb_map,rx_urb->urb_id,0);
            }
...
int atbm_usb_sw_read_port(struct sbus_priv *self, u32 addr, void *pdata,int len)
{
    ...
    atbm_usb_pm(self,0);
	status = atbm_usbctrl_vendorreq_sync(self,request,0,wvalue, index, pdata,len);
    atbm_usb_pm(self,1);
    return status;
}
				   ...
				   const void *pdata, int len,sbus_callback_handler hander)
{
    ...
    if(self->xmit_suspend){
        atbm_printk_debug("usb suspend,wait resume\n");
        usb_anchor_urb(tx_urb->test_urb,&self->drvobj->xmit_suspend);
        status = 1;
        goto error;
    }
	usb_anchor_urb(tx_urb->test_urb, &self->drvobj->tx_submitted);
...
{
	atbm_printk_err("tx_urb_map %lx rx_urb_map %lx\n",self->drvobj->tx_urb_map[0],self->drvobj->rx_urb_map[0]);
...
static void atbm_usb_receive_data_complete(struct urb *urb)
{
    ...
    if(unlikely(RecvLength == 0)){
        atbm_printk_err("%s:stop status (%d)\n",__func__,urb->status);
        goto usb_wk;
    }else if(unlikely(urb->status)){
        atbm_printk_err("%s:urb status err(%d)\n",__func__,urb->status);
    }
...
int atbm_usb_pm(struct sbus_priv *self, bool  auto_suspend)
{
    ...
    atbm_printk_debug("%s:pm[%d]\n",__func__,auto_suspend);
...
#endif  //#if CONFIG_ATBM_USB_AUTO_WAKEUP
	//printk("***********func=%s,usage_count=%d\n",__func__,self->drvobj->pusbdev->dev.power.usage_count);
	return ret;
}
int atbm_usb_pm_async(struct sbus_priv *self, bool  auto_suspend)
...
static int atbm_usb_irq_unsubscribe(struct sbus_priv *self)
{
  ...
  //  int status;
    ...
    while((urb = usb_get_from_anchor(&self->drvobj->xmit_suspend))){ 
		usb_free_urb(urb);
    }
...
//#define ALIGN(a,b)  (((a)+((b)-1))&(~((b)-1)))
    ...
    init_usb_anchor(&dvobj->xmit_suspend);
    init_usb_anchor(&dvobj->recv_suspend);
#ifdef CONDIF_ATBM_CTRL_REQUEST_ASYNC
...
#ifdef CONFIG_PM
#ifdef CONFIG_ATBM_USB_AUTO_WAKEUP	
	atbm_printk_init("CONFIG_ATBM_USB_AUTO_WAKEUP\n");
#if (LINUX_VERSION_CODE>=KERNEL_VERSION(2,6,35))
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2,6,18))
	{
		...
		pm_runtime_set_autosuspend_delay(&dvobj->pusbdev->dev,ATBM_USB_AUTO_SUSPEND_MAX_DELAY);
	}
#endif //(LINUX_VERSION_CODE > KERNEL_VERSION(2,6,18))
#endif //CONFIG_PM

#else //CONFIG_ATBM_USB_AUTO_WAKEUP
#if (LINUX_VERSION_CODE>=KERNEL_VERSION(2,6,35))
...
#endif
static void atbm_usb_resume_resubmit_xmit_urb(struct sbus_priv *self)
{
	struct dvobj_priv *dvobj = self->drvobj;
    struct sbus_urb *tx_urb;
    struct urb *urb;
    int status;
    while((urb = usb_get_from_anchor(&dvobj->xmit_suspend))){ 
		tx_urb = (struct sbus_urb*)urb->context;

		usb_free_urb(urb);

        usb_anchor_urb(urb, &dvobj->tx_submitted);
        ...
        atbm_printk_debug("resume submit urb(%d)\n",status);
...
static void atbm_usb_resume_resubmit_recv_urb(struct sbus_priv *self)
{
    ...
    while((urb = usb_get_from_anchor(&dvobj->recv_suspend))){ 
		rx_urb = (struct sbus_urb*)urb->context;
...
again:
		...
		usb_fill_bulk_urb(urb, 
						  dvobj->pusbdev, 
						  usb_rcvbulkpipe(dvobj->pusbdev, dvobj->ep_in),
						  rx_urb->test_skb->data + rx_urb->offset,RX_BUFFER_SIZE - rx_urb->offset,atbm_usb_receive_data_complete,rx_urb);
		usb_anchor_urb(urb, &dvobj->rx_submitted);
		status = usb_submit_urb(urb, GFP_ATOMIC);
		if (status) {
			usb_unanchor_urb(urb);
			atbm_printk_err("usb_submit_urb rx err %d %d\n",status,rx_urb->retries);
...
static int atbm_usb_resume(struct usb_interface *intf)
{
    ...
    atbm_usb_resume_resubmit_xmit_urb(self);
...
static int atbm_usb_suspend(struct usb_interface *intf,pm_message_t message)
{
    ...
    if(PMSG_IS_AUTO(message)){
         /*
          * not systerm suspend ,so check if rx is in busy state
          * */
         if((hw_priv->bh_running == true) || !atbm_skb_queue_empty(&hw_priv->rx_frame_queue)){
             busy = -EBUSY;
         }
    }
	spin_unlock_irqrestore(&hw_priv->rx_frame_queue.lock,flags);
    ...
    do{
        ...
        /*
         * suspend tx ,if having penning urb ,
         * it will submit urb after resume.
         * */
	    spin_lock_bh(&dvobj->self->xmit_path_lock);
	    dvobj->self->xmit_suspend=1;
	    spin_unlock_bh(&dvobj->self->xmit_path_lock);
        /*
         * wait tx complete
         */
        ret = usb_wait_anchor_empty_timeout(&dvobj->tx_submitted,HZ);
	    if(ret == 0){
		    atbm_printk_err("tx_submitted cancle timeout(%d) \n",usb_anchor_empty(&dvobj->tx_submitted));
            busy = -EBUSY;
            break;
	    }
		...
		atbm_flush_workqueue(self->rx_workqueue);
#endif
        if(!usb_anchor_empty(&dvobj->recv_suspend) && PMSG_IS_AUTO(message)){
            busy = -EBUSY;
            atbm_usb_resume(intf);
            break;
        }
        /*
         * all rx urb and tx urb in idle state
         * */
    }while(0);

	atbm_printk_pm("usb_suspend(%d)\n",busy);
	return busy;
#else
	struct dvobj_priv *dvobj = usb_get_intfdata(intf);
	int ret = 0;
...
static struct usb_driver atbm_usb_driver = {
...
#endif  //CONFIG_ATBM_USB_AUTO_WAKEUP
...
static int atbm_usb_register(void)
{
	...
	ret = usb_register(&atbm_usb_driver);

	driver_ref->atbm_driver = ret?NULL:&atbm_usb_driver;
...
retry:
	...
	intf = usb_find_interface(&atbm_usb_driver,-1);
...
static void  atbm_usb_fw_sync(struct atbm_common *hw_priv,struct dvobj_priv *dvobj)
{
	...
	*/
	...
	if(/*(hw_priv->wsm_caps.HiHwCnfBufaddr  & 0xFFF00000) == 0x9000000*/hw_priv->wsm_caps.HiHwCnfBufaddr){
