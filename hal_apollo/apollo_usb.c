...
struct BLE_ops * ble_comb_ops = NULL;
...
static int atbm_usb_adjust_wsm(struct sbus_priv *self,void *wsm)
{
...
static struct sbus_ops atbm_usb_sbus_ops = {
	...
	.sbus_adjust           = atbm_usb_adjust_wsm,
};
...
{
	int ret = 0;
	atbm_printk_err("atbm_usb_probe : idVendor[%x] idProduct[%x] \n",id->idVendor,id->idProduct);
	if(atbm_wifi_chip_probe_get() != 0){
		atbm_printk_err("current wifi is running! \n");
		return -1;
	}
...
#if 0
void atbm_wifi_bt_comb_register(struct BLE_ops * comb_ops)
{
	struct sbus_priv *self  = atbm_wifi_comb_buspriv_get();
	if(self){
		unsigned long flags=0;
		spin_lock_irqsave(&self->lock, flags);
...
EXPORT_SYMBOL(atbm_wifi_bt_comb_register);
void atbm_wifi_bt_comb_unregister(void)
{
	struct sbus_priv *self  = atbm_wifi_comb_buspriv_get();
	unsigned long flags=0;
	if(self){
...
EXPORT_SYMBOL(atbm_wifi_bt_comb_unregister);
#endif
