                        ...
                        u32 buf_len)
{
        int ret;
		int retry=0;
		atbm_printk_debug(KERN_ERR "%s: addr %x\n",__func__,addr);
        if (buf_len  >= 512) {
                atbm_dbg(ATBM_APOLLO_DBG_ERROR,
                                "%s: Can't wrire more than 0xfff words.\n",
                                __func__);
...
{
	u8 valu8 ;
	extern int  atbm_readb_func(struct atbm_common *hw_priv,unsigned int addr,u8 *dst);
	extern int  atbm_writeb_func(struct atbm_common *hw_priv,unsigned int addr,u8 b);

	valu8 = 0;
	ret = 0;
	ret = atbm_writeb_func(hw_priv,24,0xff);
	ret |= atbm_readb_func(hw_priv,24,&valu8);
	atbm_printk_err("contr24[%x][%d]\n",valu8,ret);

	valu8 = 0;
	ret = 0;
	ret = atbm_writeb_func(hw_priv,25,0xff);
	ret |= atbm_readb_func(hw_priv,25,&valu8);
	atbm_printk_err("contr25[%x][%d]\n",valu8,ret);

	valu8 = 0;
	ret = 0;
	ret = atbm_writeb_func(hw_priv,26,0xff);
	ret |= atbm_readb_func(hw_priv,26,&valu8);
	atbm_printk_err("contr26[%x][%d]\n",valu8,ret);

	valu8 = 0;
	ret = 0;
	ret = atbm_writeb_func(hw_priv,27,0xff);
	ret |= atbm_readb_func(hw_priv,27,&valu8);
	atbm_printk_err("contr27[%x][%d]\n",valu8,ret);

	do{
		ret = atbm_reg_read_32(hw_priv,6,&val32);
		msleep(200);
		atbm_printk_err("contrl  ret(%d)[%x]\n",ret,val32);
	}while(1);
}
#endif
...
retry:
	...
	do {
		ret = atbm_direct_read_reg_32(hw_priv, 0x16101028, &val32);
		if (val32 == 5) {
			break;
		}
	} while (1);
...
#if 0
        /********************************************/
        /********sdio io latch release*******/
		ret=atbm_ahb_write_32(hw_priv,0x161010dc,0x0a);
        if(ret<0){
            atbm_dbg(ATBM_APOLLO_DBG_ERROR,
                    "%s: 0x161010dc: can't write register.\n", __func__);
            goto out;
...
//		u16 val16;
		...
		val32 &= ~(0xffff0000);
		val32 |= BIT(0) | BIT(1) | (DOWNLOAD_ITCM_ADDR );
...
//			if (val32 & BIT(16))
...
#if  0
		/****************************/
                ret=atbm_ahb_read_32(hw_priv,0x161010dc,&val32);
                if(ret<0){
                        atbm_dbg(ATBM_APOLLO_DBG_ERROR,
                                "%s: 0x161010dc: can't read register.\n", __func__);
                        goto out;
                }

                val32 &= ~BIT(0);
                ret=atbm_ahb_write_32(hw_priv,0x161010dc,val32);
                if(ret<0){
                        atbm_dbg(ATBM_APOLLO_DBG_ERROR,
                                "%s: 0x161010dc: can't write register.\n", __func__);
                        goto out;
		...
		atbm_printk_err("ATBM_HIFREG_CONFIG_REG_ID(%x)\n",val32);
		val32 |= ATBM_HIFREG_CONF_IRQ_RDY_ENABLE;
		...
		ret = hw_priv->sbus_ops->sbus_wait_data_xmited ? 
		...
		while(1){
			...
			ret = atbm_reg_read_32(hw_priv, ATBM_HIFREG_CONFIG_REG_ID, &val32);
			atbm_printk_err("ATBM_HIFREG_CONFIG_REG_ID(%x)\n",val32);
			ret = atbm_reg_read_32(hw_priv,ATBM_HIFREG_CONTROL_REG_ID, &val32);
			atbm_printk_err("ATBM_HIFREG_CONTROL_REG_ID(%x)\n",val32);
		}
#endif
	out:
		return ret;
