...
static int ht_40M = 1;
module_param(ht_40M, int, 0644);
static int vht_40M = 1;
module_param(vht_40M, int, 0644);
static int he_40M = 1;
module_param(he_40M, int, 0644);
#ifdef CONFIG_ATBM_APOLLO_DEBUG
extern int atbm_debug_init_common(void * priv);
#endif
//static char *init_dev_name = "ALTOBEAM";
bool ieee80211_support_ht_40M(void)
{
    return ht_40M ? true:false;
}

bool ieee80211_support_vht_40M(void)
{
    return vht_40M ? true:false;
}

bool ieee80211_support_he_40M(void)
{
    return he_40M ? true:false;
}
				      ...
				      u32 changed)
{
	...
	if (local->hw.flags & IEEE80211_HW_SUPPORTS_MULTI_CHANNEL)
		ieee80211_recalc_channel(NULL, sdata, &changed);
#endif //CONFIG_ATBM_SUPPORT_MULTI_CHANNEL
		...
		*/
		...
		switch(skb->pkt_type){
			...
			case IEEE80211_SPECIAL_CHANGE_CHANNEL_FREQ:
				...
				ieee80211_start_csa_work(sdata,(struct ieee80211_csa_request *)skb->data);
...
void atbm_get_cfg80211_country_code(struct wiphy *wiphy,struct regulatory_request *request)
{
	...
	if((!wiphy) || (!request)){
		atbm_printk_err("%s ,%s  \n",
			wiphy?__func__:"wiphy is NULL",
			request?__func__:"request is NULL");
		return;
	}
	...
	if(cfg80211_regdomain){
		...
		if(memcmp(request->alpha2,"JP",2) == 0 || memcmp(request->alpha2,"01",2) == 0){
			atbm_printk_err("end freq = %d \n",cfg80211_regdomain->reg_rules[1].freq_range.end_freq_khz);
		...
		if(memcmp(request->alpha2,"00",2) != 0){
			atbm_set_country_code_on_driver(local,request->alpha2);
			return;
		}
		local->country_support_chan = country_chan_found(local->country_code);
	}
	if(memcmp(request->alpha2,"01",2) == 0)
		memcpy(local->country_code,"00",2);
...
struct ieee80211_hw *ieee80211_alloc_hw(size_t priv_data_len, const struct ieee80211_ops *ops)
{
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3, 8, 0)) || (CONFIG_CFG80211_COMPAT_MODIFIED==1)
	...
	local->hw.max_hw_support_rx_aggs = ATBM_IEEE80211_MAX_RX_AGGR;
#ifdef WLAN_CAPABILITY_RADIO_MEASURE
	if(1/*local->ext_wfa_test_flag == 1*/){
		local->ext_capa[2] = BIT(3); /*BTM Cap*/
		wiphy->extended_capabilities = local->ext_capa;
		wiphy->extended_capabilities_mask = local->ext_capa;
		wiphy->extended_capabilities_len =
			ARRAY_SIZE(local->ext_capa);
#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(4, 5, 0))		
	...
	atbm_skb_queue_head_init(&local->scan_rx);
	...
	spin_lock_init(&local->aggr_lock);
...
#ifdef CONFIG_ATBM_RADAR_DETECT
#if LINUX_VERSION_IS_GEQ_OR_CPTCFG(3, 9, 0)
	ATBM_INIT_WORK(&local->radar_detected_work,
...
#ifdef CONFIG_ATBM_SUPPORT_CSA
	ieee80211_chan_csa_local_init(local);
#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(3,12,0) || (LINUX_VERSION_CODE >= KERNEL_VERSION(3,16,0))) ||(CONFIG_ATBM_LOW_KERNEL_SUPPORT_CSA == 1)
	wiphy->flags |= WIPHY_FLAG_HAS_CHANNEL_SWITCH;

	wiphy->max_num_csa_counters = 2;
#endif
#endif
	__set_bit(0,local->rx_aggr_map);
	return local_to_hw(local);
}
#ifdef CONFIG_CFG880211_SUPPORT_HE
//EXPORT_SYMBOL(ieee80211_alloc_hw);
static void ieee80211_he_iftype_data_init(struct ieee80211_sband_iftype_data *iftype_data,
				struct atbm_ieee80211_sta_he_cap *he_cap)
{
	...
	memcpy(iftype_data->he_cap.he_cap_elem.mac_cap_info,he_cap->he_cap_elem.mac_cap_info,6);
	memcpy(iftype_data->he_cap.he_cap_elem.phy_cap_info,he_cap->he_cap_elem.phy_cap_info,11);
	iftype_data->he_cap.he_mcs_nss_supp.rx_mcs_80 = he_cap->he_mcs_nss_supp.rx_mcs_80;
	iftype_data->he_cap.he_mcs_nss_supp.tx_mcs_80 = he_cap->he_mcs_nss_supp.tx_mcs_80;
	iftype_data->he_cap.he_mcs_nss_supp.rx_mcs_160 = he_cap->he_mcs_nss_supp.rx_mcs_160;
	iftype_data->he_cap.he_mcs_nss_supp.tx_mcs_160 = he_cap->he_mcs_nss_supp.tx_mcs_160;
	iftype_data->he_cap.he_mcs_nss_supp.rx_mcs_80p80 = he_cap->he_mcs_nss_supp.rx_mcs_80p80;
	iftype_data->he_cap.he_mcs_nss_supp.tx_mcs_80p80 = he_cap->he_mcs_nss_supp.tx_mcs_80p80;
	memcpy(iftype_data->he_cap.ppe_thres,he_cap->ppe_thres,ATBM_IEEE80211_HE_PPE_THRES_MAX_LEN);
}
static void ieee80211_alloc_he_iftype_data(struct ieee80211_local *local)
{
...
//	struct ieee80211_supported_band *sband;
	...
	for (band = 0; band < IEEE80211_NUM_BANDS; band++) {
		...
		case IEEE80211_BAND_2GHZ:
			...
			if(local->hw.he_cap_2g4 == NULL){
				break;
			}
			
			iftype_data = atbm_kzalloc(sizeof(struct ieee80211_sband_iftype_data),GFP_KERNEL);
			if(iftype_data){
				sband->n_iftype_data = 1;
				sband->iftype_data = iftype_data;
				ieee80211_he_iftype_data_init(iftype_data,local->hw.he_cap_2g4);
			}
			break;
		case IEEE80211_BAND_5GHZ:
			if(local->hw.he_cap_5g == NULL){
				break;
			}
			...
			if(iftype_data){
				sband->n_iftype_data = 1;
				sband->iftype_data = iftype_data;
				ieee80211_he_iftype_data_init(iftype_data,local->hw.he_cap_5g);
			}
			break;
		default:
			BUG_ON(1);
...
static void ieee80211_free_he_iftype_data(struct ieee80211_local *local)
{
	enum ieee80211_band band;
//	struct ieee80211_supported_band *sband;
	...
	for (band = 0; band < IEEE80211_NUM_BANDS; band++) {
		...
		if(sband->iftype_data){
			iftype_data = sband->iftype_data;
			sband->n_iftype_data = 0;
			sband->iftype_data = NULL;
			atbm_kfree(iftype_data);
		}
	}	
}
#endif
#ifdef CONFIG_CFG880211_SUPPORT_VHT
static void ieee80211_set_sband_vht_cap(struct ieee80211_local *local)
{
    enum ieee80211_band band;
	...
	for (band = 0; band < IEEE80211_NUM_BANDS; band++) {
		...
		case IEEE80211_BAND_2GHZ:
			...
			if(local->hw.vht_cap_2g4 == NULL){
				break;
			} 	
            vht_cap = local->hw.vht_cap_2g4;
			break;
		case IEEE80211_BAND_5GHZ:

			if(local->hw.vht_cap_5g == NULL){
				break;
			}
            vht_cap = local->hw.vht_cap_5g;
			break;
		default:
			BUG_ON(1);
			break;
        ...
        if(vht_cap){
            sband->vht_cap.vht_mcs.rx_highest = vht_cap->vht_mcs.rx_highest;
            sband->vht_cap.vht_mcs.rx_mcs_map = vht_cap->vht_mcs.rx_mcs_map;
            sband->vht_cap.vht_mcs.tx_highest = vht_cap->vht_mcs.tx_highest;
            sband->vht_cap.vht_mcs.tx_mcs_map = vht_cap->vht_mcs.tx_mcs_map;
            sband->vht_cap.vht_supported      = vht_cap->vht_supported;
            sband->vht_cap.cap                = vht_cap->cap;
        }
	}	
...
int ieee80211_register_hw(struct ieee80211_hw *hw)
{
    ...
    if(local->hw.vht_cap_2g4 || local->hw.vht_cap_5g){
        supp_vht = true;
    }
#ifdef CONFIG_ATBM_HE
    if(local->hw.he_cap_2g4 || local->hw.he_cap_5g){
        supp_he = true;
    }
#endif
...
#ifdef CONFIG_CFG880211_SUPPORT_HE
	ieee80211_alloc_he_iftype_data(local);
#endif
#ifdef CONFIG_CFG880211_SUPPORT_VHT
    ieee80211_set_sband_vht_cap(local);
#endif
    ...
    if (supp_vht)
        local->scan_ies_len += 2 + sizeof(struct atbm_ieee80211_vht_cap);

    if (supp_he){
        local->scan_ies_len += 3 + sizeof(struct atbm_ieee80211_he_cap_elem) + 
            sizeof(struct atbm_ieee80211_he_mcs_nss_supp) + ATBM_IEEE80211_HE_PPE_THRES_MAX_LEN;
    }
...
#ifdef SIGMSTAR_SCAN_FEATURE
{
	...
	local->hw.wiphy->n_vendor_commands = 0;
#endif
...
void ieee80211_unregister_hw(struct ieee80211_hw *hw)
{
...
#ifdef CONFIG_ATBM_SUPPORT_CSA
	ieee80211_chan_csa_local_exit(local);
#endif
#ifdef CONFIG_ATBM_BLE
...
#ifdef CONFIG_ATBM_RADAR_DETECT
#if LINUX_VERSION_IS_GEQ_OR_CPTCFG(3, 9, 0)
	atbm_cancel_work_sync(&local->radar_detected_work);
#endif
#endif
	...
	atbm_skb_queue_purge(&local->scan_rx);
...
#ifdef CONFIG_CFG880211_SUPPORT_HE
	ieee80211_free_he_iftype_data(local);
#endif
