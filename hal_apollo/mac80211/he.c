...
int he_rx_mcs = 2;
module_param(he_rx_mcs, int, 0644);
int he_tx_mcs = 2;
module_param(he_tx_mcs, int, 0644);
 ...
 */
  ...
  unsigned char peer_tx;
  unsigned char peer_rx;
  ...
  peer_tx = min(((he_own_rx[3] >> 3)&ATBM_IEEE80211_HE_PHY_CAP3_DCM_MAX_CONST_TX_MASK), (he_peer_tx[3] & 0x07));
  peer_rx = min((he_own_tx[3] & ATBM_IEEE80211_HE_PHY_CAP3_DCM_MAX_CONST_TX_MASK) , ((he_peer_rx[3] >> 3) & 0x07));
  he_peer_rx[3] &= 0xe7; //clear rx bits
  he_peer_rx[3] |= (peer_rx << 3); //set rx bits
  he_peer_tx[3] &= 0xfc; //clear tx bits
  he_peer_tx[3] |= peer_tx; //set tx bits
				  ...
				  struct sta_info *sta)
{
	...
	memcpy(&own_he_cap,atbm_ieee80211_get_he_iftype_cap(&sdata->local->hw,sband,ieee80211_vif_type_p2p(&sdata->vif),sdata),sizeof(own_he_cap));
