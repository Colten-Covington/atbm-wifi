			       ...
			       u64 filter, u32 crc/*,  struct element *check_inherit*/)
{
	...
	while (left >= 2) {
		...
		case ATBM_WLAN_EID_HT_INFORMATION:
			if (elen >= sizeof(struct atbm_ieee80211_ht_info))
				elems->ht_info_elem = (void *)pos;
			break;
#ifdef CONFIG_ATBM_VHT
        case ATBM_WLAN_EID_VHT_CAPABILITY:
            if(elen >= sizeof(struct atbm_ieee80211_vht_cap))
                elems->vht_cap_elem = (void*)pos;
            break;
        case ATBM_WLAN_EID_VHT_OPERATION:
            if(elen >= sizeof(struct atbm_ieee80211_vht_operation))
			      ...
			      enum ieee80211_band band)
{
	...
	if (band == IEEE80211_BAND_2GHZ)
		mandatory_flag = IEEE80211_RATE_MANDATORY_B | IEEE80211_RATE_MANDATORY_G;
	else
...
u8 *ieee80211_ie_build_vht_cap(u8 *pos, struct atbm_ieee80211_sta_vht_cap *vht_cap,u32 cap)
{
    ...
    *pos++ = ATBM_WLAN_EID_VHT_CAPABILITY;
    *pos++ = sizeof(struct atbm_ieee80211_vht_cap);
    memset(pos, 0, sizeof(struct atbm_ieee80211_vht_cap));
			     ...
			     u8 channel)
{
	...
	*pos++ = supp_rates_len;
	...
	{
	    atbm_printk_init("add vht\n");
	    if(atbm_ieee80211_vht_supported(&local->hw,sband)==true){
	        __le32 tmp;
	        const struct atbm_ieee80211_sta_vht_cap *vht_cap = atbm_ieee80211_get_vht_cap(&local->hw,sband,sdata);
	        BUG_ON(vht_cap == NULL);

	        *pos++ = ATBM_WLAN_EID_VHT_CAPABILITY;
	        *pos++ = sizeof(struct atbm_ieee80211_vht_cap);
	        
	        tmp = cpu_to_le32(vht_cap->cap);
	        memcpy(pos,&tmp,sizeof(u32));
	        pos += sizeof(u32);
	...
	if(local->scan_sdata){
		he_cap = atbm_ieee80211_get_he_iftype_cap(&local->hw,sband,
						     ieee80211_vif_type_p2p(&local->scan_sdata->vif),sdata);
		if (he_cap) {
...
{
	#if LINUX_VERSION_IS_LESS_AND_NOT_CPTCFG(4, 4, 0)
				      ...
				      struct sk_buff *skb)
{
	...
	case WSM_EVENT_BSS_LOST:
        if(ieee80211_sdata_running(sdata)){
#ifdef CONFIG_SUPPORT_80M_ADAPTER
			if(wk->event.data==2/*invalid param, 80M Adapter*/){
				if((atbm_80M_adapter & ATBM_ADAPTER_LMAC)){
					sdata->bSupport40M_Next = 0;
				}
				atbm_printk_always("80M_adapter:lmac event to change to 20M\n");									
			}
#endif  //#ifdef  CONFIG_SUPPORT_80M_ADAPTER
...
#if LINUX_VERSION_IS_GEQ_OR_CPTCFG(3, 9, 0)
		{
			struct ieee80211_channel *cac_channel;
			enum atbm_nl80211_channel_type cac_channel_type;

			struct ieee80211_local *local = sdata->local;
			atbm_printk_err("Radar detected chan:%d chantype:%d ", (wk->event.data>>8)&0xff, (wk->event.data>>16)&0xff);
			cac_channel = ieee80211_get_channel_khz(local->hw.wiphy,
				ieee80211_channel_to_freq_khz((wk->event.data>>8)&0xff, NL80211_BAND_5GHZ));
			cac_channel_type = (wk->event.data>>16)&0xff;
			WARN_ON(!cac_channel);
			cfg80211_chandef_create(&local->cac_detect_chan_def, cac_channel, cac_channel_type);
			ieee80211_radar_detected(local_to_hw(local));
		}
#endif
#endif
		break;
...
static int ieee80211_build_encrype(struct ieee80211_tx_encap *encap)
{
	...
	*/
    ...
    if(unlikely(encap->hdrlen > encap->buff_len)){
        goto fail;
    }
...
static int ieee80211_build_80211_payload_tso(struct ieee80211_tx_encap *encap)
{
	...
	*/
    ...
    if(unlikely(encap->hdrlen + noffset + 
                tso->network_len + tso->transport_len +
                skb_shinfo(skb)->gso_size > encap->buff_len)){
        goto err;
    }
...
#endif
    if(unlikely(encap->hdrlen + skb->len - encap->skip_len > encap->buff_len)){
        goto fail;
    }
...
static int ieee80211_build_up_80211_frame_fast(struct ieee80211_tx_encap *encap)
{
    ...
    if(unlikely(sta_hdr->hdr_len > encap->buff_len)){
        goto fail;
    }
...
static int ieee80211_build_up_to_80211_frame_slow(struct ieee80211_tx_encap *encap)
{
    ...
    if(unlikely(encap->buff_len < hdrlen)){
        goto fail;
    }
    ...
    if(info->flags & IEEE80211_TX_CTL_QOS){
        ...
        memcpy(encap->xmit_buff,&hdr,hdrlen-2);
		tid = encap->skb->priority & IEEE80211_QOS_CTL_TAG1D_MASK;
        p = encap->xmit_buff + hdrlen - 2;
		*p = 0;
		/* preserve EOSP bit */
		...
		*p = ieee80211_vif_is_mesh(&encap->sdata->vif) ?
        ...
        memcpy(encap->xmit_buff,&hdr,hdrlen);
    }
...
#endif
#ifdef CONFIG_SUPPORT_AMSDU
static int ieee80211_encap_amsdu(struct ieee80211_tx_encap *encap,struct sk_buff *skb,int frame_len)
{
    int subframe_len = frame_len - ETH_ALEN;
    __be16 len;

    if(unlikely(encap->hdrlen + 
                2*ETH_ALEN + 2 + sizeof(rfc1042_header) + 
                subframe_len - ETH_ALEN > encap->buff_len)){
       return -1; 
    }
    /*
     * MSDU
    ...
    */
    ...
    /* 1. copy da sa*/
    if(skb_copy_bits(skb,0,encap->xmit_buff + encap->hdrlen,2*ETH_ALEN)){
		return -1;
	}
    encap->hdrlen   += 2 * ETH_HLEN;
    /* 2. set subframe_len*/
    len = cpu_to_be16(subframe_len);
    memcpy(encap->xmit_buff + encap->hdrlen,&len,2);
    encap->hdrlen  += 2;
    /* 3. copy rfc1042*/
    memcpy(encap->xmit_buff + encap->hdrlen,rfc1042_header,sizeof(rfc1042_header));
    encap->hdrlen  += sizeof(rfc1042_header);
    /* 4. copy data*/
    if(skb_copy_bits(skb,2 * ETH_ALEN,encap->xmit_buff + encap->hdrlen,subframe_len - ETH_ALEN)){
		return -1;
	}
    return 0;

}
static int ieee80211_build_up_to_amsdus_frame(struct ieee80211_tx_encap *encap)
{
    struct sk_buff *subframe = encap->skb;
	struct ieee80211_tx_info* info = IEEE80211_SKB_CB(subframe);
    struct sk_buff *segs;

    /* * first amsdu **/
    if(ieee80211_encap_amsdu(encap,subframe,info->control.amsdu_len.head_len)){
        atbm_printk_err("%s:encap amsdu err\n",__func__);
        goto fail;
    }

    /* get amsdu segs */
    segs = skb_shinfo(subframe)->frag_list;

    while(segs){
        struct sk_buff *next = segs->next;
       /*alin with 4 bytes*/
        if(encap->hdrlen % 4){
            encap->hdrlen -= (encap->hdrlen % 4);
            encap->hdrlen += 4;
        }

        if(ieee80211_encap_amsdu(encap,segs,segs->len)){
            atbm_printk_err("%s: encap segs amsdu err\n",__func__);
            goto fail;
        }
        /*move next*/
        segs = next;
...
static int ieee80211_build_up_to_80211_frame(struct ieee80211_tx_encap *encap)
{
    ...
    if(info->amsdu){
        u16 tid;
        struct sk_buff_head *amsdus_list;

        tid = encap->skb->priority & 0x07;
        amsdus_list = &encap->sta->amsdu_buf[tid]; 

        /* * delete from queue */
        spin_lock_bh(&amsdus_list->lock);
        __atbm_skb_unlink(encap->skb,amsdus_list);
        spin_unlock_bh(&amsdus_list->lock);

        if(info->control.amsdu_len.head_len != info->control.amsdu_len.total_len){
            BUG_ON(info->control.amsdu_len.head_len > info->control.amsdu_len.total_len);
            if(ieee80211_build_up_to_amsdus_frame(encap)){
                goto fail;
            }
            return 0;
        }
...
static int ieee80211_build_raw_frame(struct ieee80211_tx_encap *encap)
{
	...
	if (encap->hw_key && (encap->hw_key->cipher == WLAN_CIPHER_SUITE_AES_CMAC)) {
		struct ieee80211_mmie* mmie = (struct ieee80211_mmie*)(encap->xmit_buff + encap->hdrlen);

        if(unlikely(encap->hdrlen + sizeof(struct ieee80211_mmie) > encap->buff_len)){
            goto fail;
        }
		memset(mmie,0,sizeof(struct ieee80211_mmie));
		mmie->element_id = WLAN_EID_MMIE;
		mmie->length = sizeof(*mmie) - 2;
		mmie->key_id = cpu_to_le16(encap->hw_key->keyidx);
...
static void ieee80211_build_rate_policy(struct ieee80211_tx_encap *encap)
{
	...
	txrc.info = info;
	txrc.txrc_in.lower = (info->flags&(IEEE80211_TX_CTL_USE_MINRATE | IEEE80211_TX_CTL_NO_ACK)) || (encap->sta == NULL);
	if((tx_rate != 0xFF) && (txrc.txrc_in.lower == 0)){
        ...
        info->flags |= IEEE80211_TX_FIXED_RATE;
    }else {
...
{
	struct ieee80211_sub_if_data* sdata = vif_to_sdata(build_encap->vif);
	struct ieee80211_tx_info* info = IEEE80211_SKB_CB(build_encap->skb);
	...
	*/
	if(build_encap->sta){
		sta = container_of(build_encap->sta, struct sta_info, sta);
		sdata = sta->sdata;
		BUG_ON(sdata == NULL);
	}
	...
	encap.sdata = sdata;
	encap.sta   = sta;
	encap.skb   = build_encap->skb;
	encap.xmit_buff = build_encap->xmit_buff;
    encap.buff_len  = build_encap->buff_len;
    encap.hw_key    = NULL;
	encap.hdrlen = 0;
	encap.remain_len = 0;
	...
	*/
	if (sta && (info->flags & IEEE80211_TX_CTL_QOS)) {
		u16* seq;
		struct ieee80211_hdr* hdr = (struct ieee80211_hdr*)build_encap->xmit_buff;
		seq = &sta->tid_seq[build_encap->skb->priority & IEEE80211_QOS_CTL_TAG1D_MASK];
	...
	*/
    ...
    build_encap->hw_key = encap.hw_key;
	atbm_printk_debug("[Build80211][%d->%d][%x]\n", build_encap->skb->len, encap.hdrlen, *((u16*)build_encap->xmit_buff));
	return encap.hdrlen;
fail:
	return 0;
}
...
void ieee80211_dfs_cac_cancel(struct ieee80211_local *local)
{
	...
	atbm_printk_err("Radar detect cancel\n");
...
void ieee80211_dfs_radar_detected_work(struct atbm_work_struct *work)
{
	...
	cfg80211_radar_event(local->hw.wiphy, &local->cac_detect_chan_def, GFP_KERNEL);
}
