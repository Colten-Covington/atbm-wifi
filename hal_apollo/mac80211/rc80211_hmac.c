...
u8				n_rate_group[10] =
{
	13, 26, 39, 52, 78, 104, 117, 130, 156, 176
};
// n rate; the real rate*2; 130 is mcs7/ long gi
u8				n_rate_groups[10] =
{
	14, 28, 43, 58, 87, 116, 130, 144, 173, 195
};
...
extern int rate_for_gi;  /*manual debug options*/

unsigned char bw_select_40m_lite(struct RATE_CONTROL_RATE_S * rates_info);
unsigned char bw_select_40m(struct RATE_CONTROL_RATE_S * rates_info);
unsigned char bw_select_40m_simple(struct RATE_CONTROL_RATE_S * rates_info);
...
const s8		RateIndexMapFromPhyToDescr[48] =
{
	/*11b rate index*/
	...
	DESCR_RATE_INDEX_HE_MSC11, 
   	...
   	DESCR_RATE_INDEX_VHT_MCS0, //RATE 38,  RATE_INDEX_VHT_MCS0
	DESCR_RATE_INDEX_VHT_MCS1,
	DESCR_RATE_INDEX_VHT_MCS2,
	DESCR_RATE_INDEX_VHT_MCS3,
	DESCR_RATE_INDEX_VHT_MCS4,
	DESCR_RATE_INDEX_VHT_MCS5,
	DESCR_RATE_INDEX_VHT_MCS6,
	DESCR_RATE_INDEX_VHT_MCS7,
	DESCR_RATE_INDEX_VHT_MCS8,
	DESCR_RATE_INDEX_VHT_MCS9, //RATE 47,  RATE_INDEX_VHT_MCS9
};
...
const s8		RateIndexMapFromDescrToPhyVht[38] =
{
	/*HE index*/
	RATE_INDEX_HE_MCS11, 
	RATE_INDEX_HE_MCS10, 
	RATE_INDEX_VHT_MCS9,  //2 
	RATE_INDEX_VHT_MCS8, 
	RATE_INDEX_VHT_MCS7, 
	RATE_INDEX_VHT_MCS6, 
	RATE_INDEX_VHT_MCS5, 
...
s8 descr_rate_index_to_phy(s8 DescrRateIndex, struct RATE_CONTROL_RATE_S * rates_info)
{
   if(rates_info->vht_mode_on)
    {
        return RateIndexMapFromDescrToPhyVht[DescrRateIndex];
    }
    else
    {
...
s8 wsm_rateid_to_rate_group(s8 rateid, RATE_MODE_TYPE * rate_mode, struct RATE_CONTROL_RATE_S * rates_info)
{
	s8				rate_index = 0;
	...
	{ 
       if(rates_info != NULL)
        {         
		   if(rates_info->vht_mode_on)
	        {
	        	rate_index			= DESCR_RATE_INDEX_VHT_MCS0 - rateid;
	    		*rate_mode			= RATE_VHT;
	        }
	        else
	        {
...
s8 rate_index_to_wsm_rate(s8 rate_index, RATE_MODE_TYPE rate_mode)
{
	...
	switch (rate_mode)
	{
		...
		case RATE_HT:
			...
			break;
        case RATE_VHT:
            rateid = rate_index - RATE_INDEX_VHT_MCS0;
            break;
		case RATE_HE:
...
s8 rate_index_to_wsm_rate_mode(s8 rate_index, RATE_MODE_TYPE * rate_mode)
{
	...
	*rate_mode			= RATE_UNKNOWN;
    ...
    if (rate_index >= RATE_INDEX_VHT_MCS0){
        *rate_mode          = RATE_VHT;
    }else if (rate_index >= RATE_INDEX_HE_MCS0)
	{
		*rate_mode			= RATE_HE;
	}
...
u16 get_ideal_rate(u8 wsm_rateid, u8 short_gi_supported, struct RATE_CONTROL_RATE_S * rates_info)
{
	RATE_MODE_TYPE	rate_mode;
	u16 			ideal_rate = 0;
	u8				rate_index;

	rate_index			= wsm_rateid_to_rate_group(wsm_rateid, &rate_mode, rates_info);
...
/************************************************************************/
void hmac_tx_statu(void * priv, struct atbm_ieee80211_sta * sta, void * priv_sta)
{
	...
	static u16			last_main_idle_ratio = 0;
    static int  bw_counter = 0;
	...
	atbm_printk_rc("\nhmac_tx_status chip:%d..\n", rates_info->chip_category);
	// dump_mem((unsigned char *)txstatus, 32);
	for (i = 0; i <= rate_end_idx; i++)
	{
		if (status->txcnt[i] > 0)
		{
		  rates_info->total_tx_counter = rates_info->total_tx_counter + status->txcnt[i];
		  rates_info->total_tx_fail_counter = rates_info->total_tx_fail_counter + status->txfail[i];
      	  if(rates_info->total_tx_counter >= 0xffff0000)
      	  {
		  	rates_info->total_tx_counter = 0;
		  	rates_info->total_tx_fail_counter = 0;
		  }
			...
			rates_info->tx_pkts_count_period = rates_info->tx_pkts_count_period + (status->txcnt[i] -status->txfail[i]);
		}
	}

    if ((sta->vht_cap->vht_supported) && (rates_info->max_vht_index))
     {
          rates_info->vht_mode_on = 1;
     }
     if ((sta->he_cap->has_he) && (rates_info->max_he_index))
     {
           rates_info->vht_mode_on = 0;
     }


	if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40_MHZ_WIDTH)
	{
		if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40M_TO_20M)
		{
			if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_SHORT_GI_20M)
			{
		...
		{
			if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_SHORT_GI_40M)
			{
				short_gi_supported	= 1;
			}
		}
	...
	for (i = rate_start_idx; i <= rate_end_idx; i++)
	{
		...
		{	            
			  ideal_rate_tp		= get_ideal_rate(i, short_gi_supported, rates_info);           
		...
		*/
		...
		if ((rates_info->sample_tp_rate_idx == descr_rate_index_to_phy(i, rates_info)) && (rates_info->sample_packets_cnt > 0))
		{
			rates_info->sucess_prob[i] = ((rates_info->sucess_prob[i] *2 + cur_prob * 2) >> 2);
		}
		...
		{
			//atbm_printk_err("loss ht resample\n");
		...
		if ((status->txfail[i] == status->txcnt[i])) /* have txed data. all faile*/
		{
			if (status->txcnt[i] > 10)
			{
				rates_info->sucess_prob[i] = ((rates_info->sucess_prob[i] * 3 + cur_prob * 5) >> 3);
			}
			else
			{
				rates_info->sucess_prob[i] = ((rates_info->sucess_prob[i] * 4 + cur_prob * 4) >> 3);
			}
		}
		...
		if ((rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40_MHZ_WIDTH) && (i <= DESCR_RATE_INDEX_HT_MCS0))
		{
		...
		if ((rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40_MHZ_WIDTH) && (i >= DESCR_RATE_INDEX_G_54M)) //mixed 40M and 20m bw rates
		{
			ideal_rate_tp		= (ideal_rate_tp) / 2;
		}
		...
		{
			if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_HE_SUPPORT_DCM)
			{
		...
		if ((current_tp >= max_tp) && ((status->txcnt[i] > 0) || (i > DESCR_RATE_INDEX_B_11M))) //not purely based on history data.
		{
			...
			else if (sta->vht_cap->vht_supported) //filter out 11g 
			{
				if (((i <= DESCR_RATE_INDEX_VHT_MCS0) && (i >= DESCR_RATE_INDEX_VHT_MCS9)) ||
	...
	if (max_tp != 0)
	{
		rates_info->max_tp_rate_idx = descr_rate_index_to_phy(max_tp_idx, rates_info);
	}


	if (max_tp_history != 0) //all rates failed
	{
		rates_info->max_tp_rate_idx_history = descr_rate_index_to_phy(max_tp_idx_history, rates_info);
	}


	rates_info->max_success_rate_idx = descr_rate_index_to_phy(max_success_idx, rates_info);
	...
	{
		wsm_rateid_to_rate_group(max_tp_idx, &rate_mode, rates_info);

		// atbm_printk_rc("max_tp_idx:%d rate_mdoe:%d \n",	max_tp_idx,rate_mode);
		if((sta->he_cap->has_he)&&(rates_info->max_he_index)) /*if HE mode support, rate not only HE mode start with HE mode*/
		{
			if ((rate_mode != RATE_HE) && (rate_mode != RATE_B))
			{
			   // atbm_printk_err("##### has he resample\n");
        ...
        else if((sta->vht_cap->vht_supported)&&(rates_info->max_vht_index)) /*if HE mode support, rate not only HE mode start with HE mode*/
		{
			if ((rate_mode != RATE_VHT) && (rate_mode != RATE_B))
			{
			   // atbm_printk_err("##### has he resample\n");
				rates_info->resample_required = 1;
			}
      ...
      /*
     Cronus: snd_idle_r |main_snd_r + main_idle_r
     Lite:   main_snd_idle_r |main_idle_r + rx_update_cnt     
     */

	main_idle_ratio = (u16) (sta->txs_retrys[RATE_DESC_INDEX_MAX * 2]);         //L16
	snd_idle_ratio		= (u16) (sta->txs_retrys[RATE_DESC_INDEX_MAX * 2 + 1]); //H16
	reservedL16 	= (u16) (sta->txs_retrys[RATE_DESC_INDEX_MAX * 2 + 2]);     //L16

    rates_info->phy_cfg = (snd_idle_ratio>>15)&0x01;
	rates_info->snd_idle_ratio = snd_idle_ratio&0x7fff;   
    

	atbm_printk_rc(" chip_category:%d phy_cfg:%d main_idle_ratio:0x%x, snd_idle_ratio:0x%x, reservedL16:0x%x\n",
					rates_info->chip_category, rates_info->phy_cfg, main_idle_ratio,snd_idle_ratio, reservedL16);
	
    if(rates_info->chip_category == ATBM_CRONUS_CATEGORY)
    {  //cronus chip    
        rates_info->main_idle_ratio = reservedL16;
        rates_info->main_snd_idle_ratio = main_idle_ratio;
    }else{
         //lite
        rates_info->main_idle_ratio = main_idle_ratio;
        rates_info->main_snd_idle_ratio = rates_info->snd_idle_ratio; //snd and main idle
        rates_info->rx_idle_ratio_update_cnt = reservedL16;
    }
    //atbm_printk_rc("Rx mainidl:%d sndidl:%d tx_rc_flags:0x%x\n", rates_info->main_idle_ratio, rates_info->main_snd_idle_ratio,rates_info->tx_rc_flags);
	...
	if (rates_info->max_tp_rate_idx >= RATE_INDEX_N_6_5M) //HT or HE mode
	{
	 ...
	 if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40_MHZ_WIDTH)
		...
		if ((rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40_MHZ_WIDTH) &&
			 (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_20_MHZ_WIDTH)) //support 40M&20M
		{
		 ...
		 //atbm_printk_err("current tick:%d  updatetick:%d\n",atbm_GetOsTick(), rates_info->bw_total_pkt_update_time);

			if (time_after(atbm_GetOsTick(), rates_info->bw_total_pkt_update_time + (2000 * HZ) / 1000))
			{

                if(rates_info->chip_category == ATBM_CRONUS_CATEGORY)
                {
            
			    // atbm_printk_err("rates_info->snd_idle_ratio:%d\n",rates_info->snd_idle_ratio);
				if (rates_info->snd_idle_ratio > 0)
				{ //have valid idle ratiorates_info->snd_idle_ratio

					rates_info->bw_total_pkt_cnt =
						 (rates_info->bw_total_pkt_cnt * HZ) / (atbm_GetOsTick() - rates_info->bw_total_pkt_update_time);
					bw40_band_select	= bw_select_40m(rates_info);
				...
				else  //no idle info
				{
				   rates_info->bw_total_pkt_cnt =
						 (rates_info->bw_total_pkt_cnt * HZ) / (atbm_GetOsTick() - rates_info->bw_total_pkt_update_time);
					bw40_band_select	= bw_select_40m_simple(rates_info);
				}
               }
               else
               {        
                    bw40_band_select = bw_select_40m_lite(rates_info);
               }
               
				    //atbm_printk_err("bw40_band_select:%d rates_info->max_tp_rate_idx:%d\n",bw40_band_select, rates_info->max_tp_rate_idx);
					if (bw40_band_select == 0)
					{
						rates_info->tx_rc_flags |= ATBM_IEEE80211_TX_RC_40M_TO_20M;
						atbm_printk_rc(" bw_select_11n sig set 40M_to_20M flag bw_total_pkt_cnt:%d\n",
							 rates_info->bw_total_pkt_cnt);
					}
					else 
					{
						if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40M_TO_20M) //switch form 20M to 40M, max tp rate -1
						{
							u8				wsm_rateid;
						...
						//atbm_printk_err(" bw_select_11n 40M  rates_info->max_tp_rate_idx:%d\n", rates_info->max_tp_rate_idx);
						rates_info->tx_rc_flags &= ~ATBM_IEEE80211_TX_RC_40M_TO_20M;
					}

					rates_info->bw_total_pkt_update_time = atbm_GetOsTick();
...
//	strRateIndexPhy[rates_info->max_tp_rate_idx],sum_tp, sum_tx_cnt, main_idle_ratio, rates_info->average_throughput);
	if (time_after(atbm_GetOsTick(), rates_info->tx_pkt_count_update_time + (2000 * HZ) / 1000))
		{


		  rates_info->tx_real_eval_rates = (rates_info->tx_pkts_count_period*1500*8)/2000;
					
		  evalM =  rates_info->tx_real_eval_rates/1000;
		  if(evalM >30)
		  {
			evalM = 30;
		  }
		   ...
		   if(sum_tx_cnt > 0)
		   {
			 ...
			 {
			   if(rates_info->average_throughput == 0)
			  	{
				  rates_info->average_throughput = ((sum_tp/sum_tx_cnt)*30)/4;
			  	}
				else
			  	{
			  		rates_info->average_throughput = ((sum_tp/sum_tx_cnt)*(80 + last_main_idle_ratio/10 + evalM*2))/10;  //prob(*256)*ideal(*2)*(ratio)
			  	}
			 }
			 else
			 {
				   last_main_idle_ratio = main_idle_ratio;
				  rates_info->average_throughput = ((sum_tp/sum_tx_cnt)*(80 + main_idle_ratio/10 + evalM*2))/10;  //prob(*256)*ideal(*2)*(ratio)
			 } 
		   }
		   else
		   {
		    ...
		    if(bw_x > 1)
		    {
				rates_info->average_throughput = (rates_info->average_throughput*bw_x*3)/4;
		    }

			if(rates_info->tx_real_eval_rates > 100) //tx works
			{				
				rates_info->average_throughput = (rates_info->average_throughput*4 + rates_info->tx_real_eval_rates)/5;
			}else
			{
				rates_info->average_throughput = (rates_info->average_throughput*9 + rates_info->tx_real_eval_rates)/10;
			}
		   
            if(rates_info->max_tp_rate_idx != 0xff)
			atbm_printk_rc(" mtp:%s  eval_rates:%d average_throughput:%d sum_tp:%d sum_tx_cnt:%d bw_x:%d main_idle_ratio:%d\n\n", strRateIndexPhy[rates_info->max_tp_rate_idx],  rates_info->tx_real_eval_rates ,
				rates_info->average_throughput, sum_tp, sum_tx_cnt,bw_x, main_idle_ratio);		 

            if(rates_info->max_tp_rate_idx != 0xff)
			atbm_printk_rc(" mtp:%s  rates_info->total_tx_counter:%d rates_info->total_tx_fail_counter:%d \n", 
			           strRateIndexPhy[rates_info->max_tp_rate_idx],rates_info->total_tx_counter, rates_info->total_tx_fail_counter);		
			rates_info->tx_pkt_count_update_time = atbm_GetOsTick();			 
			
			rates_info->tx_pkts_count_period = 0;
			sum_tp = 0;
		    sum_tx_cnt = 0;
		}
	...
	//atbm_printk_err("average_throughput %d\n",  rates_info->average_throughput);
	if(rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40M_TO_20M){
    	//atbm_printk_err("tp_idx:%d tx_rc_flags:0x%x bw40sel:%d 20Mcnt:%d  jiffies:%ld uptime:%ld \n", rates_info->max_tp_rate_idx,rates_info->tx_rc_flags, bw40_band_select,bw_counter, jiffies,  rates_info->tx_pkt_count_update_time);
	}
	else
	{
		//atbm_printk_err("40M\n",rates_info->max_tp_rate_idx);
	 ...
	 u8 * min_phy_rate)
{
	...
	switch (rate_mode)
	{
        ...
        case RATE_VHT:
			if (rates_info != NULL)
			{
				if (rates_info->max_vht_index > 8)
				{
					*max_phy_rate		= RATE_INDEX_VHT_MCS9;
				}
				else 
				{
					*max_phy_rate		= RATE_INDEX_VHT_MCS0 + rates_info->max_vht_index;
				}
				*min_phy_rate		= RATE_INDEX_VHT_MCS0;
			}
			else 
			{
				*max_phy_rate		= RATE_INDEX_VHT_MCS7;
				*min_phy_rate		= RATE_INDEX_VHT_MCS0;
			}
...
26M->6.5M
*/
static void get_rate_start_11n( void * priv_sta, 
	struct atbm_ieee80211_tx_rate_control * txrc)
{
	...
	if (rates_info->max_tp_rate_idx != 0xff)
	{
		...
		if ((rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40_MHZ_WIDTH) &&
			 (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_20_MHZ_WIDTH))
		{
			rates_info->tx_rc_flags |= ATBM_IEEE80211_TX_RC_40M_TO_20M;
			txrc->info.control->tx_rc_flag |= ATBM_IEEE80211_TX_RC_40M_TO_20M;
		}
...
static void get_rate_start_11n_vht( void * priv_sta, 
	struct atbm_ieee80211_tx_rate_control * txrc)
{
	// static int pkt_num = 0;
	RATE_SET_T *	prate_set;
	...
	prate_set->rate_mode = RATE_VHT;
	iter_idx			= rates_info->packets_cnt % 3; /*0, 1,2*/
	prate_set->max_tp_rate_index = RATE_INDEX_VHT_MCS7 - 2 * iter_idx;
	prate_set->max_tp_rate_try = 2;


	if (prate_set->max_tp_rate_index > rates_info->max_vht_index + RATE_INDEX_VHT_MCS0)
	{
		prate_set->max_tp_rate_index = rates_info->max_vht_index + RATE_INDEX_VHT_MCS0;
	}
	...
	if (rates_info->max_tp_rate_idx != 0xff)
	{
		if (rates_info->max_tp_rate_idx <= RATE_INDEX_B_11M) //from 11b
		{
			prate_set->max_tp_rate_index = RATE_INDEX_VHT_MCS1;
		}
		else 
		{
			prate_set->max_tp_rate_index = rates_info->max_tp_rate_idx; //not down		   
	...
	prate_set->lowest_rate_index = RATE_INDEX_VHT_MCS0;
	prate_set->lowest_rate_try = 2;
...
MSC4->MSC0 
*/
void get_rate_start_11ax( void * priv_sta, 
	struct atbm_ieee80211_tx_rate_control * txrc)
{
	// static int pkt_num = 0;
	RATE_SET_T *	prate_set;
...
18M->6
*/
void get_rate_start_11g(void * priv_sta, 	struct atbm_ieee80211_tx_rate_control * txrc)
{
	// static int pkt_num = 0;
	RATE_SET_T *	prate_set;
	u8				iter_idx = 0;
...
void get_rate_set_11b(void * priv_sta, 	struct atbm_ieee80211_tx_rate_control * txrc)
{
...
void get_max_assoc_rate(struct atbm_ieee80211_sta * sta, struct RATE_CONTROL_RATE_S * rates_info)
{
	u8				max_rcs_index = 0;
        u8 u8band = 0;
	u32 			rate_mask = 0;
 
	if ((sta->he_cap->has_he) && atbm_ieee80211_he_mcs_rate_support(sta->he_cap->he_mcs_nss_supp.rx_mcs_80, 1))
	{
		max_rcs_index		= atbm_ieee80211_he_mcs_operation_rate(sta->he_cap->he_mcs_nss_supp.rx_mcs_80, 1);
    ...
    if (sta->vht_cap->vht_supported)
     {
          u8 peer_rx = le16_to_cpu(sta->vht_cap->vht_mcs.rx_mcs_map) &ATBM_IEEE80211_VHT_MCS_NOT_SUPPORTED;             
            switch(peer_rx){
            case ATBM_IEEE80211_VHT_MCS_NOT_SUPPORTED:
                rates_info->max_vht_index  = 0;
                break;
            case ATBM_IEEE80211_VHT_MCS_SUPPORT_0_7:
                rates_info->max_vht_index  = 7;
                break;
            case ATBM_IEEE80211_VHT_MCS_SUPPORT_0_8:
                 rates_info->max_vht_index  = 8;
                break;
            case ATBM_IEEE80211_VHT_MCS_SUPPORT_0_9:
                 rates_info->max_vht_index  = 9;
                break;
            }
			atbm_printk_rc("max_rcs_indexvHt:%d \n", rates_info->max_vht_index);
     }
    
	if ((sta->ht_cap->ht_supported) && (sta->ht_cap->mcs.rx_mask[0]))
	{
		/*
		...
		*/
		...
		else 
		{
			...
			rates_info->max_ht_index = atbm_fls(rate_mask) - 1;
#if TEST_ON_FIXED_RATE	
	if(hweight32(rate_mask) < 3){
               	atbm_printk_init("Single Rate mode Ht rate_mask:0x%x \n", rate_mask);
                rates_info->n_mode = 1;
		rates_info->tx_rc_flags |= IEEE80211_TX_CTL_USE_MINRATE;
	}
#endif
		}		
	...
	atbm_printk_rc(" supp_rates band0:0x%x band1 0x%x \n",  sta->supp_rates[0],   sta->supp_rates[1]);




       if(rates_info->center_freq >2600)
	{
          u8band = 1;
	}
 	if (sta->supp_rates[u8band]) {	 
        u8 min_g_index;

	    rates_info->g_mode      = 0;
        rate_mask               = sta->supp_rates[u8band];        
        
		rates_info->max_g_index = atbm_fls(rate_mask) - 1;
...
#if TEST_ON_FIXED_RATE	
        if(hweight32(rate_mask) < 3){
             atbm_printk_init("Single Rate mode legacy rate_mask:0x%x \n", rate_mask);
             rates_info->tx_rc_flags |= IEEE80211_TX_CTL_USE_MINRATE;
        }
#endif
		if (rates_info->max_g_index >= min_g_index) //has 11g rates
		{

            rates_info->max_g_index = rates_info->max_g_index - min_g_index;

            if(u8band == 0){
                rates_info->max_b_index = atbm_fls(rate_mask & 0x0f);
            ...
            atbm_printk_err("%s:5G band no support A rate FIX 6M!!!\n",__func__);
			rates_info->g_mode = 1;
            rates_info->max_g_index = 0;
        }

		atbm_printk_rc("maxrateidx 11n:%d vht:%d 11g:%d 11b:%d rate_mask:0x%x\n", rates_info->max_ht_index,rates_info->max_vht_index,
...
int upgrade_sample_from_11b(  struct RATE_CONTROL_RATE_S * rates_info,  bool have_he, bool have_n_mode)
{	
	u8 upgrade_rateid;
	u8 B_5M_rateid;  
	int should_upgrade = 0;   

	if(have_he)
	{
		upgrade_rateid  = phy_rate_index_to_descr(RATE_INDEX_HE_MCS1);
	}
	else if(have_n_mode)
	{
		upgrade_rateid  = phy_rate_index_to_descr(RATE_INDEX_N_13M);
	}
	else
	{
		return 1;
	}
	B_5M_rateid =  phy_rate_index_to_descr(RATE_INDEX_B_5_5M);
	if((rates_info->transmit_cnt[upgrade_rateid] > 6)&&(rates_info->transmit_cnt[B_5M_rateid] > 6))
	{
		if(rates_info->sucess_prob[upgrade_rateid]  >= rates_info->sucess_prob[B_5M_rateid]/2 )
		{
			should_upgrade = 1;
		}
		else
	...
	else /*not compare, do nothing*/
	{    
		should_upgrade = 1;
	}
	atbm_printk_rc("Mcstx:%d B5mtx:%d Mprob:%d  B5Mprob:%d  should_upgrade:%d \n",
		rates_info->transmit_cnt[upgrade_rateid], rates_info->transmit_cnt[B_5M_rateid],
		rates_info->sucess_prob[upgrade_rateid], rates_info->sucess_prob[B_5M_rateid], should_upgrade);

	if(should_upgrade == 0)
	{
		 rates_info->select_up_cnt++;
	}

	if((should_upgrade == 0)&&( rates_info->select_up_cnt%3 == 0)) /*normal upgrade sample*/
	{
		should_upgrade = 1;
		atbm_printk_rc("Mcs_rateid:%d select_cnt:%d should_upgrade:%d\n", upgrade_rateid, rates_info->select_up_cnt, should_upgrade);
	}    
	if(rates_info->select_up_cnt > 60000)
	{
		rates_info->select_up_cnt = 0;
	}        
	return should_upgrade;
}


int  he_down_to_11b(struct RATE_CONTROL_RATE_S * rates_info)
{	
	u8 er0_rateid;
	u8 B_5M_rateid;	
	int force_11b = 0;   

	er0_rateid  = phy_rate_index_to_descr(RATE_INDEX_HE_MCSER0);
	B_5M_rateid =  phy_rate_index_to_descr(RATE_INDEX_B_5_5M);

	if((rates_info->transmit_cnt[er0_rateid] > 6)&&(rates_info->transmit_cnt[B_5M_rateid] > 6))
	{
		if(rates_info->sucess_prob[er0_rateid]  <= rates_info->sucess_prob[B_5M_rateid] )
		{
			force_11b = 1;
		}
		else
	...
	//atbm_printk_rc("er0tx:%d 5mtx:%d  er0prob:%d 5mprob:%d  force_11b:%d \n",
		//rates_info->transmit_cnt[er0_rateid], rates_info->transmit_cnt[B_5M_rateid],
	//	rates_info->sucess_prob[er0_rateid], rates_info->sucess_prob[B_5M_rateid], force_11b);

	if(force_11b == 1)/*changed to 11B*/
	{
		rates_info->select_b_cnt++;
	}	
	
	if((rates_info->select_b_cnt%5 == 0)) /*er mode*/
	{
		force_11b = 0;
		//atbm_printk_rc("b_mode_set:%d select_b_cnt:%d force_11b:%d\n", rates_info->select_b_cnt, force_11b);
	}    

	if(rates_info->select_b_cnt > 60000)
	{
		rates_info->select_b_cnt = 0;
	}
...
s8 lite_idle_ratio_select(u16 main_snd_idle_ratio, u16 main_idle_ratio,  s8 bw_40m_cn_in)
{
	s8   bw_40m_cn = 0;

    if(main_idle_ratio <= 20)  //special case
    {
		bw_40m_cn = 0;
        return bw_40m_cn;
    }
    /*
    1)  如果 snd_idle_ratio<= main_idle_ratio*0.25  bw_40m_cnt =-8.
    2)  否则如果 snd_idle_ratio<= main_idle_ratio*0.35 bw_40m_cnt -=4 
    3)  否则如果 snd_idle_ratio<= main_idle_ratio*0.45 bw_40m_cnt -=3 
    4)  否则如果 snd_idle_ratio >=  main_idle_ratio*0.75,bw_40m_cnt =7.
    5)  否则如果 snd_idle_ratio>= main_idle_ratio*0.65, bw_40m_cnt +=4
    6)  否则如果 snd_idle_ratio>= main_idle_ratio*0.55,  bw_40m_cnt += 3
    */
    main_snd_idle_ratio = main_snd_idle_ratio *20;  //less than 65536
    
	if(main_snd_idle_ratio <= (main_idle_ratio * 5) )
	{
		bw_40m_cn = -8;
	}
	else if( main_snd_idle_ratio <= (main_idle_ratio * 7))
	{
		bw_40m_cn = bw_40m_cn - 4;
    }
	else if(main_snd_idle_ratio <= (main_idle_ratio * 9))
	{    
		bw_40m_cn = bw_40m_cn - 3;
	}
	else if(main_snd_idle_ratio <= (main_idle_ratio * 11))
	{
       bw_40m_cn = bw_40m_cn;    
	}
	else if(main_snd_idle_ratio <= (main_idle_ratio * 13))
	{
        bw_40m_cn = bw_40m_cn + 3;    
	}	
    else if(main_snd_idle_ratio <= (main_idle_ratio * 15))
	{
       	bw_40m_cn = bw_40m_cn + 4;
	}
    else
    ...
    atbm_printk_err("main_snd_idle_ratio:%d main_idle_ratio:%d bw_40m_cn in:%d out:%d\n",main_snd_idle_ratio/20, main_idle_ratio, bw_40m_cn_in,bw_40m_cn);
	return bw_40m_cn;
}

unsigned char bw_select_40m_lite(struct RATE_CONTROL_RATE_S * rates_info)
{   
    u16  main_snd_idle_ratio   =  rates_info->main_snd_idle_ratio;
    u16  main_idle_ratio = rates_info->main_idle_ratio;
	s8   rssi             =  (signed char) rates_info->mean_rssi;
    u8   m_tp_rate        =   rates_info->max_tp_rate_idx ;
    u8   mtp_prob;
    u8  max_tp_idx_desc;

	u8  good_sig = 0;
	u8  good_sig_thresh = 100;
    u8 	mtp_prob_thresh = 70;     
    u8  bw_40m_select = 1;  
    s8  bw40_rssi_lowest_thresh = -85;
	u16 pkt_cnt_thresh = 800;
    u16 bw_pkt_cnt      = rates_info->bw_total_pkt_cnt;
   
    

    //u8 bw40m_to_20m_idle_thresh = SND_IDLE_SET_20M_THRESH;
   // u8 mtp_prob_thresh = 60;

    //u8 rate_index_n_thresh = RATE_INDEX_N_26M;
    //u8 rate_index_he_thresh = RATE_INDEX_HE_MCS3;
    //u8 rate_index_vht_thresh = RATE_INDEX_VHT_MCS2;      
    u8 cur_band40_2_20m =  ! ! (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40M_TO_20M);
    
     max_tp_idx_desc = phy_rate_index_to_descr(m_tp_rate);
     mtp_prob = rates_info->sucess_prob[max_tp_idx_desc];
   // atbm_printk_rc("rssi:%d  snd_ratio:%d  m_sndr:%d m_tp:%d mtp_prob:%d bw20:%d\n",rssi, snd_idle_ratio,((main_idle_ratio>>8)&0xff), m_tp_rate, mtp_prob, cur_band40_2_20m);
   
	if(cur_band40_2_20m)
	{
	  good_sig_thresh = good_sig_thresh + 25;
	}   
    ...
    /* step 1 set rssi thresh */
    if((rates_info->phy_cfg == 1)&&(mtp_prob > good_sig_thresh)&&(bw_pkt_cnt > pkt_cnt_thresh))
    {
        bw40_rssi_lowest_thresh = -90;
		good_sig = 1;
    }
    else
    {
         bw40_rssi_lowest_thresh = -82;
    }
    
    if (cur_band40_2_20m)
    {
       bw40_rssi_lowest_thresh = bw40_rssi_lowest_thresh + 3;
       mtp_prob_thresh = mtp_prob_thresh + 20;
       pkt_cnt_thresh = (pkt_cnt_thresh*5)/4;
    }

    /* step2  base on rssi and bw, rate */
    if ((rssi <= bw40_rssi_lowest_thresh)&&(mtp_prob < mtp_prob_thresh))   //weak signal. froce 20M
    {       
        rates_info->bw_40m_cnt = -8;
    }
    else //normal signal
    {
        if(good_sig) //best condition force 40M
        {
            rates_info->bw_40m_cnt = 8;
        }     
        else //> bw40_rssi_lowest_thresh 
        {
            if ((m_tp_rate == RATE_INDEX_N_6_5M) ||(m_tp_rate == RATE_INDEX_VHT_MCS0)||
                (m_tp_rate == RATE_INDEX_HE_MCS0))
            {
               rates_info->bw_40m_cnt = -8;  //low rate
            }
            else
            {
               if((rates_info->rx_idle_ratio_update_cnt == rates_info->last_rx_idle_ratio_update_cnt)&&(rates_info->bw_not_update_cnt == 0))  //bw cnt not changed
               {
                    rates_info->bw_not_update_cnt++;
               }
               else
               {            
                   //combine 4 & 5 ,, update bw_40m_cnt                   
                   rates_info->bw_40m_cnt = lite_idle_ratio_select(main_snd_idle_ratio, main_idle_ratio, rates_info->bw_40m_cnt );
                   rates_info->bw_not_update_cnt = 0;
               }           
           }           
    ...
    rates_info->last_rx_idle_ratio_update_cnt = rates_info->rx_idle_ratio_update_cnt;  //update last cnt

    if(rates_info->bw_40m_cnt >= 0)
     {
        bw_40m_select = 1;
     }
     else
	...
	if((good_sig == 0)&&(bw_40m_select == 1)) 
	{
    	if ((m_tp_rate <= RATE_INDEX_N_13M) ||((m_tp_rate < RATE_INDEX_VHT_MCS2) && (m_tp_rate >= RATE_INDEX_VHT_MCS0))
        ||((m_tp_rate < RATE_INDEX_HE_MCS2) && (m_tp_rate >= RATE_INDEX_HE_MCS0)))
		{
			bw_40m_select = 0;
		}
		else if((m_tp_rate <= RATE_INDEX_N_39M) ||((m_tp_rate < RATE_INDEX_VHT_MCS4) && (m_tp_rate >= RATE_INDEX_VHT_MCS0))
        ||((m_tp_rate < RATE_INDEX_HE_MCS5) && (m_tp_rate >= RATE_INDEX_HE_MCS0)))
		{
			if(mtp_prob < mtp_prob_thresh)  //middle rate, low success ratio
			{
				bw_40m_select = 0;
			}
		}
		else  //high rate, low success ratio and low idle ratio or weak signal
		{		
			if(mtp_prob < mtp_prob_thresh)
			{
				if((main_snd_idle_ratio*6 < main_idle_ratio*5)||(bw_pkt_cnt < pkt_cnt_thresh))  
				{					
			   		 bw_40m_select = 0;
				}
			}
    ...
    if(bw_40m_select == 0){
        atbm_printk_err("rssi:%d  phy_cfg:%d, m_snd_idler:%d  m_idle_r:%d bw_pkt_cnt:%d m_tp:%d mtp_prob:%d bw20:%d bw_40m_select:%d\n",  
            rssi, rates_info->phy_cfg, main_snd_idle_ratio,main_idle_ratio, bw_pkt_cnt, m_tp_rate, mtp_prob, cur_band40_2_20m, bw_40m_select);
    }
...
ch_idle_ratio:	channel idle ratio(full 40M )
main_snd_ratio: channel idle ratio(main)
...
unsigned char bw_select_40m(struct RATE_CONTROL_RATE_S * rates_info)
{
    ...
    s8 rssi             =  (signed char) rates_info->mean_rssi;
    u8 snd_idle_ratio   = (u8) rates_info->snd_idle_ratio;
    u16 main_idle_ratio = rates_info->main_idle_ratio;
    u8 m_tp_rate        =   rates_info->max_tp_rate_idx ;
    u8 mtp_prob; //         = rates_info->sucess_prob[max_tp_idx];
    u8 cur_band40_2_20m =  ! ! (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40M_TO_20M);
    u16 bw_pkt_cnt      = rates_info->bw_total_pkt_cnt;
    u8 max_tp_idx_desc;
	u8	bw_40m_select = 1;
	...
	if (cur_band40_2_20m)
	{
        ...
        rate_index_vht_thresh = RATE_INDEX_VHT_MCS5;
	...
	else 
	{
        ...
        rate_index_vht_thresh = RATE_INDEX_VHT_MCS4;
	}
#endif
  ...
  if((rates_info->chip_category == ATBM_OCEANUS_CATEGORY)||(rates_info->chip_category == ATBM_CLITE_CATEGORY))
  	{
  	   lite_flag = 1;  //oceanus and cronus lite 
	   main_idle_ratio = main_idle_ratio&0xff;
	   bw40m_to_20m_idle_thresh = FULL_DILE_SET_20M_THRESH;
  	}
	else
  	{
         //atbm_printk_err("rssi:%d  snd_ratio:%d  m_sndr:%d m_tp:%d mtp_prob:%d	bw20:%d\n",rssi, ch_idle_ratio,((main_snd_ratio>>8)&0xff), m_tp_rate, mtp_prob, cur_band40_2_20m);
	}
	...
	if ((snd_idle_ratio == 0) || (main_idle_ratio == 0))
	{
	atbm_printk_err("lite_flag:%d rssi:%d  snd_ratio:%d	m_sndr:%d m_tp:%d mtp_prob:%d bw20:%d bw_40m_select:%d\n",  
	                 lite_flag, rssi, snd_idle_ratio,main_idle_ratio, m_tp_rate, mtp_prob, cur_band40_2_20m, bw_40m_select);
		return bw_40m_select;
	}

	/*set bw select thresh*/
	if(lite_flag == 0){
		if (main_idle_ratio > 60)
		{		
			bw40m_to_20m_idle_thresh = SND_IDLE_SET_20M_THRESH + 4; //main > snd, likely to set 20M
		}
		else if (main_idle_ratio > 40)
		{
	...
	{
		if ((m_tp_rate < RATE_INDEX_N_13M) ||(m_tp_rate == RATE_INDEX_VHT_MCS0)||
            ((m_tp_rate < RATE_INDEX_HE_MCS1) && (m_tp_rate >= RATE_INDEX_HE_MCS0)))
		{
			bw_40m_select = 0;
		}
		else if ((m_tp_rate <= rate_index_n_thresh) ||
                 ((m_tp_rate <= rate_index_vht_thresh)&& (m_tp_rate >= RATE_INDEX_VHT_MCS0)) ||
                ((m_tp_rate <= rate_index_he_thresh) && (m_tp_rate >= RATE_INDEX_HE_MCS0)))
		{
				if (snd_idle_ratio < (bw40m_to_20m_idle_thresh + 5))
				{
					bw_40m_select	= 0;
				}
				else 
...
#if BANDWIDTH_MORE_20M
				if ((bw_pkt_cnt < pkt_cnt_thresh) && (snd_idle_ratio < snd_idle_thresh)) //more likely to select 20M
				{
					bw_40m_select	= 0;
				}
#endif
		...
		else //>=39M
		{
			 if(lite_flag)
	         {
              //   bw_40m_select = lite_idle_ratio_select(snd_idle_ratio, (u8)main_idle_ratio);
				 if(mtp_prob > 75)  /*high rate 40M success ratio > thresh, not use 20M*/
				 {
				   bw_40m_select = 1;
				 }else{
				 }
			 ...
			 {
				if ((snd_idle_ratio < bw40m_to_20m_idle_thresh) && (mtp_prob < mtp_prob_thresh))
				{
					bw_40m_select	= 0;
				}
				else 
...
unsigned char bw_select_40m_simple(struct RATE_CONTROL_RATE_S * rates_info)
{
   ...
   u8  max_tp_idx_desc;
    ...
    unsigned char   rate_index_vht_thresh = RATE_INDEX_VHT_MCS3;
	...
	if(bw_40m_select == 0){
		atbm_printk_rc("rssi:%d  m_tp:%d mtp_prob:%d bw_pkt_cnt:%d	bw20:%d Select40:%d\n", rssi,  m_tp_rate, mtp_prob, bw_pkt_cnt, cur_band40_2_20m, bw_40m_select);
	}
...
void set_lowest_rate(u8 rateidx, struct atbm_ieee80211_tx_rate_control * txrc)
{
	struct atbm_ieee80211_tx_info_rc * info = &txrc->info;
	RATE_SET_T		rate_set;
	u8				wsm_rate_id;
	u8				max_rate_idx = 0;
...
static void hmac_get_rate(void * priv, void * priv_sta, 
	struct atbm_ieee80211_tx_rate_control * txrc)
{
	u8				rateid;
    ...
    //atbm_printk_err("%s:hmac_get_rate!\n", __func__);

#if TEST_ON_FIXED_RATE
	if ((sta != NULL) && (rates_info != NULL))
	{
		if(rates_info->tx_rc_flags & IEEE80211_TX_CTL_USE_MINRATE)
		{	
			if(txrc->tx_rc_in->lower == 0){
			  if(rates_info->max_he_index > 0)
			  {
				  txrc->tx_rc_in->manual_rate	= rates_info->max_he_index + RATE_INDEX_HE_MCS0;
			  }
			  else if(rates_info->n_mode == 1)
			  {			    
				 txrc->tx_rc_in->manual_rate	= rates_info->max_ht_index + RATE_INDEX_N_6_5M;			  			  
			  }
			  else if( rates_info->g_mode == 1)
			  {			     
				 txrc->tx_rc_in->manual_rate  = rates_info->max_g_index + RATE_INDEX_A_6M;
			  }
			  else if(rates_info->max_b_index > 0)
			  {
				 txrc->tx_rc_in->manual_rate  = rates_info->max_b_index;
			  }
			  else
			  { 			 
			  
			  } 	
	          atbm_printk_rc("txrc single rate mode manual rate:%d \n", txrc->tx_rc_in->manual_rate);	
			  txrc->tx_rc_in->manual = TX_RATE_FIXED;
	 		}
		 }
	 }
#endif
	/*Set manual set rate.*/
	if (txrc->tx_rc_in->manual)
	{
		u8				wsm_rateid;
		u32 			wsm_rateset;

		get_fixed_rate_set(txrc->tx_rc_in->manual_rate, &wsm_rateid, &wsm_rateset);

		if ((txrc->tx_rc_in->manual & TX_RATE_FIXED_AUTO_DOWN) == 0)
		{
		...
		if (txrc->tx_rc_in->manual & TX_RATE_FIXED_STATIC)
		{
			if ((sta != NULL) && (rates_info != NULL))
			{
				if (time_after(atbm_GetOsTick(), rates_info->last_update_time + (rate_update_interval * HZ) / 1000))
				{
					info->tx_update_rate = 1;
					rates_info->last_update_time =  atbm_GetOsTick();
				}
			}
		}

		if((txrc->tx_rc_in->manual_rate >= RATE_INDEX_HE_MCSER0)&&(txrc->tx_rc_in->manual_rate <= RATE_INDEX_HE_MCS11))
		{
			if ((rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_HE_LDPC) || (txrc->tx_rc_in->manual_rate == RATE_INDEX_HE_MCS10) ||
				 (txrc->tx_rc_in->manual_rate == RATE_INDEX_HE_MCS11))
			{
				info->control->tx_rc_flag |= ATBM_IEEE80211_TX_RC_HE_LDPC;
			}
		}
        /*manual rate mode, default set LDPC flag if support. */
		if((txrc->tx_rc_in->manual_rate >= RATE_INDEX_N_6_5M)&&(txrc->tx_rc_in->manual_rate <= RATE_INDEX_N_65M))
		{
			if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_HT_LDPC)
			{
				info->control->tx_rc_flag |= ATBM_IEEE80211_TX_RC_HT_LDPC;
			}	
		}
		if((txrc->tx_rc_in->manual_rate >= RATE_INDEX_VHT_MCS0)&&(txrc->tx_rc_in->manual_rate <= RATE_INDEX_VHT_MCS9))
		{
			if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_VHT_LDPC)
			{
				info->control->tx_rc_flag |= ATBM_IEEE80211_TX_RC_HT_LDPC;  //both HT and VHT use same LDPC flag.
			}	
		}
		return;
	}
	...
	if (txrc->tx_rc_in->lower)
	{
		///	struct ieee80211_tx_info *info = IEEE80211_SKB_CB(txrc->skb);
	    atbm_printk_rc( "rc get_rate low rate!\n");
		if ((sta != NULL) && (rates_info != NULL))
		{
			if ((rates_info->mean_rssi > -50) && (txrc->tx_rc_in->max_rate_idx == RATE_INDEX_B_1M) &&
		...
		set_lowest_rate(txrc->tx_rc_in->max_rate_idx, txrc);
	...
	he_cap				= &sta->he_cap;
	//atbm_printk_rc("%s:hmac_get_rate! rates_info:0x%x he_cap addr:0x%x has_he:%d\n", __func__,rates_info, he_cap, he_cap->has_he);
	...
	if ((maxrateidx == 0xff) || (rates_info->resample_required))
	{
		...
		if ((sta->he_cap.has_he) && (rates_info->max_he_index)) /*if HE mode support, start with HE mode*/
		{
			...
			get_rate_start_11ax(priv_sta, txrc);
		}
        else if (sta->vht_cap.vht_supported) /*if HE mode not support,  start with HT mode*/
		{
			atbm_printk_rc("sample	11n vht \n");
			get_rate_start_11n_vht(priv_sta, txrc);
		}
        
		else if (sta->ht_cap.ht_supported) /*if HE mode not support,  start with HT mode*/
		{
			atbm_printk_rc("sample	11n \n");
			get_rate_start_11n(priv_sta, txrc);
		}
		else if (rates_info->g_mode) /*if HE&HT mode not support,  start with 11g mode*/
		{
			atbm_printk_rc("sample 11g \n");
			get_rate_start_11g( priv_sta, txrc);
		}
		else //11B only
		{
			atbm_printk_rc("sample 11b tp:%d pcnt:%d\n", maxrateidx, rates_info->packets_cnt);

			get_rate_set_11b( priv_sta, txrc);
			rates_info->packets_cnt++;
		}
	...
	else /*based on statics and updated rate info*/
	{
		...
		*/
		...
		wsm_rateid_to_rate_group(rateid, &rate_mode, rates_info);
			...
			sample*/
		{
			if (rates_info->sample_packets_cnt < sample_number) // sample ampdu number packets.
			{
				...
				{
                    /*11B upgrade required, retrurn 0 or 1*/
				  	if(rate_mode == RATE_B)
				  	{
                 	     should_upgrade = upgrade_sample_from_11b(rates_info,  sta->he_cap.has_he, sta->ht_cap.ht_supported);
				  	}else
				  	{
				  		 should_upgrade = 1;
				  	}
					/*If support HE or HT mode, should_upgrade will be used to control sample from DSSS to MCS rate.
					  If HE&HT not support, sample will not be changed, should_upgrade set 1. */                
					if ((sta->he_cap.has_he) && (rates_info->max_he_index)&&(should_upgrade)) /*if HE mode support, start with HE mode*/
					{
						...
						if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_HE_SUPPORT_DCM) //support DCM, ER0 aways send with DCM
						{
							prate_set->max_tp_rate_index = min_phy_rate;
						}
                        info->control->force_policyid |= HE_MODE_DOWN_TO_11B;  /*sample HE rate default down to 11B mode*/
                        
					}
					else if (sta->vht_cap.vht_supported) /*if HE mode not support,  start with HT mode*/
					{
						get_rate_max_min(RATE_VHT, rates_info, &max_phy_rate, &min_phy_rate); // update params
						prate_set->rate_mode = RATE_VHT;
						...
						//	else
						//	{
						prate_set->max_tp_rate_index = RATE_INDEX_VHT_MCS1; //BCC mode from 13M

						//	}							
					}
					else if ((sta->ht_cap.ht_supported)&&(should_upgrade)) /*if HE mode not support,  start with HT mode*/
					{
					...
					else if ((rates_info->g_mode)&&(should_upgrade)) /*if HE&HT mode not support, start with 11g mode*/
					{
				...
				else //upgrade one step mode.
				{
					...
					descrateid			= phy_rate_index_to_descr(rates_info->max_tp_rate_idx); //new rate
					if (rates_info->sucess_prob[descrateid] < (10 * 256) / 100) //too bad
					{
						if (sample_number > 16)
						{
							sample_number = 16;
			 ...
			 (rates_info->max_he_index))) /*HE msc0 down to ER +DCM mode sample*/
		{
			if ((rates_info->sucess_prob[rateid] < (SAMPLE_DCM_PROB_THRESH * 256) / 100) && (rate_mode == RATE_HE) &&
				 (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_HE_ER))
			{
				if (rates_info->sample_packets_cnt < sample_number)
				{
						 ...
						 (rates_info->max_tp_rate_idx <= min_phy_rate + rates_info->max_dcm_rate_idx))
					{
						...
						info->control->tx_rc_flag |= ATBM_IEEE80211_TX_RC_HE_DCM;
					}
			...
			else 
			{
			 ...
			 ((rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_HE_ER) == 0))))
		...
		if (prate_set->max_tp_rate_index >= RATE_INDEX_N_6_5M) //HT or HE mode
		{
				 ...
				 (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_20_MHZ_WIDTH))
			{
				if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40M_TO_20M)
				{
					info->control->tx_rc_flag |= ATBM_IEEE80211_TX_RC_40M_TO_20M; //force 20M bandwidth
					if(prate_set->max_tp_rate_index == RATE_INDEX_VHT_MCS9)
					{
					   prate_set->max_tp_rate_index = RATE_INDEX_VHT_MCS8;  //force 20M, can not send mcs9
					}
       ...
       /*not change bandwidth from 40M to 20M*/      
       if(((wifi_special_freq <= 2402)&&(wifi_special_freq > 2300)) 
        || ((wifi_special_freq >= 2494)&&(wifi_special_freq < 2600)))  
       {
          //  atbm_printk_rc( wifi_special_freq:%d tes_info->center_freq:%d\n",wifi_special_freq, rates_info->center_freq);
             info->control->tx_rc_flag &= ~ ATBM_IEEE80211_TX_RC_40M_TO_20M;               
       }        
	...
	/*HE GI mode change short gi */
	//	if((sta->he_cap.has_he)&&(rates_info->sucess_prob[rateid] > (SAMPLE_RATE_IDEAL_SUCC_PROB_THRESH * 256) / 100)) 
	if((prate_set->max_tp_rate_index >= RATE_INDEX_HE_MCSER0)&&(prate_set->max_tp_rate_index <= RATE_INDEX_HE_MCS11))//lmac short gi from rate index, if hmac change, need to change lmac
	{
	   if(rates_info->max_tp_rate_idx != 0xff)
	   	{
			if(rate_for_gi < 0xff)
			{
				if (prate_set->max_tp_rate_index >= rate_for_gi)
				{
					info->control->tx_rc_flag |= ATBM_IEEE80211_TX_RC_SHORT_GI;
		...
		if ((rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_HE_LDPC) ||
		...
		if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_HE_SUPPORT_DCM)
		{
			info->control->tx_rc_flag |= ATBM_IEEE80211_TX_RC_HE_SUPPORT_DCM;
		}


        /*select down to 11B or ER0 mode*/
        if(he_down_to_11b(rates_info))
         {
             info->control->force_policyid |= HE_MODE_DOWN_TO_11B;  
           //  atbm_printk_rc("Force down to 11B mode:%x\n",info->control.force_policyid);
         }
         else  /*set as default setting.*/
         {
	...
	if ((rate_mode == RATE_HT)||(rate_mode == RATE_VHT))
	{
		// USE short GI
		rateid				= phy_rate_index_to_descr(rates_info->max_tp_rate_idx);
      if(rates_info->max_tp_rate_idx != 0xff)
      	{
		if ((((prate_set->max_tp_rate_index >= RATE_INDEX_N_65M)&&(rate_mode == RATE_HT) )||
				((prate_set->max_tp_rate_index >= RATE_INDEX_VHT_MCS8)&&(rate_mode == RATE_VHT)))
				&&(rates_info->sucess_prob[rateid] > (SAMPLE_RATE_UP_SUCC_PROB_THRESH * 256) / 100))			
			{

				if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_SHORT_GI_40M) 
				{
					info->control->tx_rc_flag |= ATBM_IEEE80211_TX_RC_SHORT_GI_40M;
					info->control->tx_rc_flag |= ATBM_IEEE80211_TX_RC_SHORT_GI;
				}

				if (rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_SHORT_GI_20M)
				{
					info->control->tx_rc_flag |= ATBM_IEEE80211_TX_RC_SHORT_GI_20M;
					info->control->tx_rc_flag |= ATBM_IEEE80211_TX_RC_SHORT_GI;
				}
			}
      	}
	...
	txrc->tx_rc_in->max_rate_idx	|= ((wsm_rate_id & 0x0f) << 4);
	...
	txrc->tx_rc_in->max_rate_idx	|= (prate_set->max_tp_rate_try & 0x0f);
	...
	info->control->tx_max_rate = txrc->tx_rc_in->max_rate_idx;
...
static void hmac_rate_update(void * priv,struct atbmwifi_band_info *ch_info,  struct atbm_ieee80211_sta * sta, void * priv_sta, 
	   u32 changed, enum atbm_nl80211_channel_type oper_chan_type)
{

    struct atbm_ieee80211_sta_vht_cap * vht_cap;
    struct atbm_ieee80211_sta_he_cap * he_cap;
	struct hmac_rc_priv *rc_priv = (struct hmac_rc_priv*)priv;
	...
	atbm_printk_rc("%s chip_cat:%d  has_he:0x%x\n", __func__, rc_priv->chip_cat, rates_info->sta.he_cap.has_he);

	//reset rates info struct.
	reset_rate_info(rate_info, is_ap_flag);


	memcpy((u8*)(&rates_info->sta.ht_cap), (u8*)(sta->ht_cap),	sizeof(struct ieee80211_sta_ht_cap));
	memcpy((u8*)(&rates_info->sta.he_cap), (u8*)(sta->he_cap),  sizeof(struct atbm_ieee80211_sta_he_cap ));
	memcpy((u8*)(&rates_info->sta.vht_cap), (u8*)(sta->vht_cap),  sizeof(struct atbm_ieee80211_sta_vht_cap));
		  	
	rate_info->last_update_time = atbm_GetOsTick();
	rate_info->bw_total_pkt_update_time = atbm_GetOsTick();
	rate_info->tx_pkt_count_update_time = atbm_GetOsTick();
	rate_info->average_throughput = 0;
	atbm_printk_rc("rate_info: 0x%p last_update_time:%ld centerFreq:%d channel_type:%d\n", rate_info,
		 rate_info->last_update_time, ch_info->center_freq, ch_info->channel_type);
	rate_info->center_freq = ch_info->center_freq;

	/*
	ATBM_IEEE80211_TX_RC_HT_LDPC		= BIT(8)
	ATBM_IEEE80211_TX_RC_HE_LDPC		= BIT(9)
	ATBM_IEEE80211_TX_RC_HE_DCM			= BIT(10)
	ATBM_IEEE80211_TX_RC_HE_ER			= BIT(11)
	ATBM_IEEE80211_TX_RC_40M_TO_20M		= BIT(12)
	*/
	atbm_printk_rc("%s	sta_ht_cap:%x \n", __func__, sta_ht_cap);

	if (sta_ht_cap & ATBM_IEEE80211_HT_CAP_LDPC_CODING)
	{
		rates_info->tx_rc_flags |= ATBM_IEEE80211_TX_RC_HT_LDPC;
		atbm_printk_err(" %s,%d:HT LDPC\n", __func__, __LINE__);
	}

	if (sta_ht_cap & ATBM_IEEE80211_HT_CAP_SUP_WIDTH_20_40)
	{
		rates_info->tx_rc_flags |= ATBM_IEEE80211_TX_RC_20_MHZ_WIDTH;
	...
	if ((oper_chan_type != ATBM_NL80211_CHAN_HT40MINUS) && (oper_chan_type != ATBM_NL80211_CHAN_HT40PLUS))
	{
		atbm_printk_rc("%s,%d:IEEE80211_HT_CAP_SUP_WIDTH_20_40 not support 40M\n", __func__, __LINE__);

		//sta_cap &= ~IEEE80211_HT_CAP_SUP_WIDTH_20_40;
		rates_info->tx_rc_flags &= ~ATBM_IEEE80211_TX_RC_40_MHZ_WIDTH;
	}
	else 
	{
	...
	if (sta_ht_cap & ATBM_IEEE80211_HT_CAP_SGI_20)
	{
		rates_info->tx_rc_flags |= ATBM_IEEE80211_TX_RC_SHORT_GI_20M;
		atbm_printk_rc("HT support SHORT_GI_20M\n");
	}

	if (sta_ht_cap & ATBM_IEEE80211_HT_CAP_SGI_40)
	{
		rates_info->tx_rc_flags |= ATBM_IEEE80211_TX_RC_SHORT_GI_40M;
    ...
    rates_info->chip_category = rc_priv->chip_cat;
	...
	if (he_cap != NULL)
	{
		...
		if (he_cap->has_he) /*if HE mode support, start with HE mode*/
		{
				 ...
				 ATBM_IEEE80211_HE_PHY_CAP3_DCM_MAX_CONST_TX_BPSK)
			{
				...
				rates_info->tx_rc_flags |= ATBM_IEEE80211_TX_RC_HE_SUPPORT_DCM;
			}
			...
			{
				rates_info->tx_rc_flags |= ATBM_IEEE80211_TX_RC_HE_LDPC;
			}
			else
			{
				rates_info->tx_rc_flags &= ~ATBM_IEEE80211_TX_RC_HE_LDPC;
			...
			{
				rates_info->tx_rc_flags |= ATBM_IEEE80211_TX_RC_HE_ER; //HE ER operation.
			}
	...
	vht_cap				= sta->vht_cap;
    //atbm_printk_rc("%s	sta_vht_cap: 0x%x \n", __func__, vht_cap);
	if (vht_cap != NULL)
	{
		//atbm_printk_rc("%s	\n", __func__);
		if (vht_cap->vht_supported) /*if vht mode support, start with vht mode*/
		{
            if((rates_info->tx_rc_flags & ATBM_IEEE80211_TX_RC_40_MHZ_WIDTH) == 0)
         	{
                if(rates_info->max_vht_index > 8)
                {
                  rates_info->max_vht_index = 8;
			...
			atbm_printk_rc("vht_cap.cap:%x peer rx_mcs_map:0x%x\n", sta->vht_cap->cap, sta->vht_cap->vht_mcs.rx_mcs_map);
    		if (vht_cap->cap & ATBM_IEEE80211_VHT_CAP_RXLDPC)
    		{
    			rates_info->tx_rc_flags |= ATBM_IEEE80211_TX_RC_VHT_LDPC;
    			atbm_printk_rc(" %s,%d:VHT LDPC\n", __func__, __LINE__);
    		}
        }   
    }

    rates_info->bw_40m_cnt = 4;
	...
	atbm_printk_rc(" %s,%d: done, last_update_time:%ld chip_category:%d\n", __func__, __LINE__,rates_info->last_update_time, rates_info->chip_category);

}



static void hmac_rate_init(void * priv, struct atbmwifi_band_info *ch_info, 
	struct atbm_ieee80211_sta * sta, void * priv_sta)
{
	u8				is_ap_flag = 0;
	RATE_CONTROL_RATE * rate_info = priv_sta;
	...
	hmac_rate_update(priv, ch_info, sta, priv_sta, 0, ch_info->channel_type);
...
/*
sta : get tx status buffer for rate control
*/
static void * hmac_alloc_sta(void * priv, struct atbm_ieee80211_sta * sta, gfp_t gfp)
{

	RATE_CONTROL_RATE * pHmac_rate;  
    
	struct hmac_rc_priv *rc_priv;

	// uint8_t is_ap_flag;
	rc_priv = (struct hmac_rc_priv*)priv;
	atbm_printk_rc("%s	chip_category:%d\n", __func__, rc_priv->chip_cat);
    ...
    pHmac_rate->chip_category = rc_priv->chip_cat;
	...
	pHmac_rate->last_update_time = atbm_GetOsTick();
	...
	atbm_printk_rc("%s	chip_category:%d nrates:%d \n", __func__, rc_priv->chip_cat, sta->n_rates);

	return pHmac_rate;
}


static void * hmac_alloc(u8 chip_cat)
{
	struct hmac_rc_priv * mp;
	atbm_printk_rc( "hmac_alloc chip_cat:%d\n", chip_cat);
...
static void hmac_free_sta(void * priv, 	void * priv_sta)
{
	atbm_kfree(priv_sta);
}


static void hmac_rx_status(void * priv, void * priv_sta, 
	struct atbmn_ieee80211_rx_status * status)
{
   ...
   //if(ieee80211_is_data(status->frame_ctrl) || ieee80211_is_data_qos(status->frame_ctrl))
	if(status->flag & RX_FLAG_HE){
		...
		rx_rate_val = ax_rate_group16[status->rate_idx];
	}
	else if (status->flag & RX_FLAG_HE_ER){
		rc_sta_info->rx_rate_idx = status->rate_idx + 23;
		rx_rate_val = ax_rate_er242_16[status->rate_idx];
	}
	else if (status->flag & RX_FLAG_HT){
		rc_sta_info->rx_rate_idx = status->rate_idx + 14;
		rx_rate_val = n_rate_group[status->rate_idx];
	}
	else if (status->flag & RX_FLAG_VHT){
		rc_sta_info->rx_rate_idx = status->rate_idx + 38;
		rx_rate_val = n_rate_group[status->rate_idx];
	}
		...
		if (status->band == IEEE80211_BAND_5GHZ)
			...
			rx_rate_val = g_rate_group[status->rate_idx];
	}else{
		rc_sta_info->rx_rate_idx = status->rate_idx;
		rx_rate_val = b_rate_group[status->rate_idx];
	}
	
    //atbm_printk_rc("hmac_rx_status:rx_rate_idx=%d \n",rc_sta_info->rx_rate_idx);
	rc_sta_info->rssi_count++;

	rc_sta_info->received_num++;
   if(rc_sta_info->rx_rate_avg == 0)   	
   	{
   		rc_sta_info->rx_rate_avg =  rx_rate_val/2;
   	}
	else{
		rc_sta_info->rx_rate_avg = rc_sta_info->rx_rate_avg *4 / 5 + rx_rate_val/10; //unit M.
   	}
...
struct atbm_rate_control_ops mac80211_ratectrl =
{
    .name = "rc_hmac", 
...
{
	atbm_printk_rc("%s\n", __func__);
	return atbm_ieee80211_rate_control_register(&mac80211_ratectrl);
}
...
{
	atbm_printk_rc("%s\n", __func__);
	atbm_ieee80211_rate_control_unregister(&mac80211_ratectrl);
}
