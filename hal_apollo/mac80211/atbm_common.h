...
typedef struct  _AP_BASIC_INFO_ {
...
#define BEST_CHAN_SCAN_FLAGS_DISABLED BIT(0)
#define BEST_CHAN_SCAN_FLAGS_RADAR_DETECTED BIT(1)

struct best_ch_scan_result_def{
	unsigned char chan;
	unsigned char channel_ap_num;
	unsigned char busy_ratio;
	unsigned char chan_flags;
	//unsigned char weight;
};

typedef struct _best_ch_scan_result2_
{
	struct best_ch_scan_result_def chan[CHANNEL_NUM];
	unsigned int support_chan_num;
	unsigned int suggest_ch;
	AP_BASIC_INFO apInfo[13];
}Best_Channel_Scan_Result2;
typedef struct _best_ch_scan_result3_
{
	struct best_ch_scan_result_def chan[CHANNEL_NUM];
	unsigned int support_chan_num;
	unsigned int suggest_ch[3];
}Best_Channel_Scan_Result3;
