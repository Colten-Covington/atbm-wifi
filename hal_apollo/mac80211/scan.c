...
#include "../apollo.h"
void ieee80211_atbm_scan_cancel(struct ieee80211_hw *hw);
void __ieee80211_scan_cancel(struct ieee80211_local *local);
#endif
...
u8 CN_5G_chan[]={36,40,44,48,52,56,60,64,149,153,157,161,165,0};
u8 US_5G_chan[]={36, 40, 44, 48, 52, 56, 60, 64, 100, 104, 108, 112, 116, 120, 124, 128, 132, 136, 140, 149, 153, 157, 161, 165,0};
u8 JP_5G_chan[]={36, 40, 44, 48, 52, 56, 60, 64, 100, 104, 108, 112, 116, 120, 124, 128, 132, 136, 140, 144,0};
u8 ALL_5G_chan[]={34,36,38,40,42,44,46,48,52,56,60,64,100,104,108,112,116,120,124,128,132,136,140,144,149,153,157,161,165,0};
...
static int ieee80211_check_country_limit_scan_chan(struct ieee80211_local *local,struct cfg80211_scan_request *req)
{


  	int country_chan = 0,freq,scan_chan,scan_n_chans;
 	u8 *country_5g_chan = NULL;
  	int i = 0,j = 0,k = 0,m = 0;
	void *pptr[64];

  	do{
	  	if(memcmp(local->country_code,country_t[i].country,2) == 0){
		  	country_chan = country_t[i].chan_2_4g;
			country_5g_chan = country_t[i].chan_5g;
			break;
	  	}
	  	i++;
	...
	if((country_chan > 0) && (req != NULL)){
		...
		for(j = 0; j < req->n_channels ; j++){
			...
			scan_chan = ieee80211_frequency_to_channel(freq);
		
			if(((scan_chan >= 1) && (scan_chan <= 14)) && (scan_chan <= country_chan)){
				pptr[k++] = req->channels[j];
			//	atbm_printk_err("%s :country[%c%c] support chan [%d] ++++++++ \n",__func__,
			//						local->country_code[0],local->country_code[1],scan_chan);
				continue;
			}else if(scan_chan > 14 && country_5g_chan){//5g channel
				for(m = 0;country_5g_chan[m] != 0; m++){
					if(scan_chan == country_5g_chan[m]){ // country arry found ,scan channel is support
						pptr[k++] = req->channels[j]; // remenber channels point
					//	atbm_printk_err("%s :country[%c%c] support chan [%d] ++++++++ \n",__func__,
					//				local->country_code[0],local->country_code[1],scan_chan);
						break;
					}
				}
				// arry last is 0 , not support chan
				if(country_5g_chan[m] == 0){
					atbm_printk_err("%s :country[%c%c] not support chan [%d] ++++++++ \n",__func__,
									local->country_code[0],local->country_code[1],scan_chan);
					scan_n_chans--;
				}
...
static int ieee80211_country_scan_results_limit(struct ieee80211_local *local,
														struct sk_buff *skb,
														struct ieee80211_channel *channel)
{


	int freq,curr_chan,i=0,country_chan = 0,fc,j = 0;
	u8 *country_5g_chan = NULL;
	...
	if((country_chan > 0) && (skb != NULL)){
		...
		if(((curr_chan >= 1) && (curr_chan <= 14)) && (curr_chan > country_chan)){
			atbm_printk_err("%s : current scan results channals = %d,limit channel=%d ++++++++ \n",
					__func__,curr_chan,country_chan);
			return -1;
		}else if (curr_chan > 14 && country_5g_chan){
			for(j = 0;j < country_5g_chan[j];j++){
				if(curr_chan == country_5g_chan[j])
					break;
			}
			if(country_5g_chan[j] == 0){
				atbm_printk_err("%s : current scan results channals = %d ++++++++ \n",
					__func__,curr_chan);
				return -1;
			}
		}
		...
		atbm_printk_debug("ieee80211_country_scan_results_limit:%d :country_chan = %d \n",__LINE__,country_chan);
	}
	return 0;
...
//				should_split = true;
	...
	local->scan_idle_time = 0;
    if(local->scan_channel_space > local->hw.max_scan_channels_support){
        local->scan_channel_space = local->hw.max_scan_channels_support;
    }
...
static void ieee80211_internal_bss_info_update(struct ieee80211_local *local,
			struct ieee80211_bss *bss,
			struct ieee802_atbm_11_elems *elems,
			struct ieee80211_channel *channel,
			bool beacon,bool notransmit,u8 *transmitbssid)
{
	int clen, srlen;
	...
	bss->nontransmit = notransmit;
	bss->he_used = elems->he_cap && elems->he_cap_len;
    bss->vht_used = !!elems->vht_cap_elem;
//    atbm_printk_err("%s:bssid[%s][%d][%d][%p][%p]\n",__func__,bss->ssid,bss->he_used,bss->vht_used,elems->he_cap,elems->vht_cap_elem);
	if(transmitbssid)
		memcpy(bss->transbssid,transmitbssid,6);
	bss->band = channel->band;
}
struct ieee80211_bss *
			  ...
			  bool beacon,bool notransmit,u8 *transmitbssid)
{
	...
	*/
	...
	struct cfg80211_bss *notransmit_cbss;
	struct ieee80211_bss *notransmit_bss;
#endif
	s32 signal = 0;
	...
	if(ieee80211_country_scan_results_limit(local,NULL,channel) < 0)
		return NULL;
	...
	if(hw_priv->scan.req &&  hw_priv->scan.req->n_ssids > 0 && !hw_priv->scan.req->ssids[0].ssid_len){
			if(memcmp(elems->ssid,hw_priv->scan.req->ssids[0].ssid,hw_priv->scan.req->ssids[0].ssid_len) == 0){
			ieee80211_atbm_scan_cancel(&local->hw);
			atbm_printk_err("scan channel[%d] ssid=%s ssid_update = %s\n",channel_center_freq(channel),hw_priv->scan.req->ssids[0].ssid,elems->ssid);
			}
	}
#endif
...
#endif
	bss = (void *)cbss->priv;
	ieee80211_internal_bss_info_update(local,bss,elems,channel,beacon,notransmit,transmitbssid);	
#ifdef CONFIG_CFG880211_SUPPORT_HE
	list_for_each_entry(notransmit_cbss, &cbss->nontrans_list, nontrans_list) {
		notransmit_bss = (void *)notransmit_cbss->priv;
		ieee80211_internal_bss_info_update(local, notransmit_bss, elems,
						channel, beacon,true,transmitbssid);
	}
#endif
	return bss;
}
#ifndef CONFIG_CFG880211_SUPPORT_HE
bool ieee80211_is_element_inherited(const struct atbm_element *elem,
				   const struct atbm_element *non_inherit_elem)
{
...
static size_t ieee80211_gen_new_ie(const u8 *ie, size_t ielen,
				  ...
				  u8 *new_ie)
{
	...
	sub_copy = atbm_kmalloc(subie_len,GFP_KERNEL);
	if (!sub_copy)
		return 0;
	...
	/* set new ssid */
	tmp_new = atbm_ieee80211_find_ie(ATBM_WLAN_EID_SSID, sub_copy, subie_len);
	if (tmp_new) {
	...
	non_inherit_elem =
		ieee80211_find_ext_elem(ATBM_WLAN_EID_EXT_NON_INHERITANCE,
				       sub_copy, subie_len);

	/* go through IEs in ie (skip SSID) and subelement,
	 * merge them into new_ie
	 */
	tmp_old = atbm_ieee80211_find_ie(ATBM_WLAN_EID_SSID, ie, ielen);
	...
	while (tmp_old + tmp_old[1] + 2 - ie <= ielen) {
		...
		if (tmp_old[0] == ATBM_WLAN_EID_EXTENSION)
			tmp = (u8 *)ieee80211_find_ext_elem(tmp_old[2], sub_copy,
							 subie_len);
		else
			tmp = (u8 *)atbm_ieee80211_find_ie(tmp_old[0], sub_copy,
						     subie_len);

		if (!tmp) {
			const struct atbm_element *old_elem = (void *)tmp_old;

			/* ie in old ie but not in subelement */
			if (ieee80211_is_element_inherited(old_elem,
							  non_inherit_elem)) {
			 ...
			 */
			if (tmp_old[0] == ATBM_WLAN_EID_VENDOR_SPECIFIC) {
	...
	while (tmp_new + tmp_new[1] + 2 - sub_copy <= subie_len) {
		if (!(tmp_new[0] == ATBM_WLAN_EID_NON_TX_BSSID_CAP ||
		      tmp_new[0] == ATBM_WLAN_EID_SSID)) {
			memcpy(pos, tmp_new, tmp_new[1] + 2);
			pos += tmp_new[1] + 2;
		}
...
*ieee80211_get_profile_continuation(const u8 *ie, size_t ielen,
				   const struct atbm_element *mbssid_elem,
				   const struct atbm_element *sub_elem)
{
	const u8 *mbssid_end = mbssid_elem->data + mbssid_elem->datalen;
	const struct atbm_element *next_mbssid;
	const struct atbm_element *next_sub;

	next_mbssid = ieee80211_find_elem(ATBM_WLAN_EID_MULTIPLE_BSSID,
					 mbssid_end,
					 ielen - (mbssid_end - ie));
	...
	*/
	 ...
	 * of a new profile
	 */
	return next_sub->data[0] == ATBM_WLAN_EID_NON_TX_BSSID_CAP ?
	       NULL : next_mbssid;
}

size_t ieee80211_merge_profile(const u8 *ie, size_t ielen,
			      const struct atbm_element *mbssid_elem,
			      const struct atbm_element *sub_elem,
			      u8 *merged_ie, size_t max_copy_len)
{
	...
	while ((next_mbssid = ieee80211_get_profile_continuation(ie, ielen,
								mbssid_elem,
								sub_elem))) {
		const struct atbm_element *next_sub = (void *)&next_mbssid->data[1];
...
static inline void ieee80211_gen_new_bssid(const u8 *bssid, u8 max_bssid,
					  u8 mbssid_index, u8 *new_bssid)
{
	u64 bssid_u64 = atbm_ether_addr_to_u64(bssid);
...
static void ieee80211_parse_mbssid_data(struct ieee80211_local *local,
			struct ieee80211_rx_status *rx_status,
			struct atbm_ieee80211_mgmt *mgmt,
			size_t len,
			struct ieee802_atbm_11_elems *elems,
			...
			bool beacon)
{
	...
	const struct atbm_element *elem, *sub;
	size_t new_ie_len;
	...
	const u8 *ie;
	size_t ielen;
	struct atbm_ieee80211_mgmt *clone_mgmt=NULL;
	struct ieee80211_bss *bss;
	
	ie = mgmt->u.beacon.variable;
	ielen = len - ((u8 *) mgmt->u.beacon.variable - (u8 *) mgmt);

	new_ie = atbm_kmalloc(2304, GFP_KERNEL);
	if (!new_ie){
		atbm_printk_err("new_ie err\n");
		return;
	}
	profile = atbm_kmalloc(ielen, GFP_KERNEL);
	if (!profile){
		atbm_printk_err("profile err\n");
		goto out;
	}
	clone_mgmt = atbm_kmalloc(len,GFP_KERNEL);

	if(!clone_mgmt){
		atbm_printk_err("clone_mgmt err\n");
		goto out;
	}
	memcpy(clone_mgmt,mgmt,len);
	
	atbm_for_each_element_id(elem, ATBM_WLAN_EID_MULTIPLE_BSSID, ie, ielen) {
		if (elem->datalen < 4)
			continue;
		atbm_for_each_element(sub, elem->data + 1, elem->datalen - 1) {
			u8 profile_len;
			...
			profile_len = ieee80211_merge_profile(ie, ielen,
							     elem,
							     sub,
							     profile,
							     ielen);
			...
			mbssid_index_ie = atbm_ieee80211_find_ie
				(ATBM_WLAN_EID_MULTI_BSSID_IDX,
				 profile, profile_len);
			...
			ieee80211_gen_new_bssid(mgmt->bssid,
					       elem->data[0],
					       mbssid_index_ie[2],
					       new_bssid);
			
			memset(new_ie, 0, 2304);
			
			new_ie_len = ieee80211_gen_new_ie(ie, ielen,
							 profile,
							 profile_len, new_ie);
			if (!new_ie_len)
				continue;
			...
			memcpy(clone_mgmt->bssid,new_bssid,ETH_ALEN);
			memcpy(clone_mgmt->sa,new_bssid,ETH_ALEN);
			mgmt->u.beacon.capab_info = capability;
			if(new_ie_len > ielen){
				atbm_printk_err("new_ie_len (%zu),ielen(%zu)\n",new_ie_len,ielen);
				new_ie_len = ielen;
			}
			
			memcpy(clone_mgmt->u.beacon.variable,new_ie,new_ie_len);
			
			bss = ieee80211_bss_info_update(local,rx_status,clone_mgmt,
											new_ie_len + ((u8 *) mgmt->u.beacon.variable - (u8 *) mgmt),
											elems,channel,beacon,true,mgmt->bssid);
			if (!bss){
				atbm_printk_err("bss err\n");
				break;
			}
			bss->max_bssid_indicator = elem->data[0];
			bss->bssid_index = mbssid_index_ie[2];
...
out:
	atbm_kfree(new_ie);
	atbm_kfree(profile);
	atbm_kfree(clone_mgmt);
}
#endif
struct ieee80211_bss *ieee80211_multibss_info_update(struct ieee80211_local *local,
			struct ieee80211_rx_status *rx_status,
			struct atbm_ieee80211_mgmt *mgmt,
			size_t len,
			struct ieee802_atbm_11_elems *elems,
			...
			bool beacon)
{
	struct ieee80211_bss *transbss = NULL;	
	transbss = ieee80211_bss_info_update(local,rx_status,mgmt,len,elems,channel,beacon,false,mgmt->bssid);

	if(transbss == NULL)
		goto exit;
#ifndef CONFIG_CFG880211_SUPPORT_HE
	/*
	*check support multi bssid
	*/
//	atbm_printk_err("multibssid[%p][%d],he_cap[%p][%d]\n",elems->multibssid,elems->multibssid_len,elems->he_cap,elems->he_cap_len);
	if((elems->multibssid == NULL) || (elems->multibssid_len == 0)){
		goto exit;
	}
	/*
	*chech support he cap
	*/
	if((elems->he_cap == NULL) || (elems->he_cap_len == 0)){
		goto exit;
	}
	ieee80211_parse_mbssid_data(local,rx_status,mgmt,len,elems,channel,beacon);
#endif	
exit:
	return transbss;
}
...
void ieee80211_scan_cca_notify(struct ieee80211_hw *hw,struct ieee80211_internal_scan_notity *notify)
{
	...
	u8* radar_exist = notify->radar.val;
	/*
	...
	*/
	...
	for(index = 0;index<req->n_channels;index++){
		...
		req_wrap->radar_exist[channel_hw_value(req->channels[index])-1] = radar_exist[index];
		atbm_printk_debug("Chan[%d] radar[%d]\n", channel_hw_value(req->channels[index]), radar_exist[index]);
	}
}
...
void ieee80211_scan_radar_detected_put(struct ieee80211_hw *hw)
{
	struct ieee80211_local *local = hw_to_local(hw);
	struct ieee80211_scan_req_wrap *req_wrap = &local->scan_req_wrap;

	memset(req_wrap->radar_exist,0,IEEE80211_ATBM_MAX_SCAN_CHANNEL_INDEX);
}

u8* ieee80211_scan_radar_detected_get(struct ieee80211_hw *hw)
{
	struct ieee80211_local *local = hw_to_local(hw);
	struct ieee80211_scan_req_wrap *req_wrap = &local->scan_req_wrap;


	return req_wrap->radar_exist;
}
...
static int get_rsn_key_mgmt_to_bitfield(const u8 *s)
{
...
int get_ap_encryption_algorithm(char *ie,int ie_len, u32 *encry_info,enum ieee80211_enc_type enc_type)
{
	char *pos = NULL;
	int encry_type_count = 0,alg_i = 0;
	int left = 0;
	int encry_flag = 0;
	int rsn_authkey_mgmt = 0;
	int rsn_pairkey_mgmt = 0;
...
ieee80211_scan_rx_internal_sta_info(struct ieee80211_sub_if_data *sdata, struct sk_buff *skb)
{
	...
	if(scan_info.sta.ssid_len > IEEE80211_MAX_SSID_LEN)
		scan_info.sta.ssid_len = IEEE80211_MAX_SSID_LEN;
	if(elems.ssid_len>0)
		memcpy(scan_info.sta.ssid,elems.ssid,elems.ssid_len > IEEE80211_MAX_SSID_LEN ? IEEE80211_MAX_SSID_LEN : elems.ssid_len);
...
ieee80211_scan_rx_internal(struct ieee80211_sub_if_data *sdata, struct sk_buff *skb)
{
	...
	if(ieee80211_country_scan_results_limit(sdata->local,skb,NULL) < 0)
		return handle;
...
ieee80211_scan_rx(struct ieee80211_sub_if_data *sdata, struct sk_buff *skb)
{
	...
	if (baselen > skb->len)
		return RX_DROP_MONITOR;
    atbm_skb_queue_tail(&sdata->local->scan_rx,skb);
...
static bool ieee80211_prep_hw_scan(struct ieee80211_local *local)
{
	...
	do {
		...
		request.action = SCAN_POLICY_ACTION_PREPARE_CHANNEL;
		...
		*/
	    ...
	    atbm_printk_err("%s:scan index[%d][%d]\n",__func__,i,req->n_channels);	
		for (; i < req->n_channels; i++) {
			...
			*/
			...
			request.priv = req->channels[i];
			results = policy->func ? policy->func(&request) : SCAN_POLICY_RESULTS_OK;
...
start_scan:
	...
	ielen = ieee80211_build_preq_ies(local->scan_sdata, (u8 *)local->hw_scan_req->ie,
...
static void __ieee80211_scan_notify_bss(struct ieee80211_local *local)
{
	struct sk_buff *skb;
	struct ieee802_atbm_11_elems elems;
	struct ieee80211_rx_status *rx_status;
	...
	while ((skb = atbm_skb_dequeue(&local->scan_rx))){
		...
		bss = ieee80211_multibss_info_update(sdata->local, rx_status,
						mgmt, skb->len, &elems,
						channel,ieee80211_is_beacon(mgmt->frame_control));
		if (bss)
				       ...
				       bool was_hw_scan)
{
	...
	*/
	...
	ieee80211_chan_start_pending_csa(local);
#endif
	ieee80211_queue_work(&local->hw, &local->work_work);
    atbm_printk_scan("%s:completed\n",__func__);
}
#ifdef ATBM_USE_FASTLINK
void ieee80211_atbm_scan_cancel(struct ieee80211_hw *hw)
{
...
int __ieee80211_start_scan(struct ieee80211_sub_if_data *sdata,
				  struct cfg80211_scan_request *req)
{
	...
	if(ieee80211_check_country_limit_scan_chan(local,req) < 0){
		atbm_printk_scan("%s:%d\n",__func__,__LINE__);
		return -EBUSY;
	}
	...
	if (ieee80211_csa_busy(local)){
		...
		atbm_printk_scan("%s(%s):csa running,pend scan\n",__func__,sdata->name);
		return 0;
	}
#endif
	...
	*/
	...
	if (ieee80211_scan_policy_prepare(sdata) == true) {
		...
		atbm_printk_err("[%s] prepare scan policy err\n",sdata->name);
		rc = -1;
	}
	if (rc) {
		atbm_kfree(local->hw_scan_req);
...
#ifdef ATBM_USE_FASTLINK
	if (test_and_clear_bit(SCAN_CANCEL, &local->scanning)) {
		__ieee80211_scan_cancel(local);
		atbm_printk_err("ieee80211_scan_work:SCAN_CANSEL \n");
		mutex_unlock(&local->mtx);
		return;
	}
#endif
...
void ieee880211_start_pending_scan(struct ieee80211_local *local)
{
	struct cfg80211_scan_request *scan_req = local->scan_req;
	struct ieee80211_sub_if_data *scan_sdata = local->scan_sdata;
	
	if(scan_req == NULL || scan_sdata == NULL){
		return ;
	}

	if(local->scanning){
		atbm_printk_err("[scan runing,start pending?]\n");
		return;
	}
	...
	if(ieee80211_sdata_running(scan_sdata) == 0){
		goto done;
	}
	
	if(__ieee80211_start_scan(scan_sdata,scan_req)){
		goto done;
	}

	return;
done:
	atbm_printk_err("[%s]: force scan done\n",__func__);
	atbm_notify_scan_done(local,scan_req, false);
	local->scanning = 0;
	local->scan_req = NULL;
	local->scan_sdata = NULL;
	return;
...
bool ieee80211_internal_scan_triger(struct ieee80211_sub_if_data *sdata,struct cfg80211_scan_request *req)
{
	...
	if(local->internal_scan.req.req_flags & IEEE80211_INTERNAL_SCAN_FLAGS__UPDATE_BSS){
		local->scan_req_wrap.flags |= IEEE80211_SCAN_REQ_UPDATE_BSS;
	}
	...
	//if(local->internal_scan.req.req_flags & IEEE80211_INTERNAL_SCAN_FLAGS__RADAR_DETECT){
	//	local->scan_req_wrap.flags |= IEEE80211_SCAN_REQ_RADAR_DETECT;
	//}	
	...
	local->scan_req_wrap.req = local->hw_scan_req;
	if(ieee80211_scan_policy_prepare(sdata) == true){
		if(local->scan_req_wrap.flags & IEEE80211_SCAN_REQ_RADAR_DETECT){
			memset(sdata->scan_policy[IEEE80211_SCAN_POLICY_5G_IR].channel_unmap, 0xff,
				sizeof(sdata->scan_policy[IEEE80211_SCAN_POLICY_5G_IR].channel_unmap));
			atbm_printk_err("Skip second scan req for radar detect\n");
		}
		WARN_ON(!ieee80211_prep_hw_scan(local));
		rc = 0;
	}else {
...
void __ieee80211_scan_cancel(struct ieee80211_local *local)
{
	...
	if (test_bit(SCAN_HW_SCANNING, &local->scanning)) {
		...
		if(local->scan_req_wrap.flags & IEEE80211_SCAN_REQ_ETF){
			local->scan_req_wrap.flags |= IEEE80211_SCAN_REQ_ETF_STOP;
		}else
#endif
...
static enum scan_policy_results ieee80211_scan_policy_user(struct ieee80211_scan_policy_requset *request)
{
	enum scan_policy_results results = SCAN_POLICY_RESULTS_SKIP;
	...
	case SCAN_POLICY_ACTION_INIT:{
		...
		rcu_read_lock();
		conf = rcu_dereference(sdata->internal_ap_conf);
		while(conf){
			struct ieee80211_channel *ap_channel = ieee8011_chnum_to_channel(&sdata->local->hw,conf->channel);

			if(ap_channel == NULL){
				results =  SCAN_POLICY_RESULTS_SKIP;
				break;
			}
			memset(policy->channel_unmap,0xFF,sizeof(policy->channel_unmap));
			clear_bit(conf->channel,policy->channel_unmap);
			if(conf->channel <= 14){
				policy->band = IEEE80211_BAND_2GHZ;
			}else {
				policy->band = IEEE80211_BAND_5GHZ;
			}
			atbm_printk_debug("%s: use ap channel[%d]\n",__func__,conf->channel);
			results = SCAN_POLICY_RESULTS_FINISHED;
			break;
		}
		rcu_read_unlock();
		//results = SCAN_POLICY_RESULTS_SKIP;
	...
	case SCAN_POLICY_ACTION_PREPARE_CHANNEL:{
		struct ieee80211_channel *channel = request->priv;

		if(channel->band != policy->band){
			results = SCAN_POLICY_RESULTS_SKIP;
			break;
		}
		if(test_bit(channel_hw_value(channel),policy->channel_unmap)){
			results = SCAN_POLICY_RESULTS_SKIP;
			break;
		}
		results = SCAN_POLICY_RESULTS_FINISHED;
	...
	case SCAN_POLICY_ACTION_PREPARE_RX:
		results = SCAN_POLICY_RESULTS_OK;
		break;
	}

	return  results;
}
static enum scan_policy_results ieee80211_scan_policy_2G(struct ieee80211_scan_policy_requset *request)
{
	enum scan_policy_results results = SCAN_POLICY_RESULTS_OK;
	struct ieee80211_scan_policy *policy = request->policy;
	...
	switch(request->action){
	case SCAN_POLICY_ACTION_INIT:{
		memset(policy->channel_unmap,0x00,sizeof(policy->channel_unmap));
		results = SCAN_POLICY_RESULTS_OK;
		break;
	}
	case SCAN_POLICY_ACTION_PREPARE_CHANNEL:{
...
static enum scan_policy_results ieee80211_scan_policy_5G_no_ir(struct ieee80211_scan_policy_requset *request)
{
	enum scan_policy_results results = SCAN_POLICY_RESULTS_OK;
	struct ieee80211_scan_policy *policy = request->policy;
	...
	case SCAN_POLICY_ACTION_PREPARE_CHANNEL:{
		...
		if(test_bit(channel_hw_value(channel),policy->channel_unmap)){
			atbm_printk_err("5G_no_ir channel(%d) skip\n",channel_hw_value(channel));
			results = SCAN_POLICY_RESULTS_SKIP;
			break;
		}
		if(channel->flags & (BIT(1)|IEEE80211_CHAN_RADAR)){ //BIT(1) is no-ir or passive scan
				channel_mask_no_ir(channel);
				results = SCAN_POLICY_RESULTS_OK;
				break;
		}
		results = SCAN_POLICY_RESULTS_SKIP;
...
static enum scan_policy_results ieee80211_scan_policy_5G_ir(struct ieee80211_scan_policy_requset *request)
{
	enum scan_policy_results results = SCAN_POLICY_RESULTS_OK;
	struct ieee80211_scan_policy *policy = request->policy;
	...
	case SCAN_POLICY_ACTION_PREPARE_CHANNEL:{
		...
		if(test_bit(channel_hw_value(channel),policy->channel_unmap)){
			atbm_printk_scan("5G_ir channel(%d) skip\n",channel_hw_value(channel));
			results = SCAN_POLICY_RESULTS_SKIP;
			break;
		}
		if(channel_is_no_ir(channel)){
			atbm_printk_scan("5G_ir channel(%d) skip no ir\n",channel_hw_value(channel));
			results = SCAN_POLICY_RESULTS_SKIP;
			break;
		}
		results = SCAN_POLICY_RESULTS_OK;
...
#endif
bool ieee80211_scan_policy_prepare(struct ieee80211_sub_if_data *sdata)
{
	struct ieee80211_local *local = sdata->local;
	u8 index = 0;
	struct ieee80211_scan_policy *policy;
    ...
    ieee80211_ht_cap_adjust_by_customer(&local->hw,sdata);

	request.action = SCAN_POLICY_ACTION_INIT;
	request.sdata  = sdata;
	for(index = 0; index < IEEE80211_SCAN_POLICY_MAX ; index++){
		...
		case SCAN_POLICY_RESULTS_OK:
			...
			break;
		case SCAN_POLICY_RESULTS_SKIP:
			break;
		case SCAN_POLICY_RESULTS_FINISHED:
			list_add_tail(&policy->list,&local->scan_policy);
			index = IEEE80211_SCAN_POLICY_MAX;
			break;
		}
	}

	return list_empty(&local->scan_policy) ? false : true;
}
void ieee80211_scan_policy_init(struct ieee80211_sub_if_data *sdata)
{
	u8 index = 0;
	struct ieee80211_scan_policy *policy;
	...
	for(index = 0; index < IEEE80211_SCAN_POLICY_MAX; index++){
		...
		case IEEE80211_SCAN_POLICY_USER_CHANNEL:
			...
			break;
		case IEEE80211_SCAN_POLICY_2_4G:
			policy->band   = IEEE80211_BAND_2GHZ;
			policy->func   = ieee80211_scan_policy_2G;
			break;
#if defined (CONFIG_ATBM_APOLLO_5GHZ_SUPPORT)
		case IEEE80211_SCAN_POLICY_5G_ON_IR:
			policy->band   = IEEE80211_BAND_5GHZ;
			policy->func   = ieee80211_scan_policy_5G_no_ir;
			break;
#endif
#if  defined (CONFIG_ATBM_5G_PRETEND_2G) || defined (CONFIG_ATBM_APOLLO_5GHZ_SUPPORT)
		case IEEE80211_SCAN_POLICY_5G_IR:
			policy->band   = IEEE80211_BAND_5GHZ;
			policy->func   = ieee80211_scan_policy_5G_ir;
			break;
#endif
		default:
			break;
...
void ieee80211_scan_policy_rx(struct ieee80211_sub_if_data *sdata, struct sk_buff *skb)
{
	struct ieee80211_local *local = sdata->local;
	struct ieee80211_scan_policy *policy = local->policy;
	struct atbm_ieee80211_mgmt *mgmt;
	struct ieee80211_rx_status *rx_status = IEEE80211_SKB_RXCB(skb);
	u8 *elements;
	struct ieee80211_channel *channel;
	...
	request.action = SCAN_POLICY_ACTION_PREPARE_RX;
	request.policy = policy;
	request.sdata  = local->scan_sdata;
	request.priv   = channel;
	BUG_ON(policy->func == NULL);
	policy->func(&request);
}
