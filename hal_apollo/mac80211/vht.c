...
void ieee80211_vht_cap_ie_to_sta_vht_cap(struct ieee80211_sub_if_data *sdata,
				    ...
				    struct sta_info *sta)
{
	struct atbm_ieee80211_sta_vht_cap *vht_cap = &sta->sta.vht_cap;
	...
	if (!vht_cap_ie || (atbm_ieee80211_vht_supported(&sdata->local->hw,sband) == false))
		return;
    
    sband_cap = atbm_ieee80211_get_vht_cap(&sdata->local->hw,sband,sdata);
    ...
    memcpy(&own_cap,sband_cap,sizeof(struct atbm_ieee80211_sta_vht_cap));
#if 0
	/*
	...
	vht_cap->cap |= min_t(u32, cap_info & ATBM_IEEE80211_VHT_CAP_MAX_MPDU_MASK,
	...
	if (ieee80211_hw_check(&sdata->local->hw, SUPPORTS_VHT_EXT_NSS_BW))
		...
		vht_cap->vht_mcs.tx_highest &=
			~cpu_to_le16(ATBM_IEEE80211_VHT_EXT_NSS_BW_CAPABLE);
	...
	for (i = 0; i < 8; i++) {
		...
		vht_cap->vht_mcs.rx_mcs_map &=
			~cpu_to_le16(ATBM_IEEE80211_VHT_MCS_NOT_SUPPORTED << i * 2);
	...
	if (vht_cap->vht_mcs.rx_mcs_map == cpu_to_le16(0xFFFF)) {
		...
		atbm_printk_err("Ignoring VHT IE from %pM due to invalid rx_mcs_map\n",
			   sta->sta.addr);
		return;
	}

    set_sta_flag(sta, WLAN_STA_VHT);
}
#if 0
