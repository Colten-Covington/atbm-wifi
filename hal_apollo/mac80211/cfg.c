...
#endif
extern unsigned int atbm_get_rate_from_rateid(unsigned char maxrate_id,unsigned char dcm_used,unsigned char support_40M_symbol);
...
extern  bool atbm_internal_cmd_scan_triger(struct ieee80211_sub_if_data *sdata,struct ieee80211_internal_scan_request *req);
...
#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(3,6,0))
...
#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(3,7,0))
					const char *name,
#else
					char *name,
#endif
#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(4, 1, 0))
				unsigned char name_assign_type,
#endif
			      enum nl80211_iftype type,
#if (LINUX_VERSION_IS_LEQ_AND_NOT_CPTCFG(4, 20, 0))
			      u32 *flags,
#endif
...
#else
	 atbm_printk_init("add iftype(%d),if_cnt(%d)\n",type,interface_cnt);
#endif 
	if(interface_cnt>=2){
...
static int ieee80211_change_iface(struct wiphy *wiphy,
				  struct net_device *dev,
				  enum nl80211_iftype type,
#if (LINUX_VERSION_IS_LESS_AND_NOT_CPTCFG(4,20,0))
				  u32 *flags,
#endif
				  struct vif_params *params)
{
...

{
 ...
 static void rate_idx_to_bitrate_temp(struct rate_info *rate, struct sta_info *sta, int idx)
 {
	 ...
	 if (!(rate->flags & RATE_INFO_FLAGS_MCS)) {
		...
		// atbm_printk_err("%s:sband->bitrates[%d].bitrate=%d \n",__func__,idx,sband->bitrates[idx].bitrate);
		 
		 rate->legacy = atbm_get_rate_from_rateid(idx,0,0)/50000;//sband->bitrates[idx].bitrate;
	 } else
		 rate->mcs = idx;
 }
#if 0
...
static void sta_set_sinfo(struct sta_info *sta, struct station_info *sinfo)
{
    ...
    RATE_CONTROL_RATE *rate_info = NULL;

	sinfo->generation = sdata->local->sta_generation;
#if (LINUX_VERSION_IS_LESS_AND_NOT_CPTCFG(4, 0, 0))
	...
	if (sta->last_tx_rate.flags & ATBM_IEEE80211_TX_RC_MCS)
		sinfo->txrate.flags |= RATE_INFO_FLAGS_MCS;
	if (sta->last_tx_rate.flags & ATBM_IEEE80211_TX_RC_40_MHZ_WIDTH)
#if (LINUX_VERSION_IS_LESS_AND_NOT_CPTCFG(4, 0, 0))		
		sinfo->txrate.flags |= RATE_INFO_FLAGS_40_MHZ_WIDTH;
#else
		sinfo->txrate.bw = RATE_INFO_BW_40;
#endif
	if (sta->last_tx_rate.flags & ATBM_IEEE80211_TX_RC_SHORT_GI)
		sinfo->txrate.flags |= RATE_INFO_FLAGS_SHORT_GI;
	
	rate_info = (RATE_CONTROL_RATE*)sta->rate_ctrl_priv;
	rate_idx_to_bitrate_temp(&sinfo->txrate, sta, rate_info->max_tp_rate_idx);	
...
//	rate_idx_to_bitrate(&sinfo->txrate, sta, sta->last_tx_rate.idx);
	...
	rate_idx_to_bitrate_temp(&sinfo->rxrate, sta, rate_info->rx_rate_idx);
...
static int ieee80211_get_station(struct wiphy *wiphy, struct net_device *dev,
				 #if (LINUX_VERSION_IS_LESS_AND_NOT_CPTCFG(3, 16, 0))
				 u8 *mac,
				 #else
				 const u8 *mac,
				 #endif
...
static void ieee80211_config_ap_cap(struct ieee80211_sub_if_data *sdata,u8 * ie, size_t len)
{
	if(ie == NULL  || len <= 0){
		return;
	}
	...
	*/
	if(sdata->vif.p2p == true){
		...
		ieee802_11_parse_elems(ie,len, false, &elems,NULL,NULL);

		if(elems.ht_cap_elem){
			elems.ht_cap_elem->cap_info &= ~(IEEE80211_HT_CAP_SUP_WIDTH_20_40 | 
		           IEEE80211_HT_CAP_DSSSCCK40 |
		           IEEE80211_HT_CAP_SGI_40);
		}

		if(elems.ht_info_elem){
			elems.ht_info_elem->ht_param &= ~(BIT(0) | BIT(1) | BIT(2));
		}
	}
}
...
#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(3, 1, 0))
				   ...
				   const struct ieee80211_color_change_settings *cca)
{
	...
	ieee80211_config_ap_cap(sdata,new->tail,new->tail_len);
#ifdef CONFIG_ATBM_HE_AP
	ieee80211_setup_ap_he_data(sdata);
#endif
	...
	*(__le16 *)new->head = __cpu_to_le16(IEEE80211_FTYPE_MGMT | IEEE80211_STYPE_PROBE_RESP);
	...
	if (csa)
		memcpy(new->cntdwn_counter_offsets, csa->counter_offsets_presp,
		       csa->n_counter_offsets_presp * sizeof(new->cntdwn_counter_offsets[0]));
	else if (cca)
...
static int ieee80211_set_beacon_sdata(struct ieee80211_sub_if_data *sdata,
				struct beacon_parameters *params,
				   const struct ieee80211_csa_settings *csa,
				   const struct ieee80211_color_change_settings *cca)
{
	...
	ret = ieee80211_config_proberesp(sdata, params,csa,cca);
	if (!ret)
		ret = ieee80211_config_beacon(sdata, params,csa,cca);
	return ret;
#else

	return ieee80211_config_beacon(sdata, params,csa,cca);
#endif
}
				...
				struct beacon_parameters *params)
{
	...
	if (old)
		return -EALREADY;
#ifdef ATBM_AP_SME
	ieee80211_ap_sme_paras_ap_info(sdata,params);
	ieee80211_config_associate_ie(sdata,params);
#endif
	...
	ret = ieee80211_set_beacon_sdata(sdata, params,NULL,NULL);
	if(!ret)
...
//	int ret =0;
	...
	atbm_printk_always("[%s] set_beacon[%d]\n",sdata->name,sdata->csa_state);
	if(ieee80211_csa_busy(sdata->local)){
		return -EBUSY;
	}
#endif
	old = rtnl_dereference(sdata->u.ap.beacon);
	...
	return ieee80211_set_beacon_sdata(sdata, params,NULL,NULL);
}
							...
							struct cfg80211_ap_settings *settings)
{
	...
	||(defined(CONFIG_ATBM_RADAR_DETECT) && LINUX_VERSION_IS_GEQ_OR_CPTCFG(3, 9, 0))
	struct ieee80211_sub_if_data *sdata= IEEE80211_DEV_TO_SUB_IF(dev);
#endif  //CONFIG_ATBM_HE
	...
	sdata->radar_required = cfg80211_chandef_dfs_required(wiphy,
					    &settings->chandef,
					    sdata->wdev.iftype) > 0 ? true : false;
	atbm_printk_debug("sdata:%p radar_required:%x\n", sdata, sdata->radar_required);
#endif
#endif

#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(3, 6, 0))
#if (LINUX_VERSION_IS_LEQ_AND_NOT_CPTCFG(3, 7, 2))
	...
	if (settings->he_cap && settings->he_oper) {
		u32 changed=0;
		sdata->vif.bss_conf.he_support = true;
		sdata->vif.bss_conf.htc_trig_based_pkt_ext =
			le32_get_bits(settings->he_oper->he_oper_params,
	...
	sdata->vif.bss_conf.twt_responder = settings->twt_responder;
	/*
	...
	*/
	sdata->vif.bss_conf.he_obss_pd.enable = settings->he_obss_pd.enable;
	sdata->vif.bss_conf.he_obss_pd.sr_ctrl = settings->he_obss_pd.sr_ctrl;
	sdata->vif.bss_conf.he_obss_pd.non_srg_max_offset = settings->he_obss_pd.non_srg_max_offset;
	sdata->vif.bss_conf.he_obss_pd.min_offset = settings->he_obss_pd.min_offset;
	memcpy(sdata->vif.bss_conf.he_obss_pd.bss_color_bitmap, settings->he_obss_pd.bss_color_bitmap,8);
	memcpy(sdata->vif.bss_conf.he_obss_pd.partial_bssid_bitmap,settings->he_obss_pd.partial_bssid_bitmap,8);
	
	//sdata->vif.bss_conf.he_bss_color = settings->he_bss_color;
	sdata->vif.bss_conf.he_bss_color.color = settings->he_bss_color.color;
	sdata->vif.bss_conf.he_bss_color.enabled = settings->he_bss_color.enabled;
	sdata->vif.bss_conf.he_bss_color.partial = settings->he_bss_color.partial;
#endif  //CONFIG_ATBM_HE
									   ...
									   struct cfg80211_beacon_data *info)
{
	...
	memcpy(ssid,sdata->vif.bss_conf.ssid,sdata->vif.bss_conf.ssid_len);
	params.ssid     = ssid;
	params.ssid_len = sdata->vif.bss_conf.ssid_len;
	params.hidden_ssid =  sdata->vif.bss_conf.hidden_ssid ? 
						  NL80211_HIDDEN_SSID_ZERO_CONTENTS : NL80211_HIDDEN_SSID_NOT_IN_USE;
	return ieee80211_set_beacon(wiphy, dev,&params);
}

#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
static int ieee80211_del_beacon(struct wiphy *wiphy, struct net_device *dev,unsigned int link_id)
...
#endif
#ifdef CONFIG_ATBM_HE_AP
	old_mlme = rtnl_dereference(sdata->u.ap.mlme);
	rcu_assign_pointer(sdata->u.ap.mlme, NULL);
	synchronize_rcu();
	if(old_mlme){
		atbm_kfree(old_mlme);
	}
...
*/
    ...
    if(params->vht_capa){
        ieee80211_vht_cap_ie_to_sta_vht_cap(sdata,sband,(struct atbm_ieee80211_vht_cap *)params->vht_capa,sta);
    }
#endif
#endif
#endif	
				...
				struct station_parameters *params)
{
	...
	rcu_read_unlock();
    ieee80211_recalc_dcm(sdata->local);
#endif
	return 0;
}

static int ieee80211_del_station(struct wiphy *wiphy, struct net_device *dev,
				#if (LINUX_VERSION_IS_LESS_AND_NOT_CPTCFG(3, 18, 31))
				u8 *mac
				#elif (LINUX_VERSION_IS_LESS_AND_NOT_CPTCFG(3, 19, 0))
				const u8 *mac
				#else
				...
				)
{
	...
	struct ieee80211_local *local = wiphy_priv(wiphy);
#endif
#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(3, 19, 0))
				    ...
				    struct station_parameters *params)
{
	...
	sta_apply_parameters(local, sta, params);
	/*
	*sta has been authorized,set fast tx
	*/
	ieee80211_assign_sta_hdr(sta);
	rcu_read_unlock();
...
#endif
	struct atbm_ieee80211_tx_queue_params p;
...
#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(3, 8, 0))
...
static int ieee80211_request_fast_link(struct ieee80211_sub_if_data *sdata,struct cfg80211_scan_request *req)
{
	...
	memset(&(sdata->local->scan_info),0,sizeof(struct cfg80211_scan_info));
	memcpy(sdata->associated_ssid,req->ssids[0].ssid,32);
	sdata->associated_ssid_len = req->ssids[0].ssid_len;
	atbm_printk_err("fast: %s: Ssid = %s,ssid_len = %d\n",sdata->name,req->ssids[0].ssid,req->ssids[0].ssid_len);
	if(memcmp(sdata->name,"p2p",3))
		fp =filp_open(ATBM_SAVE_BSS_ID_0,O_RDWR,0644);
	else
		fp =filp_open(ATBM_SAVE_BSS_ID_1,O_RDWR,0644);

	if (IS_ERR(fp)){
		atbm_printk_err("fast: %s :open file error,go to scan\n",sdata->name);
		sdata->fastlink_enable = 0;
		return ieee80211_request_scan(sdata, req);	
	}else{
		fs = get_fs();
		set_fs(KERNEL_DS);
		pos = 0;
		kernel_read(fp,fp->f_pos,&len,sizeof(len));
		atbm_printk_err("fast: %s : read mgmt.ielen----- = %d\n",sdata->name,len);
		if(len){
			fast_mgmt = kzalloc(len, GFP_ATOMIC);
		
		kernel_read(fp,sizeof(len),fast_mgmt,len);
			
		atbm_printk_err("fast: fast_mgmt = %p \n",fast_mgmt);
		...
		fast_channel = ieee80211_get_channel(sdata->local->hw.wiphy, freq);
		//for(i_test=0;i_test<len ;i_test++){
		//	printk("%x,",*((u8*)fast_mgmt + i_test));
		//}
		//printk("\n");
		atbm_printk_err(" fast: %s :channel.center_freq = %d \n",sdata->name,fast_channel->center_freq);
		cbss = cfg80211_inform_bss_frame(sdata->local->hw.wiphy, fast_channel,
		...
		if (!cbss)
		{
            ...
            set_fs(fs);
            kfree(fast_mgmt);
            cfg80211_scan_done(req,&(sdata->local->scan_info));        
            atbm_printk_err("fast: %s link cbss err \n",sdata->name);
            return 0;
		}
		...
		ieee80211_atbm_put_bss(sdata->local->hw.wiphy,container_of((void *)bss, struct cfg80211_bss, priv));
		kfree(fast_mgmt);
		}
	filp_close(fp,NULL);
	set_fs(fs);
...
#if	(LINUX_VERSION_IS_GEQ_OR_CPTCFG(4,9,0))
...
#ifdef ATBM_USE_FASTLINK
	if(sdata->fastlink_enable == 1)
	{
		sdata->fastlink_enable == 0;
		return ieee80211_request_fast_link(sdata, req);
	}
#endif	
	return ieee80211_request_scan(sdata, req);
}
...
void ieee80211_start_ap_changechannel(struct wiphy *wiphy, struct ieee80211_sub_if_data *sdata,struct cfg80211_auth_request *req)
{
	...
	if(ht_information_ie){
		hti = (struct atbm_ieee80211_ht_info *)&ht_information_ie[2];
	...
	list_for_each_entry_rcu(ap_sdata, &local->interfaces, list) {
		...
		ieee80211_bss_info_change_notify(ap_sdata,	BSS_CHANGED_BEACON | BSS_CHANGED_HT_CHANNEL_TYPE);
...
void ieee80211_ap_channel_event_work(struct atbm_work_struct *work)
{
	...
	atbm_printk_err("ieee80211_ap_channel_event_work,chan_type=%d\n",chan_state->_oper_channel_type);
			  ...
			  struct cfg80211_auth_request *req)
{
	...
	if((atbm_ieee80211_suspend(local)==true) || (ieee80211_sdata_running(sdata) == 0)){
	...
	if ((req->bss->proberesp_ies==NULL) && (req->bss->beacon_ies == NULL)){
		/*
		*trigger scan to probe ap
		*/
		struct ieee80211_internal_scan_request internal_scan;
		u8 channel = (u8)channel_hw_value(req->bss->channel);
        u8 tries = 0;
...
retry:
		...
		if((req->bss->proberesp_ies == NULL) && 
		   (req->bss->beacon_ies == NULL)){
		   goto retry;
		}
...
static int ieee80211_set_wiphy_params(struct wiphy *wiphy,
		#if (LINUX_VERSION_IS_LESS_AND_NOT_CPTCFG(3, 0, 0))
		struct net_device *dev, 
		#endif
		u32 changed)
{
...
#endif
	work_on = ieee80211_work_busy(local);
...
static int ieee80211_remain_on_channel(struct wiphy *wiphy,
					...
					#endif
				       struct ieee80211_channel *chan,
				       #if (LINUX_VERSION_IS_LESS_AND_NOT_CPTCFG(3,8,0))
				       enum nl80211_channel_type channel_type,
				       #endif
...
static int ieee80211_mgmt_tx(struct wiphy *wiphy, 
			...
			struct net_device *dev,
#endif
#if (LINUX_VERSION_IS_LESS_AND_NOT_CPTCFG(3,14,0))
...
//	printk( "%s:req_chan(%x),tmp_channel(%x),oper_channel(%x),hw_roc_channel(%x)\n",__func__,*cookie);
	...
	data = atbm_skb_put_data(skb, buf, len);
	...
	if (sdata->vif.csa_active &&
		...
		struct proberesp_data *probe_resp = NULL;
		rcu_read_lock();
		...
		if (sdata->vif.type == NL80211_IFTYPE_AP)
			...
			probe_resp = (struct proberesp_data *)rcu_dereference(sdata->u.ibss.presp);
#endif
#ifdef CONFIG_MAC80211_ATBM_MESH
		else if (ieee80211_vif_is_mesh(&sdata->vif))
			beacon = rcu_dereference(sdata->u.mesh.beacon);
#endif //CONFIG_MAC80211_ATBM_MESH
		...
		if (beacon){
			...
			for (i = 0; i < params->n_csa_offsets; i++)
				data[params->csa_offsets[i]] =
					probe_resp->cntdwn_current_counter;	
		}
#endif
		rcu_read_unlock();
	}
...
#endif
#if (LINUX_VERSION_IS_LESS_AND_NOT_CPTCFG(5, 10, 0))
				    ...
				    struct cfg80211_gtk_rekey_data *data)
{
	...
	ieee80211_rekey_set_data_enable(sdata,data->kck,data->kek);
	return 0;
}
#endif
#endif
...
#ifdef CONFIG_ATBM_RADAR_DETECT
#if LINUX_VERSION_IS_GEQ_OR_CPTCFG(3, 9, 0)
static int ieee80211_dfs_set_channel(struct ieee80211_sub_if_data *sdata,
	...
	case CHAN_MODE_FIXED:
		...
		if(chan_state->oper_channel != chan)
			return -EBUSY;
		/*
		*chantype equal not start radar
		*req 40M cur 40M busy
		*req 40M cur 20M start radar
		*req 20M not start radar
		*/
		if(chan_state->_oper_channel_type != channel_type){
			char request_type = ieee80211_chw_is_ht40(channel_type);
			char oper_type = ieee80211_chw_is_ht40(chan_state->_oper_channel_type);
			if(request_type == 1){
				if(request_type == oper_type)
					return -EBUSY;
				else
					break;
			}
	...
	atbm_printk_debug("Need start radar detect %d %d\n", chan->center_freq, channel_type);
	local->hw.conf.radar_enabled = true;
	sdata->radar_required = true;
	local->cac_sdata = sdata;
	local->dfs_cac_chan_def = *chandef;
	chan_state->_oper_channel_type = channel_type;
	chan_state->oper_channel = chan;
...
static int ieee80211_start_radar_detection(struct wiphy *wiphy,
					   struct net_device *dev,
					   struct cfg80211_chan_def *chandef
#if LINUX_VERSION_IS_GEQ_OR_CPTCFG(3, 16, 0)
					   ,u32 cac_time_ms
#endif
					   )
{
...
#endif
	atbm_printk_debug("Start radar detection %d %d %d\n", chandef->chan->center_freq,
		chandef->width, cac_time_ms);

	err = ieee80211_dfs_set_channel(sdata, chandef);
	if (err)
		return err;
			      ...
			      struct net_device *dev)
{
	...
	atbm_printk_err("Radar detect end\n");
	mutex_lock(&local->mtx);
	list_for_each_entry(sdata, &local->interfaces, list) {
		 ...
		 */
		atbm_cancel_delayed_work(&sdata->dfs_cac_timer_work);
					...
					u32 *changed)
{
	...
	case NL80211_IFTYPE_AP: {
		...
		struct beacon_parameters params;
		memset(&params,0,sizeof(struct beacon_parameters));
		BEACON_PARAMS_CPY(params,sdata->u.ap.next_beacon);

		ret = ieee80211_set_beacon_sdata(sdata, &params,
					      NULL, NULL);
...
ieee80211_set_color_change_beacon(struct ieee80211_sub_if_data *sdata,
				  struct cfg80211_color_change_settings *setting,
				  u32 *changed)
{
	...
	case NL80211_IFTYPE_AP:
		{
			struct beacon_parameters params;
			memset(&params,0,sizeof(struct beacon_parameters));
			BEACON_PARAMS_CPY(params,&setting->beacon_color_change);
		
			sdata->u.ap.next_beacon =	cfg80211_beacon_dup(&setting->beacon_next);
			...
			color_change.counter_offset_beacon =
				setting->counter_offset_beacon;
			color_change.counter_offset_presp =
				setting->counter_offset_presp;
			color_change.count = setting->count;

			//err = ieee80211_assign_beacon(sdata, &params->beacon_color_change,
			//			      NULL, &color_change);
			

			err = ieee80211_set_beacon_sdata(sdata, &params,NULL,&color_change);
...
#endif
#ifdef CONFIG_ATBM_SUPPORT_CSA
#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(3,12,0)) ||(CONFIG_ATBM_LOW_KERNEL_SUPPORT_CSA == 1)

static struct beacon_data *ieee80211_csa_beacon_data_alloc(struct cfg80211_beacon_data *data)
{
	struct beacon_data *new;

	new = atbm_kzalloc(sizeof(struct beacon_data) + 
			data->head_len + data->tail_len + data->beacon_ies_len, GFP_KERNEL);

	if(new){
		new->head = ((u8 *) new) + sizeof(*new);
		new->tail = new->head + data->head_len;
		new->head_len = data->head_len;
		new->tail_len = data->tail_len;

		memcpy(new->head,data->head,data->head_len);
		memcpy(new->tail,data->tail,data->tail_len);

		if(data->beacon_ies){
			memcpy(new->tail + new->tail_len,data->beacon_ies,data->beacon_ies_len);
			new->tail_len += data->beacon_ies_len;
		}
	}
...
static struct proberesp_data *ieee80211_csa_probersp_data_alloc(struct cfg80211_beacon_data *data)
{
	struct proberesp_data *new;

	new = atbm_kzalloc(sizeof(struct proberesp_data) + 
			data->probe_resp_len + data->proberesp_ies_len, GFP_KERNEL);

	if(new){
		new->head = ((u8 *) new) + sizeof(*new);
		new->tail = new->head + data->probe_resp_len;
		new->head_len = data->probe_resp_len;
		new->tail_len = data->proberesp_ies_len;

		memcpy(new->head,data->probe_resp,data->head_len);

		if(data->proberesp_ies_len){
			memcpy(new->tail,data->proberesp_ies,data->proberesp_ies_len);
		}
	}
...
static int ieee80211_ap_channel_switch(struct wiphy *wiphy, struct net_device *dev,
			     struct cfg80211_csa_settings *params)
{
	...
	if(ieee80211_csa_busy(local)){
		err = -EBUSY;
		atbm_printk_err("csa_mlme[%s][%d] busy\n",sdata->name,local->next_csa_state);
		goto exit;
	}

	if(ieee80211_get_channel_mode(local, sdata) == CHAN_MODE_FIXED){
		err = -EBUSY;
		atbm_printk_err("channel_mode[%s][%d] busy\n",sdata->name,ieee80211_get_channel_mode(local, sdata));
		goto exit;
	}
	if(ieee80211_get_channel_state(local, sdata)->oper_channel == params->chandef.chan){
		struct cfg80211_chan_def chandef;
		/*
		*not report err
		*/
		err = 0;
		cfg80211_chandef_create(&chandef,params->chandef.chan,sdata->vif.bss_conf.channel_type);
		ieee80211_ch_switch_notify(sdata,&chandef);
		goto exit;
	}
	if(params->beacon_csa.head == NULL || params->beacon_csa.tail == NULL || 
	   params->beacon_after.head == NULL || params->beacon_after.tail == NULL){
		err = -EINVAL;
		goto exit;
	}

    if(params->beacon_csa.head_len == 0 || params->beacon_csa.tail_len == 0 || 
	   params->beacon_after.head_len == 0 || params->beacon_after.tail_len == 0){
		err = -EINVAL;
		goto exit;
	}
	
	csa_beacon = ieee80211_csa_beacon_data_alloc(&params->beacon_csa);
	if(csa_beacon == NULL){
		err = -ENOMEM;
		goto ie_err;
	}
	
	beacon_after = ieee80211_csa_beacon_data_alloc(&params->beacon_after);
	if(beacon_after == NULL){
		err = -ENOMEM;
		goto ie_err;
	}
	
	if(params->beacon_csa.probe_resp && params->beacon_csa.probe_resp_len){
		csa_proberesp = ieee80211_csa_probersp_data_alloc(&params->beacon_csa);
		if(csa_proberesp == NULL){
			err = -ENOMEM;
			goto ie_err;
		}
	}
	...
	csa_mlme->settings = *params;
	csa_mlme->chan     = params->chandef.chan;
	csa_mlme->count    = params->count;
	csa_mlme->block_tx = params->block_tx;
#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(3,16,0))
	if(params->counter_offsets_beacon)
		csa_mlme->counter_offsets_beacon = params->counter_offsets_beacon[0];
	if(params->counter_offsets_presp)
		csa_mlme->counter_offsets_presp  = params->counter_offsets_presp[0];
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3,13,0))
	csa_mlme->counter_offsets_beacon = params->counter_offsets_beacon;
	csa_mlme->counter_offsets_presp  = params->counter_offsets_presp;
#else
	csa_mlme->counter_offsets_beacon = params->counter_offset_beacon;
	csa_mlme->counter_offsets_presp  = params->counter_offset_presp;
#endif
	rcu_assign_pointer(csa_mlme->csa_beacon,csa_beacon);
	rcu_assign_pointer(csa_mlme->beacon_after,beacon_after);
	rcu_assign_pointer(csa_mlme->csa_proberesp,csa_proberesp);
	rcu_assign_pointer(csa_mlme->proberesp_after,proberesp_after);

	csa.cfg80211 = true;
	csa.chan     = params->chandef.chan;
	csa.chantype = cfg80211_get_chandef_type(&params->chandef);
	csa.mactime  = jiffies;
	csa.ie.count = params->count;
	csa.ie.mode  = csa_mlme->block_tx;
	csa.ie.new_ch_num = channel_hw_value(csa.chan);
	
	if(ieee80211_start_csa_work(sdata,&csa)){
		atbm_printk_csa("[%s:%s] start csa fail\n",__func__,sdata->name);
		goto start_err;
	}
...
#ifdef CONFIG_ATBM_SUPPORT_CSA
#if (LINUX_VERSION_IS_GEQ_OR_CPTCFG(3,12,0))||(CONFIG_ATBM_LOW_KERNEL_SUPPORT_CSA == 1)
	.channel_switch = ieee80211_ap_channel_switch,
#endif
#endif
};
