...
#define ENABLE_HE_MLME_FOR_AP 1
...
static bool ieee80211_tx_amsdu(struct sk_buff* skb, struct sta_info* sta)
{
    ...
    bool continue_tx = true;
    bool can_aggr = false;
    /*
     *mucast can not send with amsdu
     */
    if(unlikely(sta == NULL)){
        atbm_printk_limit("%s:sta is NULL\n",__func__);
        goto exit;
    }

    if((info->flags & IEEE80211_TX_ENCAP_AMSDU_MASK) != IEEE80211_TX_ENCAP_AMSDU_MASK){
        atbm_printk_limit("%s:not amsdu mask(%x)\n",__func__,info->flags);
        goto exit;
    }

	tid = skb->priority & 0x07;
    amsdus_list = &sta->amsdu_buf[tid]; 

    can_aggr = (ethertype >= 0x600) && (info->gso == 0) && (skb->len <= 256) && (skb_has_frag_list(skb) == false);

	spin_lock_bh(&amsdus_list->lock);

    head = skb_peek_tail(amsdus_list); 

    if(can_aggr == 0){
        if(head){
            IEEE80211_SKB_CB(head)->flags |= IEEE80211_TX_CTL_AMSDU_END;
        }
        atbm_printk_limit("amsdu can not aggr\n");
        goto exit_bh;
    }

    info->control.amsdu_len.head_len  = skb->len;
    info->control.amsdu_len.total_len = skb->len;
    info->amsdu = 1;

    if(head == NULL || ((IEEE80211_SKB_CB(head)->flags & IEEE80211_TX_CTL_AMSDU_END))){
        if(head)
            atbm_printk_limit("%s:head(%p)\n",__func__,head);
        __atbm_skb_queue_tail(amsdus_list,skb);
        goto exit_bh;
    }

    if((head->len + skb->len > 1300) ||(memcmp(head->data,skb->data,12)) ){
        IEEE80211_SKB_CB(head)->flags |= IEEE80211_TX_CTL_AMSDU_END;
        __atbm_skb_queue_tail(amsdus_list,skb);
        atbm_printk_limit("%s:len(%d)\n",__func__,head->len);
        goto exit_bh;
    };
    ...
    IEEE80211_SKB_CB(head)->control.amsdu_len.total_len += skb->len;

    if(skb_has_frag_list(head)){
        BUG_ON(IEEE80211_SKB_CB(head)->control.amsdu.tail == NULL);
        IEEE80211_SKB_CB(head)->control.amsdu.tail->next = skb;
    }else {
        skb_shinfo(head)->frag_list = skb;
    }
    /*
     * recorder the tail of the frag list
     */
    IEEE80211_SKB_CB(head)->control.amsdu.tail = skb;
    atbm_printk_limit("%s:amsdu(%d)\n",__func__,head->len);
exit_bh:
...
#endif
#ifdef CONFIG_SUPPORT_AMSDU
    if(ieee80211_tx_amsdu(skb,sta) == true){
        drv_tx(local,skb);
    }
#else
	drv_tx(local, skb);
...
static bool ieee80211_ap_can_xmit(struct net_device *dev, struct ieee80211_sub_if_data *sdata)
{
	struct ieee80211_channel_state *chan_state = ieee80211_get_channel_state(sdata->local, sdata);
	struct ieee80211_channel *chan = chan_state->conf.channel;
	bool can_xmit = true;

#if !defined(CONFIG_ATBM_RADAR_DETECT) && !defined(CONFIG_ATBM_5G_PRETEND_2G)
	if ((chan->flags & (
	...
	can_xmit = cfg80211_reg_can_beacon(sdata->local->hw.wiphy,&chandef
...
//	atbm_printk_err("TX monitor[%x][%d][%d]\n",hdr->frame_control,skb->len);
	 ...
	 */
	if(ieee80211_ap_can_xmit(dev, sdata)==false){
		rcu_read_unlock();
		goto fail;
	}
