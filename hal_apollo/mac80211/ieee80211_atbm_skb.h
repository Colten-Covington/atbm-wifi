...
extern int ieee80211_atbm_napi_gro_receive(struct napi_struct *napi, struct sk_buff *skb);
...
#define	atbm_napi_gro_receive(_napi,_skb)						ieee80211_atbm_napi_gro_receive(_napi,_skb)
#define atbm_skb_peek_tail(__list)                              skb_peek_tail(__list)
...
#define atbm_netif_rx_ni(_skb)									netif_rx(_skb)
#else
...
#define	atbm_napi_gro_receive(_napi,_skb)						napi_gro_receive(_napi,_skb)
#define atbm_skb_peek_tail(_list)                               skb_peek_tail(__list)
#if 0
#if (LINUX_VERSION_IS_LESS_AND_NOT_CPTCFG(4, 8, 17))// (LINUX_VERSION_CODE < KERNEL_VERSION(4, 7, 0))
...
static inline void *atbm_skb_put_data(struct sk_buff *skb, const void *data,
				 unsigned int len)
{
