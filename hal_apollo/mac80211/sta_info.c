				...
				u8 *addr, gfp_t gfp)
{
	...
	atbm_ewma_init(&sta->avg_signal_data, 1024, 8);
	...
	for (i = 0; i < STA_TID_NUM; i++) {
		...
		ratelimit_state_init(&sta->ampdu_mlme.trylimit[i],HT_AGG_TX_INTREVAL,HT_AGG_TX_BURST);
	}
    ...
    for (i = 0; i < IEEE80211_QOS_CTL_TID_MASK + 1; i ++){
		atbm_skb_queue_head_init(&sta->amsdu_buf[i]);
    }
#endif
	for (i = 0; i < NUM_RX_DATA_QUEUES; i++){
		...
		for(j = 0;j <  64; j++)
			sta->qos_seq_buff[i][j]  = cpu_to_le16(USHRT_MAX);
	}
...
static int sta_info_insert_non_ibss(struct sta_info *sta) __acquires(RCU)
{
	...
	if (err) {
		if(-1 == err)
		{
			...
			atbm_printk_err("dummy_insert_fcnt:%d",local->dummy_insert_failcnt);
		}
		mutex_unlock(&local->sta_mtx);
		rcu_read_lock();
	atbm_printk_err("%s %d:ERR=%d \n",__func__,__LINE__,err);
...
static inline bool __bss_tim_get(struct ieee80211_if_ap *bss, u16 id)
{
	/*
	 * This format has been mandated by the IEEE specifications,
	 * so this line may not be changed to use the test_bit() format.
	 */
	return (bss->tim[id / 8] & (1 << (id % 8))) ? true:false;
}
...
void sta_info_recalc_tim(struct sta_info *sta)
{
	...
	if(indicate_tim != __bss_tim_get(bss,sta->sta.aid)){

		atbm_printk_err("sta[%pM] update tim[%d]\n",sta->sta.addr,indicate_tim);
...
#ifdef CONFIG_SUPPORT_AMSDU
    for (i = 0; i < IEEE80211_QOS_CTL_TID_MASK + 1; i ++){
	    atbm_skb_queue_purge(&sta->amsdu_buf[i]);
    }
#endif
#ifdef CONFIG_MAC80211_ATBM_MESH
	if (ieee80211_vif_is_mesh(&sdata->vif))
		...
		while ((skb = __atbm_skb_dequeue(&frames))) {
			...
			if(info->flags & IEEE80211_TX_CTL_8023){
				if (more_data || !atbm_skb_queue_empty(&frames))
					info->flags |= IEEE80211_TX_CTL_MORE;
				else
					info->flags &= ~IEEE80211_TX_CTL_MORE;

				if(info->flags & IEEE80211_TX_CTL_QOS){
					if (reason == IEEE80211_FRAME_RELEASE_UAPSD && atbm_skb_queue_empty(&frames))
						info->flags |= IEEE80211_TX_CTL_EOSP;

					tids |= BIT(skb->priority & IEEE80211_QOS_CTL_TAG1D_MASK);
				}else {
					tids |= BIT(0);
				}
			}else {
...
bool ieee80211_sta_support_40w(struct ieee80211_sta *pubsta)
{
	...
	return test_sta_flag(sta,WLAN_STA_40M_CH)&&(!test_sta_flag(sta,WLAN_STA_40M_CH_SEND_20M));
}
static void ieee80211_sta_hdr_free(struct rcu_head *h)
{
	struct ieee80211_sta_hdr *sta_hdr =
		container_of(h, struct ieee80211_sta_hdr, rcu_head);
...
{
	struct ieee80211_sta_hdr build = {}, *sta_hdr = NULL, *old;
	//struct ieee80211_local *local = sta->local;
	struct ieee80211_sub_if_data *sdata = sta->sdata;
	...
	build.key = rcu_dereference(sta->ptk);

	if(build.key == NULL){
		build.key = rcu_dereference(sdata->default_unicast_key);
	}
	if (build.key) {
		...
		case WLAN_CIPHER_SUITE_CCMP:
			...
			build.icv_len 	  += build.key->conf.icv_len;
			break;
		default:
	...
	//sta_hdr = kmemdup(&build, sizeof(build), GFP_ATOMIC);
	sta_hdr = atbm_kzalloc(sizeof(struct ieee80211_sta_hdr), GFP_ATOMIC);
	/* if the kmemdup fails, continue w/o fast_tx */
	if (!sta_hdr)
		goto out;

	memcpy(sta_hdr,&build,sizeof(struct ieee80211_sta_hdr));
	atbm_printk_err("sta[%pM] alloc txhdr\n",sta->sta.addr);	
 out:
	/* we might have raced against another call to this function */
	old = rcu_dereference_protected(sta->sta_hdr,
					lockdep_is_held(&sta->lock));
	rcu_assign_pointer(sta->sta_hdr, sta_hdr);
	if (old)
		call_rcu(&old->rcu_head, ieee80211_sta_hdr_free);
...
void ieee80211_clear_sta_hdr(struct sta_info *sta)
{
	...
	spin_lock_bh(&sta->lock);
	sta_hdr = rcu_dereference_protected(sta->sta_hdr,
					    lockdep_is_held(&sta->lock));
	RCU_INIT_POINTER(sta->sta_hdr, NULL);
	spin_unlock_bh(&sta->lock);

	if (sta_hdr)
		call_rcu(&sta_hdr->rcu_head, ieee80211_sta_hdr_free);
}
...
void __ieee80211_recalc_dcm(struct ieee80211_local *local)
{
	struct sta_info *sta, *tmp;
    int n_stas = 0;
    bool he_sta = false;
    bool new = true;
    bool old = local->hw.dcm_enable;

    list_for_each_entry_safe(sta, tmp, &local->sta_list, list){
        n_stas ++;

        if(sta->sta.he_cap.has_he == false){
            continue;
        }

        he_sta = true;

		if (((sta->sta.he_cap.he_cap_elem.phy_cap_info[3] >> 3) & 0x03) == 0){
            new = false;
            break;
        }
    ...
    if((n_stas == 0) || (he_sta == false)){
        new = false;
    }

    if(new != old){
        /*
         * change dcm set to fw
         */
        atbm_printk_always("%s:dcm(%d)\n",__func__,new);
        local->hw.dcm_enable = new;
        if(drv_do_change_dcm(local) != 0){
            atbm_printk_err("%s:hw change dcm err(%d)\n",__func__,new);
            local->hw.dcm_enable = old;
        }
    }
}
void ieee80211_recalc_dcm(struct ieee80211_local *local)
{
	mutex_lock(&local->sta_mtx);
    __ieee80211_recalc_dcm(local);
    mutex_unlock(&local->sta_mtx);
}
