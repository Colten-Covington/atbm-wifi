...
static int ieee80211_ble_ioctl_open(struct inode *inode, struct file *filp)
{
	...
	local = ble_to_local(ble_cdev->pdev);
	hw_priv=local->hw.priv;
	if(hw_priv->loader_ble == 0){
		atbm_printk_err("ieee80211_ble_ioctl_open : not support ble\n");
		goto err_ble;
	}
...
static int ieee80211_ble_ioctl_release(struct inode *inode, struct file *filp)
{
	...
	atbm_printk_always("ioctl_release(%p)(%p),task(%s)\n",ble_file->dev,ble_cdev,comm);
...
#ifdef  CONFIG_BLE_RECV_DATA_HIGHT_LAYER
#define ETH_P_CUSTOM_BLE 0x88ff

int ieee80211_add_ble_8023_header(struct sk_buff *skb, const char *addr,
			   enum nl80211_iftype iftype)
{
  ...
  ehdr->h_proto = htons(ETH_P_CUSTOM_BLE);
  return 0;
}

int ieee80211_send_ble_L2_2_hight_layer(struct ieee80211_sub_if_data *sdata,
											  struct sk_buff *skb,struct net_device *dev)
{
  int res = -1;
  struct sk_buff *xmit_skb = skb;
 // xmit_skb = atbm_skb_clone(skb, GFP_ATOMIC);
  if (!xmit_skb && net_ratelimit())
	  atbm_printk_err( "ieee80211_send_L2_2_hight_layer: failed to clone "
			 "multicast frame\n");
  res = ieee80211_add_ble_8023_header(xmit_skb, sdata->vif.addr, sdata->vif.type);
  if (res < 0){
  ...
  //  atbm_printk_err("[atbm_netif_receive_skb] protocol ==> %x \n ",eth_type_trans(xmit_skb, dev));
	  ...
	  xmit_skb->protocol = htons(ETH_P_CUSTOM_BLE);
	  //xmit_skb->pkt_type = PACKET_OTHERHOST;
	  memset(xmit_skb->cb, 0, sizeof(xmit_skb->cb));
	  if(xmit_skb->len > 0){
		  res = atbm_netif_rx(xmit_skb);
		  if(res < 0){
...
static int ieee80211_ble_file_submit_skb(struct ieee80211_ble_file *ble_file,struct sk_buff *skb)
{
...
#ifdef  CONFIG_BLE_RECV_DATA_HIGHT_LAYER	
	struct ieee80211_local *local = NULL;//ble_cdev->pdev->dev.platform_data;
	struct ieee80211_sub_if_data *sdata = NULL;
	int i = 0,sdata_flag=0;
	char * ptr;
 
	if(!ble_file || !skb){
		atbm_printk_err("ieee80211_ble_file_submit_skb ble_file or skb is NULL\n");
		goto exit;
	}
	local = (struct ieee80211_local *)ble_file->pdev->dev.platform_data;
	if(!local){
		atbm_printk_err("ieee80211_ble_file_submit_skb local is NULL\n");
		goto exit;
	}	
	...
	if(sdata_flag){
	#if 0
		atbm_printk_err("skb:%x \n",skb);
		ptr = (char *)skb;
		for(i = 0;i < 64 ; i++){
			atbm_printk_err("%x ",ptr[i]);
		}
		atbm_printk_err("\nskb->data:%x\n",skb->data);
		ptr = skb->data;
		for(i = 0;i < 64 ; i++){
	...
	#endif
		ret = ieee80211_send_ble_L2_2_hight_layer(sdata,skb,sdata->dev);
	}else
		ret = -1;
exit:
#else
...
static int ieee80211_ble_bluez_open(struct hci_dev *hdev)
{
	...
	hw_priv=local->hw.priv;
	if(hw_priv->loader_ble == 0){
		atbm_printk_err("ieee80211_ble_bluez_open : not support ble\n");
		goto err_cdev;
	}
...
#endif
int ieee80211_ble_bluez_send_frame(
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 12, 0))
			struct hci_dev *hdev,
#endif
			struct sk_buff *skb)
					...
					char __user *buff, size_t len)
{
	...
	if(cb->nr != 0){
		atbm_printk_err("ieee8021_ble_operations_default_read:nr != 0 \n");
		return -1;
	}
