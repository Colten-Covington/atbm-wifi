					...
					int idx, bool uni, bool multi)
{
	...
	if (uni){
		...
		ieee80211_assign_sta_hdr_iface(sdata);
	}
	if (multi)
	...
	/*
	*set default key id to driver
	*/
	if (key && (key->conf.cipher == WLAN_CIPHER_SUITE_WEP40 || key->conf.cipher == WLAN_CIPHER_SUITE_WEP104)){
		drv_do_set_defualt_wep_key(sdata->local, sdata, &key->conf);
	}
}
					  ...
					  size_t seq_len, const u8 *seq)
{
	...
	case WLAN_CIPHER_SUITE_TKIP:
		...
		key->conf.icv_len = TKIP_ICV_LEN+8/*mic*/;
		if (seq) {
