...
enum ieee80211_sta_info_flags {
	...
	WLAN_STA_HE,
    WLAN_STA_VHT,
	WLAN_STA_HE_TWT_DOZE,
	WLAN_STA_HE_TWT_DELIVER,
};
...
#define HT_AGG_STATE_QUEUE_STOP		6

#define HT_AGG_TX_INTREVAL			200*HZ
#define HT_AGG_TX_BURST				2
/**
...
struct sta_ampdu_mlme {
	...
	struct ratelimit_state trylimit[STA_TID_NUM];
};
/*
*twt state and infor
*/
struct sta_twt_mlme{
	spinlock_t lock;
	enum ieee80211_sta_twt_state state;
...
struct sta_info {
	...
	struct atbm_ewma avg_signal_data;

	/* Plus 1 for non-QoS frames */
	__le16 last_seq_ctrl[NUM_RX_DATA_QUEUES + 1];
	__le16 last_frame_ctrl[NUM_RX_DATA_QUEUES + 1];
	u16    qos_seq_buff[NUM_RX_DATA_QUEUES + 1][64];
...
#endif
	enum atbm_ieee80211_sta_rx_bandwidth cur_max_bandwidth;
	...
	u8 	mic_len;
	u8  handshake_state;
	struct ieee80211_sta_hdr __rcu *sta_hdr;
	struct sta_twt_mlme twt;
#ifdef CONFIG_SUPPORT_AMSDU
    struct sk_buff_head amsdu_buf[IEEE80211_QOS_CTL_TID_MASK + 1];
#endif
	/* keep last! */
...
static inline bool sta_support_aggr(struct sta_info *sta)
{
	return sta->sta.ht_cap.ht_supported || sta->sta.he_cap.has_he;
}
