...
static int HeApComp = 1;
module_param(HeApComp, int, 0644);
...
static void ieee80211_sdata_gro_setup(struct ieee80211_sub_if_data *sdata)
{
	...
	if(sdata->vif.type == NL80211_IFTYPE_AP || sdata->vif.type == NL80211_IFTYPE_STATION){
		...
		netif_napi_add(ndev,&sdata->napi_rx,ieee80211_rx_napi_poll);
#else
...
//	mutex_lock(&local->iflist_mtx);
	...
	ret = copy_to_user(rq->ifr_data,(u8*)(&vendor_msg),sizeof(vendor_msg));
	if (ret)
	{
		atbm_printk_err("error, netdev copy_to_user err = %d\n",ret);
		ret = -EFAULT;
		goto exit;
	}
				...
				struct sta_info *sta,struct atbm_ieee80211_mgmt *mgmt,int len)
{
	...
	case WLAN_HT_ACTION_NOTIFY_CHANWIDTH:
		if(mgmt->u.action.u.notify_chan_width.chan_width){
            ...
            atbm_printk_err("%s:only can send 20M\n",__func__);
...
static void ieee80211_iface_work(struct atbm_work_struct *work)
{
	...
	while ((skb = atbm_skb_dequeue(&sdata->skb_queue))) {
		...
		if(0){}
#ifdef CONFIG_ATBM_SUPPORT_CSA
        else if(skb->pkt_type == IEEE80211_SDATA_QUEUE_CSA){
            ieee80211_sdata_csa_completed_notify(sdata);
        }
#endif
#ifdef CONFIG_ATBM_SUPPORT_NET_TIMEOUT
...
#endif
		else if (skb->pkt_type == IEEE80211_SDATA_QUEUE_ACTION) {
			int len = skb->len;
        ...
        else if(skb->pkt_type == IEEE80211_SDATA_QUEUE_MLME){
            ieee80211_sdata_queue_mlme_process(sdata,skb);
        }
		else {
			switch (sdata->vif.type) {
			case NL80211_IFTYPE_AP:
...
void ieee80211_setup_ap_he_data(struct ieee80211_sub_if_data *sdata)
{
	...
	if((hw_priv->hw->he_cap_2g4 == NULL)&&(hw_priv->hw->he_cap_5g == NULL))
	{
		atbm_printk_always("Wifi6 not support on Oceanus variant\n");
        return;
	}
	...
	if(mlme->he_cap_len){
        struct atbm_ieee80211_he_cap_elem *he_cap_ie = (struct atbm_ieee80211_he_cap_elem *)(&mlme->he_cap[3]);
        he_cap_ie->phy_cap_info[0] &= ~ATBM_IEEE80211_HE_PHY_CAP0_CHANNEL_WIDTH_SET_40MHZ_80MHZ_IN_5G;
...
{	
	...
	if(skb){
		...
		if(mlme && mlme->he_cap && mlme->he_op){
		...
		if(he_skb){
			...
			if(sta){
                ...
                drv_sta_remove(sdata->local,sdata,&sta->sta);
                drv_sta_add(sdata->local,sdata,&sta->sta);
...
static bool ieee80211_he_ap_check_valid_hecap(struct ieee80211_sub_if_data *sdata,const u8 *he_cap,u8 he_cap_len)
{
	...
	sband = sdata->local->hw.wiphy->bands[chan_state->oper_channel->band];
	own_cap = atbm_ieee80211_get_he_iftype_cap(&sdata->local->hw,sband,ieee80211_vif_type_p2p(&sdata->vif),sdata);
...
#endif
	ieee80211_scan_policy_init(sdata);
	/*
...
u32 __ieee80211_recalc_idle(struct ieee80211_local *local)
{
	...
	list_for_each_entry(sdata, &local->interfaces, list) {
		...
		if(sdata->vif.type == NL80211_IFTYPE_STATION && sdata->u.mgd.conn.state != IEEE80211_CONN_IDLE){
			authening = 1;
