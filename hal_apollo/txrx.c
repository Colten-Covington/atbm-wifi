...
void atbm_build_wsm_hdr(struct wsm_tx_encap *wsm_encap)
{
	...
	if (wsm_encap->hw_key) {
		...
		if (wsm_encap->hw_key->cipher == WLAN_CIPHER_SUITE_AES_CMAC) {
			atbm_printk_debug("11w WLAN_CIPHER_SUITE_AES_CMAC\n");
			wsm->hdr.enc_hdr.EncFlags |= __cpu_to_le32(ENC_FORCE_PROTECT_MGMT);
		}
	}
    //atbm_printk_err("%s:%d\n",__func__,(u8*)wsm + sizeof(struct wsm_tx) - (u8*)(&wsm->hdr.enc_hdr.EncFlags));
...
{	
    ...
    u8 rate_mode = tx_info->control.txrc_out.tx_rate_sets & RATE_MODE_MASK;
	...
	wsm->maxTxRate   = tx_info->control.txrc_out.tx_max_rate;	
	wsm->txRateSets  = __cpu_to_le32(tx_info->control.txrc_out.tx_rate_sets);
   		
	wsm->htTxParameters &= ~(__cpu_to_le32(WSM_NEED_TX_CONFIRM));
	if (tx_info->flags & IEEE80211_TX_CTL_REQ_TX_STATUS) {
		wsm->htTxParameters |= __cpu_to_le32(WSM_NEED_TX_CONFIRM);
	}
    if (tx_info->flags & IEEE80211_TX_FIXED_RATE){
        wsm->htTxParameters |= __cpu_to_le32(WSM_HT_TX_USE_FIXED_RATE);
    }
  ...
  if(((tx_info->control.txrc_out.force_policyid & 0x70) > 0))
  	{
  	    wsm->flags &= 0x8f;  //clear bit[4:6]
		wsm->flags |= (tx_info->control.txrc_out.force_policyid & 0x70) ;  //set bit[4:6]
  	}
	
	/*
	*try to set WSM_HT_AGGR_EN and WSM_HT_TX_WIDTH_40M
	*/
	if ((RATE_HT ==  rate_mode) || 
        (RATE_HE ==  rate_mode) ||
        (RATE_VHT == rate_mode)
       ){
	...
	if((RATE_HT ==  rate_mode) || (RATE_VHT == rate_mode)) {
		...
		if(tx_info->control.txrc_out.tx_rc_flag & ATBM_IEEE80211_TX_RC_SHORT_GI){	

			if(wsm->htTxParameters &__cpu_to_le32(WSM_HT_TX_WIDTH_40M)){
					if(tx_info->control.txrc_out.tx_rc_flag & ATBM_IEEE80211_TX_RC_SHORT_GI_40M)
					{
						wsm->htTxParameters |=	__cpu_to_le32(WSM_HT_TX_SGI);
						atbm_printk_rc("HT SHORT_GI_40M\n");
					}
			}else{  //20M
					if(tx_info->control.txrc_out.tx_rc_flag & ATBM_IEEE80211_TX_RC_SHORT_GI_20M)
					{
						wsm->htTxParameters |=	__cpu_to_le32(WSM_HT_TX_SGI);
		...
		if(tx_info->control.txrc_out.tx_rc_flag & ATBM_IEEE80211_TX_RC_40M_TO_20M){  
            if(tx_info->control.txrc_out.tx_rc_flag & ATBM_IEEE80211_TX_RC_SHORT_GI_20M)
            {
                wsm->htTxParameters |= __cpu_to_le32(WSM_HT_TX_SGI);
            }
            ...
            atbm_printk_limit("%s: force to 20M\n",__func__);
			wsm->htTxParameters  &=	~__cpu_to_le32(WSM_HT_TX_WIDTH_40M);
		}			
		
        // ht_ldpc = 1;
		//0 bcc , 1 ldpc, 2 auto
		if((ht_ldpc==1)||(tx_info->control.txrc_out.tx_rc_flag & ATBM_IEEE80211_TX_RC_HT_LDPC)){
	...
	if(RATE_HE <=  rate_mode){
        ...
        if((tx_info->control.txrc_out.tx_rc_flag & ATBM_IEEE80211_TX_RC_HE_SUPPORT_DCM)){
                wsm->heTxParameters |= __cpu_to_le32(WSM_TX_HE_SUPPORT_DCM); 
        }        
        if((tx_info->control.txrc_out.tx_rc_flag & ATBM_IEEE80211_TX_RC_HE_DCM)){
		...
		if((tx_info->control.txrc_out.tx_rc_flag & ATBM_IEEE80211_TX_RC_HE_LDPC)){
	...
	if(((tx_info->control.txrc_out.force_policyid & HE_MODE_DOWN_TO_11B) > 0))
	   {
		 wsm->heTxParameters |= __cpu_to_le32(WSM_TX_HE_DOWN_TO_11B);
	...
	/*
	  ...
	  //manual force bandwidth
		//if(he_not_40M == 0 ||ht_vht_not_40M == 0)
		if(he_not_40M == 0)
		{
			wsm->htTxParameters |= __cpu_to_le32(WSM_HT_TX_WIDTH_40M);
		}

		//if(he_not_40M == 1 && ht_vht_not_40M==1)
		else if(he_not_40M == 1)
		{
			wsm->htTxParameters &= ~__cpu_to_le32(WSM_HT_TX_WIDTH_40M);
		}
...
void atbm_tx(struct ieee80211_hw* dev, struct sk_buff* skb)
{
	...
	if (atbm_tcpack_offload_tx_check(dev->priv,skb)) {
		return ;
	} 
...
//		atbm_printk_err("passive mode ,scan\n");
	...
	case ATBM_APOLLO_JOIN_STATUS_STA:
		...
		if(WSM_RX_STATUS_ENCRYPTION(arg->flags)){
            ...
            if((arg->flags & WSM_RX_STATUS_GROUP_KEY) && (arg->status == 0)){
                ...
                hdr->flag |= RX_FLAG_SKIP_KEY | RX_FLAG_DECRYPTED;
            }
#endif
		}else {
			if(ieee80211_has_protected(frame->frame_control)){
				atbm_printk_err("[RX]:fc[%x],da[%pM],sa[%pM],bssid[%pM] need sw decrypted?\n",mgmt->frame_control,da,sa,bssid);
				
	//			frame_hexdump("recv skb:", skb->data, skb->len);
