...
extern u8 MCS_LUT_20M_Modify[]; 
extern s8 MCS_LUT_20M_delta_mode[];						
extern s8 MCS_LUT_20M_delta_bw[];

extern const s8 MCS_LUT_40M[]; 
extern s8 MCS_LUT_40M_Modify[];
extern s8 MCS_LUT_40M_delta_mode[]; 
extern s8 MCS_LUT_40M_delta_bw[]; 

extern const char *rate_name[]; 

//-----------cronus
extern const s8 MCS_LUT_20M_Cronus[];
extern u8 MCS_LUT_20M_Modify_Cronus[]; 
extern s8 MCS_LUT_20M_delta_mode_Cronus[]; 						
extern s8 MCS_LUT_20M_delta_bw_Cronus[];

extern const s8 MCS_LUT_40M_Cronus[]; 
extern s8 MCS_LUT_40M_Modify_Cronus[];
extern s8 MCS_LUT_40M_delta_mode_Cronus[]; 
extern s8 MCS_LUT_40M_delta_bw_Cronus[]; 
	...
	*/
	for(index = 0;index<table_size;index++){
		...
		if(strlen(string_table[index].string) > ATBM_STRING_MAX_LEN){
			//atbm_printk_err("%s %d ,ERROR !!! strlen(string_table[index].string) = %ld > 16\n",
...
//extern bool atbm_sdio_channel_test(struct atbm_common *hw_priv);

static bool atbm_module_attr_example(struct atbm_common *hw_priv,struct atbm_store_cmd_code *cmd_store,const char *buf,int len)
{	
//	atbm_sdio_channel_test(hw_priv);
#if 0
	u8 index = 0;
...
///extern bool atbm_sdio_channel_test_thread(struct atbm_common *hw_priv);
//#define TX_THREAD_TEST
#ifdef TX_THREAD_TEST
static void *bh = NULL;
static int atbm_module_tx_thread_test(void *priv)
{
	while(kthread_should_stop() == 0){
//		atbm_sdio_channel_test_thread(priv);
		msleep(110);
	}
...
{
#ifdef TX_THREAD_TEST
	bh = kthread_create(atbm_module_tx_thread_test,hw_priv,"module_fs");
														...
														const char *buf,int len)
{
	...
	struct best_ch_scan_result_def chan[64];
	int num_channels;
//	u8 ignor_channels[2]; // = {13,14};
	...
	*/
	...
	for(i = 0; i < select->channels.n_channels; i++){
		chan[i].chan = select->channels.channels[i];
	}
	num_channels = select->channels.n_channels;
	req.channels = select->channels.channels;
	req.n_channels = select->channels.n_channels;
	...
	*get results
	*/
	if(atbm_internal_channel_auto_select_results(sdata,&results, chan, num_channels) == false){
	...
	for(i = 0;i<num_channels;i++){
		struct ieee80211_channel *ch = ieee8011_chnum_to_channel(hw_priv->hw,chan[i].chan);
		atbm_module_show_put(&cmd_store->echo,"channel[%d] ap_count[%d] ratio[%d] special_freq[%s]"ATBM_LINEF_STR,chan[i].chan,
							chan[i].channel_ap_num,chan[i].busy_ratio,
...
static void atbm_store_cmd_echo_ready(struct atbm_store_cmd_code *cmd_store)
{
	...
	atbm_printk_always( "%s cmd add tail\n",cmd_store->label);
...
static void atbm_module_cmd_init(void)
{
	...
	for(index = 0;index<ARRAY_SIZE(cmd_code_buff);index++){
		...
		if(strlen(cmd_code_buff[index].label) > ATBM_CMD_MAX_STRING_LEN){
			//atbm_printk_err("%s %d ,ERROR !!! strlen(%s)=%ld > 32\n",
	...
	backup_show_size = 0;
	atbm_printk_always("cmd_init next %p %p\n", atbm_cmd_echo_ready_list.next, atbm_cmd_echo_ready_list.prev);
}

static void atbm_module_cmd_exit(void)
{
	atbm_printk_always("cmd_exit next %p %p\n", atbm_cmd_echo_ready_list.next, atbm_cmd_echo_ready_list.prev);
	...
	while (!list_empty(&atbm_cmd_echo_ready_list)) {
		...
		atbm_printk_always( "cmd_codev %p\n",cmd_code);
		if(cmd_code->show_cmd)
			cmd_code->show_cmd(atbm_hw_priv_dereference(),cmd_code,NULL,false);
		atbm_store_cmd_echo_done(cmd_code);
		atbm_store_cmd_deinit_show(&cmd_code->echo);		
...
extern atbm_caps_display caps_display[];
static void atbm_module_firmware_caps_show(struct atbm_module_show *show_buff,struct atbm_common *hw_priv)
{
	...
	atbm_module_show_put(show_buff,"[%s]\n" ,hw_priv->fw_lable); 
	while(caps_ptr->name != NULL){
		atbm_module_show_put(show_buff,"%s  [%d]\n" ,caps_ptr->name, WSM_CAP(caps_ptr->wsm_cap_flag)); 
		caps_ptr++;
	}
#ifdef ATBM_PRODUCT_TEST_USE_FEATURE_ID
		atbm_module_show_put(show_buff,"CONFIG_PRODUCT_TEST_USE_FEATURE_ID [1]\n");
#else
		atbm_module_show_put(show_buff,"CONFIG_PRODUCT_TEST_USE_FEATURE_ID [0]\n");
#endif
#ifdef CONFIG_ATBM_PRODUCT_TEST_USE_GOLDEN_LED
		atbm_module_show_put(show_buff,"CONFIG_PRODUCT_TEST_USE_GOLDEN_LED [1]\n");
#else
		atbm_module_show_put(show_buff,"CONFIG_PRODUCT_TEST_USE_GOLDEN_LED [0]\n");
#endif
	...
	atbm_module_show_put(show_buff,"WOW         [%d]\n" ,WSM_CAP(CAPABILITIES_WOW));
	...
	atbm_module_show_put(show_buff,"CHIP FLAG   [%d]\n" ,WSM_CAP(CAPABILITIES_CHIP_FLAG));  
	atbm_module_show_put(show_buff,"TXCAL       [%d]\n" ,WSM_CAP(CAPABILITIES_TXCAL));
	atbm_module_show_put(show_buff,"CTS_BUG     [%d]\n" ,WSM_CAP(CAPABILITIES_NAV_ENABLE));
	atbm_module_show_put(show_buff,"HIF_TRACE   [%d]\n" ,WSM_CAP(CAPABILITIES_HIF_TRACE));  
	atbm_module_show_put(show_buff,"PM_ACTIVE   [%d]\n" ,WSM_CAP(CAPABILITIES_FORCE_PM_ACTIVE));
	...
	atbm_module_show_put(show_buff,"CAPABILITIES_BLE   [%d]\n" ,WSM_CAP(CAPABILITIES_BLE));  
	atbm_module_show_put(show_buff,"CAPABILITIES_40M   [%d]\n" ,WSM_CAP(CAPABILITIES_40M));  
	atbm_module_show_put(show_buff,"CAPABILITIES_LDPC  [%d]\n" ,WSM_CAP(CAPABILITIES_LDPC));  
	...
	atbm_module_show_put(show_buff,"RXCAL       [%d]\n" ,WSM_CAP(CAPABILITIES_RXCAL));
	atbm_module_show_put(show_buff,"80211_TO_8023 		  [%d]\n" ,WSM_CAP(CAPABILITIES_80211_TO_8023));
	atbm_module_show_put(show_buff,"USB_RECOVERY_BUG      [%d]\n" ,WSM_CAP(CAPABILITIES_USB_RECOVERY_BUG)); 	
	atbm_module_show_put(show_buff,"WFA     			  [%d]\n" ,WSM_CAP(CAPABILITIES_WFA));
	atbm_module_show_put(show_buff,"EAP_GROUP_REKEY       [%d]\n" ,WSM_CAP(CAPABILITIES_EAP_GROUP_REKEY));
...
#else
//	if()
	if((hw_priv->chip_version == CRONUS_NO_HT40) || 
			     ...
			     struct kobj_attribute *attr, char *buf)
{
	...
	atbm_module_show_put(&sys_show,LIGHT"DriverVer   [%d] BUILD_TIME[%s]"NORMAL ENTER ,DRIVER_VER,buildtime);
...
static ssize_t atbm_module_show_cfg_power(struct kobject *kobj,
			 struct kobj_attribute *attr, char *buf)
 {
	 ...
	 atbm_show_init(&sys_show,buf);
	 atbm_printk_err("sys_show.count = %d \n",sys_show.show_size);
	 atbm_module_muxlock();
	...
	if(atbm_hw_priv_chip_id() == HW_CHIP_VERSION_Cronus)
		total_len = atbm_cronus_internat_cmd_get_cfg_txpower(hw_priv,&results);
	else
		total_len = atbm_internat_cmd_get_cfg_txpower(hw_priv,&results);
	if(total_len > 0)
		atbm_module_show_put(&sys_show,results);
	else
		atbm_module_show_put(&sys_show,"atbm_module_show_cfg_power:get faile!\n");
...
static ssize_t atbm_module_show_txpower(struct kobject *kobj,
			 struct kobj_attribute *attr, char *buf)
{
	 struct atbm_module_show sys_show;
	char * results = NULL;
	int len = 0;
	 ...
	 atbm_module_muxlock();
	if(atbm_hw_priv_chip_id() == HW_CHIP_VERSION_Cronus)
		len = atbm_cronus_internat_cmd_get_txpower(NULL,&results);
	else
		len = atbm_internat_cmd_get_txpower(NULL,&results);
	if(len > 0)
		atbm_module_show_put(&sys_show,results);
	else
		atbm_module_show_put(&sys_show,"get tx power fail!");
...
static struct kobj_attribute atbm_module_get_txpower = __ATTR(atbm_get_txpower,  0444,atbm_module_show_txpower, NULL);
