...
//extern int atbm_get_work_channel(struct ieee80211_sub_if_data *sdata);
...
int __ieee80211_set_channel(struct wiphy *wiphy,
				 struct net_device *netdev,
				 struct ieee80211_channel *chan,
				 enum nl80211_channel_type channel_type);
...
extern u32 atbm_get_chip_version(struct atbm_common *hw_priv);
...
static int atbm_dev_atbm_get_work_channel(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_set_best_channel_scan(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_set_best_channel_scan2(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_set_best_channel_scan3(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_set_best_channel_scan_optional(struct net_device *dev, struct altm_wext_msg *msg);
...
static int atbm_dev_get_snr(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_get_noise_level(struct net_device *dev, struct altm_wext_msg *msg);
...
#endif
extern char * atbm_get_chip_type(void);
...
#endif
static int atbm_dev_send_special_probe_req(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_get_tx_pkt_status(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_set_beacon_loss_cnt(struct net_device *dev, struct altm_wext_msg *msg);
#ifdef ATBM_USE_FASTLINK
static int atbm_dev_reset_fastlink(struct net_device *dev, struct altm_wext_msg *msg);
#endif
static int atbm_dev_set_ampdu(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_set_single_scan_time(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_get_cca_threshold(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_set_cca_threshold(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_get_rts_threshold(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_set_rts_threshold(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_get_txpower_table(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_set_temp_dcxo_table(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_get_temprature(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_ioctl_set_txpower_single(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_set_scan_channel_limit(struct net_device *dev, struct altm_wext_msg *msg);
...
dev_ioctl_cmd_t dev_cmd[]={
	...
	{ATBM_DEV_IO_GET_WORK_CHANNEL,atbm_dev_atbm_get_work_channel,
...
#endif	
	{ATBM_DEV_IO_SET_BEST_CHANNEL_SCAN2,atbm_dev_set_best_channel_scan2,"best channel scan,static other channel same ssid"},
	{ATBM_DEV_IO_SET_BEST_CHANNEL_SCAN3,atbm_dev_set_best_channel_scan3,"best channel scan without conn ap"},
	{ATBM_DEV_IO_SEND_PROBE_REQ,atbm_dev_send_special_probe_req,"ATBM_DEV_IO_SEND_PROBE_REQ = 53"},
	{ATBM_DEV_IO_GET_TX_PKT_STATUS,atbm_dev_get_tx_pkt_status,"get tx packet num and tx fail packet num!"},
	{ATBM_DEV_IO_SET_BEACON_LOSS_CNT,atbm_dev_set_beacon_loss_cnt,"sta mode set beacon loss conter "},
#ifdef ATBM_USE_FASTLINK
	{ATBM_DEV_IO_RESET_FASTLINK,atbm_dev_reset_fastlink,"ATBM_DEV_IO_RESET_FASTLINK"},
#endif
	{ATBM_DEV_IO_SET_AMPDU,atbm_dev_set_ampdu,"ATBM_DEV_IO_SET_AMPDU"},
	{ATBM_DEV_IO_SET_SINGLE_SCAN_TIME,atbm_dev_set_single_scan_time,"ATBM_DEV_IO_SET_SINGLE_SCAN_TIME"},
	{ATBM_DEV_IO_GET_NOISE_LEVEL,atbm_dev_get_noise_level,"ATBM_DEV_IO_GET_NOISE_LEVEL"},
	{ATBM_DEV_IO_GET_SNR,atbm_dev_get_snr,"ATBM_DEV_IO_GET_SNR"},
	{ATBM_DEV_IO_GET_CCA_THRESHOLD,atbm_dev_get_cca_threshold,"ATBM_DEV_IO_GET_CCA_THRESHOLD = 61"},
	{ATBM_DEV_IO_SET_CCA_THRESHOLD,atbm_dev_set_cca_threshold,"ATBM_DEV_IO_SET_CCA_THRESHOLD = 62"},
	{ATBM_DEV_IO_GET_RTS_THRESHOLD,atbm_dev_get_rts_threshold,"ATBM_DEV_IO_GET_RTS_THRESHOLD = 63"},
	{ATBM_DEV_IO_SET_RTS_THRESHOLD,atbm_dev_set_rts_threshold,"ATBM_DEV_IO_SET_RTS_THRESHOLD = 64"},
//	{ATBM_DEV_IO_GET_AP_INFO_NEW,atbm_dev_get_ap_info,
//				"ATBM_DEV_IO_GET_AP_INFO = 64 : sta mode get connect ap information "},
	{ATBM_DEV_IO_GET_STA_INFO_NEW,atbm_dev_get_sta_info_new,
				"ATBM_DEV_IO_GET_STA_INFO = 49:ap mode get connect sta information"},
	{ATBM_DEV_IO_GET_TX_POWER_TABLE,atbm_dev_get_txpower_table,"ATBM_DEV_IO_GET_TX_POWER_TABLE = 65"},
	{ATBM_DEV_IO_SET_TEMP_DCXO_TABLE,atbm_dev_set_temp_dcxo_table,"ATBM_DEV_IO_SET_TEMP_DCXO_TABLE = 66 "},
	{ATBM_DEV_IO_GET_CHIP_TEMPATURE,atbm_dev_get_temprature,"ATBM_DEV_IO_GET_CHIP_TEMPATURE = 67"},
	{ATBM_DEV_IO_SET_TXPWR_SINGLE,atbm_dev_ioctl_set_txpower_single,"ATBM_DEV_IO_SET_TXPWR_SINGLE = 68"},
	{ATBM_DEV_IO_SET_BEST_CHANNEL_SCAN_OPTIONAL,atbm_dev_set_best_channel_scan_optional,"ATBM_DEV_IO_SET_BEST_CHANNEL_SCAN_OPTIONAL = 69"},
	{ATBM_DEV_IO_SET_SCAN_CHANNEL_LIMIT,atbm_dev_set_scan_channel_limit,"ATBM_DEV_IO_SET_SCAN_CHANNEL_LIMIT = 70"},
	{-1,NULL,NULL},
...
int atbm_dev_set_scan_channel_limit(struct net_device *dev, struct altm_wext_msg *msg)
{
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	struct ieee80211_local *local = sdata->local;

 	struct atbm_scan_chan_limit ch_ilmit;

	char *ptr = NULL;

	
	ch_ilmit.channel_2_4G_start = msg->externData[0];
	ch_ilmit.channel_2_4G_end = msg->externData[1];
	ptr = &msg->externData[2];
	ch_ilmit.channel_5G_start = ptr[0];
	ch_ilmit.channel_5G_end = ptr[1];

	if(ieee80211_set_scan_channel_limit(local,&ch_ilmit) < 0){
		atbm_printk_err("%s : set channel fail!\n",__func__);
		return -1;
	}
	return 0;	
}
	...
	switch(subtype){
		case ATBM_PRIVATE_PROBE_RESP:{
				...
				if(start_send == 0){
					...
					atbm_printk_err("sdata:%p,ssid=%s , ssid_len = %d,psk=%s,psk_len=%d \n",sdata,
...
extern void atbm_oceanus_set_tx_power(int functionIndex, int wifi_mode, int ofdm_mode, int bw, int rateIndex, int rateMax, int power);
extern void atbm_cronus_set_tx_power(int functionIndex, int wifi_mode, int ofdm_mode, int bw, int rateIndex, int rateMax, int power);
static int atbm_dev_ioctl_set_txpower_single(struct net_device *dev, struct altm_wext_msg *msg)
{
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
    ...
    struct atbm_common *hw_priv = priv->hw_priv;
	atbm_wifi_tx_power_info txpwr_single;
	int ret = 0;
	int wifi_mode = 0;
	int ofdm_mode = 0;
	int rateMin = 0;
	int rateMax = 0;
	int bw = 0;
	memcpy(&txpwr_single, &msg->externData[0], sizeof(txpwr_single)); 
	
	txpwr_single.power = txpwr_single.power * 4;
	
	switch(txpwr_single.mode){
	case 0://DSSS
	...
	if((txpwr_single.mode < 0) || (txpwr_single.mode > 4)){
		atbm_printk_err("Invalid mode index\n");
		ret = -EINVAL;
		return ret;
	}

	if((txpwr_single.rate < rateMin) || (txpwr_single.rate > rateMax)){
	...
	bw = bw?1:0;
	atbm_printk_err("power:%d\n", txpwr_single.power);

	if(atbm_hw_priv_chip_id() == HW_CHIP_VERSION_Cronus)
		atbm_cronus_set_tx_power(txpwr_single.function_index, wifi_mode, ofdm_mode, bw, txpwr_single.rate, rateMax, txpwr_single.power);
	else
		atbm_oceanus_set_tx_power(txpwr_single.function_index, wifi_mode, ofdm_mode, bw, txpwr_single.rate, rateMax, txpwr_single.power);
...
static int atbm_dev_get_sta_info(struct net_device *dev, struct altm_wext_msg *msg)
{
    ...
    list_for_each_entry_rcu(sta, &local->sta_list, list) {
        ...
        if(sta != NULL){
            if (sta->sdata->vif.type == NL80211_IFTYPE_STATION){	
                ...
                p_info->rssi_data = (s8) -atbm_ewma_read(&sta->avg_signal_data);
    ...
    dev_printk("    RSSI %d, RSSI_DATA %d\n", p_info->rssi, p_info->rssi_data);
    dev_printk("    RX Pkts %ld\n", p_info->rx_packets);
    dev_printk("    TX Pkts %ld\n", p_info->tx_packets);
...
static int atbm_dev_get_sta_info_new(struct net_device *dev, struct altm_wext_msg *msg)
{
 ...
 //   struct atbm_common *hw_priv = priv->hw_priv;	
	...
	memset(&txrx_stat,0,sizeof(struct TX_RX_Statistics_S));
    dev_printk("%s\n", __func__);
        ...
        if(sta != NULL){
            if (sta->sdata->vif.type == NL80211_IFTYPE_STATION){	
                ...
                if(cbss){
					...
					atbm_internal_get_tx_pkt_status(sdata,mac_addr,&tx_total_pkt,&tx_fail_pkt);
                }
                memcpy(p_info->bssid, sta->sta.addr, ETH_ALEN);
                p_info->rssi = (s8) -atbm_ewma_read(&sta->avg_signal);
                p_info->rx_packets = txrx_stat.received_num;
                p_info->tx_packets = tx_total_pkt;//txrx_stat.transmit_total_cnt;
				p_info->tx_suc_count = tx_fail_pkt;//txrx_stat.sum_success_cnt;
				p_info->rx_avg_rate =  txrx_stat.rx_rate_avg;
				p_info->tx_main_rate = atbm_internal_ioctl_get_tx_rate(sdata,NULL);
                p_info->channel_idle = g_ch_idle_ratio;
                break;
            }
        }
    ...
    memcpy(msg->externData, p_info, sizeof(atbm_wifi_sta_info_new));
...
static int atbm_scan_result_filter_single_ch(u8 * recv_info,int channel)
{
...
static int atbm_dev_set_sta_scan(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	if(internal_scan.n_channels == 1)
		atbm_scan_result_filter_single_ch(recv_info,scan_ch);
...
static int atbm_dev_send_special_probe_req(struct net_device *dev, struct altm_wext_msg *msg){
    ...
    if(local->listen_started || sdata->vif.bss_conf.assoc){
        for (i = 0; i < 6; i++) {
            if (msg->ssid[0] == '\0'){
                ieee80211_send_probe_req(sdata,NULL,NULL,0,special,len,-1,true,false);
            }else{
                ieee80211_send_probe_req(sdata,NULL,msg->ssid,strlen(msg->ssid),special,len,-1,true,false);
            }
        }
    }
exit:
...
static int atbm_dev_get_tx_pkt_status(struct net_device *dev, struct altm_wext_msg *msg)
{
	unsigned long tx_pkt_status[2] = {0};
	char mac_buff[6]={0};
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);

	if (sdata->vif.type == NL80211_IFTYPE_AP){
		memcpy(mac_buff,msg->externData,6);
		sscanf(msg->externData, "%02x:%02x:%02x:%02x:%02x:%02x", (int*)&mac_buff[0], (int*)&mac_buff[1], (int*)&mac_buff[2]
			, (int*)&mac_buff[3], (int*)&mac_buff[4], (int*)&mac_buff[5]);
		atbm_printk_err("sta mac[%pM]\n",mac_buff);
	}
	
	if(atbm_internal_get_tx_pkt_status(sdata,mac_buff,&tx_pkt_status[0],&tx_pkt_status[1]) < 0){
		atbm_printk_err("%s ,get tx pkt status err!!\n",__func__);
		return -1;
	}
	if (sdata->vif.type == NL80211_IFTYPE_AP)
		atbm_printk_err("[AP MODE] sta_mac:%pM , tx_pkt=%ld,tx_fail_pkt=%ld \n",mac_buff,tx_pkt_status[0],tx_pkt_status[1]);
	else
		atbm_printk_err("[STA MODE] tx_pkt=%ld,tx_fail_pkt=%ld \n",tx_pkt_status[0],tx_pkt_status[1]);

	memcpy(msg->externData,tx_pkt_status,sizeof(tx_pkt_status));
	return 0;
}
...
static int atbm_dev_reset_fastlink(struct net_device *dev, struct altm_wext_msg *msg){
	...
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	sdata->fastlink_enable = msg->value;
	return 0;
...
static int atbm_dev_set_ampdu(struct net_device *dev, struct altm_wext_msg *msg)
{
   ...
   // struct atbm_vif *priv = (struct atbm_vif *)sdata->vif.drv_priv;
	...
	int ampdu_value = msg->value;
	
	if(atbm_internal_wsm_set_ampdu(sdata,ampdu_value) < 0){
		atbm_printk_err("%s() atbm dev set ampdu err!!\n",__func__);
		return -EINVAL;
	}
	return 0;
}
...
static int atbm_dev_set_single_scan_time(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	int scan_value = msg->value;
	if(scan_value <= 35 || scan_value > 255){
		atbm_printk_err("%s() scan_value(%d) not in range(36,255)\n",__func__,scan_value);
		return -EINVAL;
	}
...
static int atbm_dev_get_temprature(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	{
		atbm_printk_always("chip temp:%d\n", tjroom_temp.stempC);
	}
	else
	{
...
static int atbm_dev_get_cca_threshold(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	atbm_printk_err("cca threshold = %d \n",cca_threshold);
	msg->value = cca_threshold;
	return ret;
}
static int atbm_dev_set_cca_threshold(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	if((cca < -85) || (cca > -60)){
		atbm_printk_err("atbm_dev_set_cca_threshold: cca range : [-85,-60] \n");
		return -1;
	}
	cmd_len = snprintf(cmd_buf,64,"cca 1 %d",cca);
	atbm_printk_err("cmd_buf:%s , cmd_len=%d \n",cmd_buf,cmd_len);
	
	ret = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,cmd_buf,cmd_len, priv->if_id);
...
static int atbm_dev_get_rts_threshold(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	atbm_printk_err("rts threshold = %d \n",value_array[0]);
	msg->value = value_array[0];
	return ret;
}
static int atbm_dev_set_rts_threshold(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	rts_th = msg->value;
	atbm_printk_err("atbm_dev_set_rts_threshold:%d \n",rts_th);
	ret = (wsm_write_mib(hw_priv, WSM_MIB_ID_DOT11_RTS_THRESHOLD,&rts_th, sizeof(rts_th), priv->if_id));


	return ret;
}
/*
	iwpriv wlan0 common get_txpower
*/
static int atbm_dev_get_txpower_table(struct net_device *dev, struct altm_wext_msg *msg)
{
...
//	struct ieee80211_hw *hw = &local->hw;
	//struct atbm_common *hw_priv = (struct atbm_common *) hw->priv;
	//struct atbm_vif *priv = (struct atbm_vif *)sdata->vif.drv_priv;
	//char *txpower_table = NULL;
	int ret = 0;
	//int i = 0;
	if(atbm_hw_priv_chip_id() == HW_CHIP_VERSION_Cronus)
		ret = atbm_cronus_internat_cmd_get_txpower_value(NULL,msg->externData);
	else
		ret = atbm_internat_cmd_get_txpower_value(NULL,msg->externData);
	if(ret > 0){
/*		
		printk("txpower_table:ret = %d",ret);
		for(i = 0; i < ret;i++)
			printk("%d ",msg->externData[i]);
		printk("\n");
*/		
	}else
...
static int atbm_dev_set_txpwr_dcxo(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	if(atbm_hw_priv_chip_id() == HW_CHIP_VERSION_Oceanus_FM)
    {
    	for(i=0;i<10;i++)
    	{
			memcpy(&deltagain_5g[i], &msg->externData[16+i*4], 4);/*detla gain & txpwr*/
		    if(deltagain_5g[i] > deltagainMax || deltagain_5g[i] < 0){
		        atbm_printk_err("error, deltagain_5g[%d] %d\n", i, deltagain_5g[i]);
		        return -1;
		    }
		}
	}   
 ...
 //   if(ret < 0){
  //  }
	...
	reg_efuse.delta_gain1_5g = deltagain_5g[0];
	reg_efuse.delta_gain2_5g = deltagain_5g[1];
	reg_efuse.delta_gain3_5g = deltagain_5g[2];
	reg_efuse.delta_gain4_5g = deltagain_5g[3];
	reg_efuse.delta_gain5_5g = deltagain_5g[4];
	reg_efuse.delta_gain6_5g = deltagain_5g[5];
	reg_efuse.delta_gain7_5g = deltagain_5g[6];
	reg_efuse.delta_gain8_5g = deltagain_5g[7];
	reg_efuse.delta_gain9_5g = deltagain_5g[8];
	reg_efuse.delta_gain10_5g = deltagain_5g[9];
	if(atbm_hw_priv_chip_id() == HW_CHIP_VERSION_Cronus)
		atbm_cronus_set_reg_deltagain(&reg_efuse);
	else
		atbm_set_reg_deltagain(&reg_efuse);
    atbm_DCXOCodeWrite(hw_priv,dcxo);
...
int atbm_dev_set_txpwr(struct net_device *dev, struct altm_wext_msg *msg)
{
  ...
  //  struct atbm_vif *priv = (struct atbm_vif *)sdata->vif.drv_priv;
   ...
   // memcpy(&txpwr_indx, &msg->externData[0], sizeof(int));
	...
	if(atbm_hw_priv_chip_id() == HW_CHIP_VERSION_Cronus)
		ret = atbm_cronus_set_txpower_mode(txpwr_indx);
	else
...
//#if defined(TP_BEST_CHANNEL_ADD_FUNC)	
...
//	struct ieee80211_internal_channel_auto_select_req req;
	...
	scan_result=atbm_kmalloc(sizeof(Best_Channel_Scan_Result2),GFP_KERNEL);
	if(!scan_result){
		ret = -ENETDOWN;
		goto exit;		
	}
	memset(scan_result,0,sizeof(Best_Channel_Scan_Result2));
	...
	*/
	...
	start_channel = msg->externData[4];
	end_channel = msg->externData[5];
	atbm_printk_err("user_pos=%p \n",&user_pos);
	memcpy(results->ap_ssid,&(msg->externData[6]),32);
	results->best_channel_flag = 2;
	atbm_printk_err("define  TP_BEST_CHANNEL_ADD_FUNC AP_ssid[%s]!\n",results->ap_ssid);

	//Determine channel validity
	if(atbm_dev_best_channel_scan_end_process(sdata,start_channel,end_channel,results,(Best_Channel_Scan_Result *)scan_result) < 0){
		atbm_printk_err("atbm_dev_set_best_channel_scan_param_detail err!! \n");
		ret = -1;
		goto exit;
	}

	if(atbm_internal_channel_auto_select_results(sdata,results,scan_result->chan, scan_result->support_chan_num) == false){
		ret = -EINVAL;
		goto exit;
	}

	scan_result->suggest_ch = results->susgest_channel;
	memcpy(scan_result->apInfo,results->apInfo,sizeof(results->apInfo));
	if(copy_to_user(user_pos, scan_result, sizeof(Best_Channel_Scan_Result2)) != 0){
		ret = -EINVAL;
	}
...
//#elif defined(CONFIG_ANKER_MISC)
static int atbm_dev_set_best_channel_scan3(struct net_device *dev, struct altm_wext_msg *msg)
{
	int ret = 0;
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	...
	memset(&scan_result,0,sizeof(Best_Channel_Scan_Result3));
	results = atbm_kmalloc(sizeof(struct ieee80211_internal_channel_auto_select_results),GFP_KERNEL);
	if(!results){
		ret = -ENETDOWN;
		goto exit;		
...
*/
        ...
        if(sta != NULL){
            if (sta->sdata->vif.type == NL80211_IFTYPE_STATION){	
                ...
                if(cbss){
					...
					memcpy(results->conn_ap_mac,cbss->bssid,6);
                  	break;
                }  
            }
        }
	...
	if(atbm_internal_channel_auto_select_results2(sdata,results,scan_result.chan,scan_result.support_chan_num) == false){
			ret = -EINVAL;
			goto exit;
	}

	scan_result.suggest_ch[0] = results->susgest_3_channel[0];
	scan_result.suggest_ch[1] = results->susgest_3_channel[1];
	scan_result.suggest_ch[2] = results->susgest_3_channel[2];

	if(copy_to_user(user_pos, &scan_result, sizeof(scan_result)) != 0){
		ret = -EINVAL;
	}
...
static int atbm_dev_set_best_channel_scan(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	atbm_printk_err("scan_result len =  %d\n",sizeof(scan_result));
	...
	*/
	...
	//Determine channel validity
	if(atbm_dev_best_channel_scan_end_process(sdata,start_channel,end_channel,results,&scan_result) < 0){
		atbm_printk_err("atbm_dev_set_best_channel_scan_param_detail err!! \n");
		ret = -1;
		goto exit;
	}
...
static int atbm_dev_set_best_channel_scan_optional(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	struct ieee80211_internal_channel_auto_select_results *results = NULL;
	Best_Channel_Scan_Result scan_result;
	u8 channel_list[10] = {0},channel_quantity = 0;
	int i = 0;
	...
	*/
	for(i=0;i<10;i++){
		if(msg->externData[i] != 0){
			channel_list[i] = msg->externData[i];
			channel_quantity++;
		}
	}
	results->best_channel_flag = 1;
	//Determine channel validity
	if(atbm_dev_best_channel_scan_optional_process(sdata,channel_list,channel_quantity,results,&scan_result) < 0){
		atbm_printk_err("atbm_dev_set_best_channel_scan_param_detail err!! \n");
		ret = -1;
		goto exit;
	}
...
int atbm_dev_get_ap_list(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	chan_cnt = msg->value;
	atbm_printk_err("scan channel cnt = %d \n",chan_cnt);
	if(chan_cnt >= 1 &&  chan_cnt <= 14){
		channel = atbm_kzalloc(sizeof(u8) * chan_cnt, GFP_KERNEL);
		if(channel == NULL){
			ret =  -ENOMEM;
			goto exit;
		}
		atbm_printk_err("scan channel : ");
		for(i = 0; i <chan_cnt;i++){
			/*
				D����a����?����?D?�̨� ��???�����?��y?YD����a��D��???��?????2?��?
				msg->value = 8;
				msg->externDate[100] = 2
				msg->externDate[101] = 2
				msg->externDate[102] = 3
				msg->externDate[103] = 3
				msg->externDate[104] = 12
				msg->externDate[105] = 12
				msg->externDate[106] = 13
				msg->externDate[107] = 13
		*/
			if( (msg->externData[100 + i] >=1) && ( msg->externData[100 + i]<=14)){
				channel[i] = msg->externData[100 + i]; 
				atbm_printk_err(" %d ",channel[i]);
			}
		}
		atbm_printk_err("\n");
		internal_scan.n_channels = chan_cnt;
...
//	static unsigned char last_rate_id = 0;
	...
	if (sdata->vif.type == NL80211_IFTYPE_AP) {
        ...
        atbm_str2mac(mac_addr, msg->externData);
		...
		rcu_read_unlock();
        atbm_printk_err("mac:%pM,sta_id %d,sta_info:%p\n", mac_addr,sta_id,stainfo);
...
int atbm_dev_etf_test(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	chipid = atbm_get_chip_version(hw_priv);
...
static int atbm_dev_get_etf_rx_results(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	memset(&rx_results_t,0,sizeof(rx_results_t));
	ret = atbm_internal_get_start_rx_results(hw_priv,&rx_results_t.rxSuccess,&rx_results_t.FcsErr);
	if(ret < 0){
...
static int  atbm_dev_set_cail_auto_ppm(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	atbm_open_auto_cfo(hw_priv,msg->value);
...
//	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	//struct ieee80211_local *local = sdata->local;
	//struct atbm_common *hw_priv=local->hw.priv;
	char *chip_name = atbm_get_chip_type();
...
static int atbm_dev_set_ant_control(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	if(trigger_probe == 1){
		...
		scan_ch_list[0] = scan_ch_list[1] = scan_ch_list[2] = atbm_get_work_channel(sdata,0);
		internal_scan.channels = scan_ch_list;
		internal_scan.n_channels = 3;
		if(atbm_internal_cmd_scan_triger(sdata,&internal_scan) == false){
...
static int atbm_dev_set_efuse_dcxo(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	atbm_DCXOCodeWrite(hw_priv,efuse_data.dcxo_trim);
