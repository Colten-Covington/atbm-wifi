...
static int atbm_usb_bh_proc(void *arg);
...
int atbm_register_bh(struct atbm_common *hw_priv)
{
	...
	hw_priv->bh_thread = kthread_create(&atbm_usb_bh_proc, hw_priv, ieee80211_alloc_name(hw_priv->hw,"atbm_usb_bh_proc"));
...
#define __ALL_HW_BUFS_USED (hw_priv->hw_bufs_used)
	...
	while (1) {
		...
		if (!status && __ALL_HW_BUFS_USED)
		{
			...
			if (!atomic_read(&hw_priv->recent_scan)) {
