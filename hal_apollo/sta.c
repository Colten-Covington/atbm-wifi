...
void atbm_recalc_radar_detect(struct ieee80211_hw *dev){
	struct atbm_common *hw_priv = dev->priv;
	struct ieee80211_conf *conf = &dev->conf;
	struct ieee80211_local *local = hw_to_local(dev);
	struct atbm_vif *priv;

	if(conf->radar_enabled == hw_priv->cac_started)
		return;

	if(!local->cac_sdata)
		return;

	priv = (struct atbm_vif *)local->cac_sdata->vif.drv_priv;
	if(conf->radar_enabled){
		if (local->dfs_cac_chan_def.chan->flags & IEEE80211_CHAN_RADAR) {
			if (local->dfs_cac_chan_def.chan->dfs_state != NL80211_DFS_AVAILABLE){
				hw_priv->cac_started = true;
				hw_priv->dfs_if_id = priv->if_id;
				atbm_enable_listening(priv,	local->dfs_cac_chan_def.chan);
			}
		}
		return;
	}
	hw_priv->cac_started = false;
	hw_priv->dfs_if_id = -1;
	atbm_disable_listening(priv);
}
#endif
...
#if defined(CONFIG_ATBM_STA_LISTEN) || defined(CONFIG_ATBM_SUPPORT_P2P) || defined(CONFIG_ATBM_RADAR_DETECT)
...
#ifdef CONFIG_ATBM_RADAR_DETECT
#if	LINUX_VERSION_IS_GEQ_OR_CPTCFG(3, 9, 0)
	atbm_printk_debug("Radar %d %d %d\n", hw_priv->cac_started, sdata->radar_required, hw_priv->channel_type);
	if(hw_priv->cac_started && sdata->radar_required){
