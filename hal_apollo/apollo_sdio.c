...
const char DRIVER_INFO[]={"wifi6_40M_lite"};
...
static int driver_build_info(void)
{
	...
	atbm_printk_init("SVN_VER=%d,DPLL_CLOCK=%d,%s,BUILD_TIME=%s\n", build.ver, build.dpll, build.driver_info, buildtime);
...
#endif
#ifdef CONFIG_SDIO_SUPPORT_PS
	struct atbm_sdio_ps ps;
#endif
};
...
static int atbm_sdio_set_ps(struct sbus_priv* self, bool powersave);

#ifdef CONFIG_SDIO_SUPPORT_PS
static void atbm_sdio_ps_init(struct sbus_priv* self)
{
	struct atbm_sdio_ps* ps = &self->ps;

	/*
	 * default is wakeup state
	 */
	ps->count = 1;

	mutex_init(&ps->lock);
}
static int __atbm_sdio_pm_active(struct sbus_priv* self)
{
#ifdef CONFIG_SDIO_SUPPORT_PS
	struct atbm_sdio_ps* ps = &self->ps;
	int ret = 0;
//	int val;
	...
	ret = atbm_sdio_set_ps(self, false);
	
	if (ret) {
		atbm_printk_err("%s:set active err", __func__);
		goto exit;
	}
...
static int __atbm_sdio_pm_doze(struct sbus_priv* self)
{
#ifdef CONFIG_SDIO_SUPPORT_PS
	struct atbm_sdio_ps* ps = &self->ps;
	int ret = 0;
	
	if (WARN_ON(ps->count == 0)) {
		goto exit;
	}
	...
	ret = atbm_sdio_set_ps(self, true);

	if (ret) {
		ps->count++;
		atbm_printk_err("%s:set doze err\n", __func__);
		goto exit;
	}
...
static int atbm_sdio_rx_thread(void *priv)
{
	...
	*/
	...
	while (!atbm_sdio_wait_action(&self->rx_thread)) {
		atbm_sdio_pm(self, false);
		atbm_sdio_rx_bh(self->core);
		atbm_sdio_pm(self, true);
	};
...
#endif//CONFIG_ATBM_SDIO_TX_PROCESS_NORMAL
...
#define MAX_LOOP_POLL_CNT  (2*3000)
	...
	while(enough == false){
        ...
        //atbm_printk_err("hw_ximit(%x),(%d)(%d)(%d)\n",hw_priv->wsm_caps.NumOfHwXmitedAddr,hw_xmited,hw_priv->wsm_caps.numInpChBufs,hw_priv->n_xmits);
		if(ret){
			enough = false;
			break;
		}
...
static int atbm_sdio_cook_sg(struct sbus_priv *self,struct sdio_scatterlist *sgl)
{
	...
	do {
	    ...
	    encap.buff_len = SDIO_SCATTERLIST_CACHE_SIZE - sgl->totallen;

        BUG_ON(encap.buff_len <= 0);
...
static int __atbm_sdio_pm(struct sbus_priv* self, bool  suspend)
{
#ifdef CONFIG_SDIO_SUPPORT_PS
//	struct atbm_sdio_ps* ps = &self->ps;
	int ret = 0;
	...
	if (suspend == false) {
		ret = __atbm_sdio_pm_active(self);
	}
	else {
		ret = __atbm_sdio_pm_doze(self);
	}
	return ret;
#else
	return 0;
...
#ifdef CONFIG_SDIO_SUPPORT_PS
	sdio_claim_host(self->func);
	ret = __atbm_sdio_pm(self, suspend);
	sdio_release_host(self->func);
#endif
...
#ifdef CONFIG_SDIO_SUPPORT_PS
static int __atbm_sdio_set_ps_active(struct atbm_common* hw_priv)
{
	int ret = 0;
	int retries = 0;
	u16 ctrl_reg = 0;

	ret = atbm_reg_write_16(hw_priv, ATBM_HIFREG_CONTROL_REG_ID, ATBM_HIFREG_CONT_WUP_BIT);

	if (ret) {
		atbm_printk_err("%s:err 1\n", __func__);
		goto exit;
	}
...
retry:
	...
	if (retries > 1000) {
		atbm_printk_err("%s:retries(%d)(%x)\n", __func__, retries, ctrl_reg);
		ret = 1;
		goto exit;
	}
...
static int __atbm_sdio_set_ps_doze(struct atbm_common* hw_priv)
{
	return atbm_reg_write_16(hw_priv, ATBM_HIFREG_CONTROL_REG_ID, 0);
}
static int atbm_sdio_set_ps(struct sbus_priv* self, bool powersave)
{
	if (powersave == true) {
		return __atbm_sdio_set_ps_doze(self->core);
	}
	else {
		return __atbm_sdio_set_ps_active(self->core);
	}
}
#endif
			      ...
			      const struct sdio_device_id *id)
{
    ...
    if(func->card->host->caps & MMC_CAP_SDIO_IRQ){
        atbm_printk_err("host enable irq\n");
    }else {
        atbm_printk_err("host disable irq\n");
    }
    atbm_printk_init("sdio clock(%u)\n",func->card->host->ios.clock);
...
static struct sdio_driver atbm_sdio_driver = {
...
#endif
	ret = sdio_register_driver(&atbm_sdio_driver);
	if (ret)
		goto err_reg;
#if ((ATBM_WIFI_PLATFORM != 10) && (ATBM_WIFI_PLATFORM != PLATFORM_AMLOGIC_S805)\
	&& (ATBM_WIFI_PLATFORM != PLATFORM_AMLOGIC_905))
...
err_clk:
	sdio_unregister_driver(&atbm_sdio_driver);
err_reg:
	return ret;
}
