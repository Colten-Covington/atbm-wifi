...
void atbm_monitor_pc(struct atbm_common *hw_priv)
{
	...
	if (hw_priv->chip_version == RHEA){
		atbm_direct_write_reg_32(hw_priv,0x16100050,5);
	}else{
...
void print_efuse_details(struct atbm_common *hw_priv,struct efuse_headr *efuse_save)
{
	if((NULL == hw_priv) || (NULL == efuse_save))
	{
		atbm_printk_err("Invalid ptr\n");
		return;
	}

	atbm_printk_always("specific:%d\n", efuse_save->specific);
	atbm_printk_always("version:%d\n", efuse_save->version);
	atbm_printk_always("dcxo_trim:%d\n", efuse_save->dcxo_trim);
	atbm_printk_always("delta_gain1:%d\n", efuse_save->delta_gain1);
	atbm_printk_always("delta_gain2:%d\n", efuse_save->delta_gain2);
	atbm_printk_always("delta_gain3:%d\n", efuse_save->delta_gain3);
	atbm_printk_always("Tj_room:%d\n", efuse_save->Tj_room);
	atbm_printk_always("topref_ctrl_bias_res_trim:%d\n", efuse_save->topref_ctrl_bias_res_trim);
	atbm_printk_always("PowerSupplySel:%d\n", efuse_save->PowerSupplySel);
	atbm_printk_always("mac:%02x:%02x:%02x:%02x:%02x:%02x\n", efuse_save->mac[0],efuse_save->mac[1],efuse_save->mac[2],
		efuse_save->mac[3],efuse_save->mac[4],efuse_save->mac[5]);
	if(hw_priv->chip_version == OCEANUS)
	{
		atbm_printk_always("delta_gain1_5g :%d\n", efuse_save->delta_gain1_5g);
		atbm_printk_always("delta_gain2_5g:%d\n", efuse_save->delta_gain2_5g);
		atbm_printk_always("delta_gain3_5g:%d\n", efuse_save->delta_gain3_5g);
		atbm_printk_always("delta_gain4_5g :%d\n", efuse_save->delta_gain4_5g);
		atbm_printk_always("delta_gain5_5g:%d\n", efuse_save->delta_gain5_5g);
		atbm_printk_always("delta_gain6_5g:%d\n", efuse_save->delta_gain6_5g);
		atbm_printk_always("delta_gain7_5g :%d\n", efuse_save->delta_gain7_5g);
		atbm_printk_always("delta_gain8_5g:%d\n", efuse_save->delta_gain8_5g);
		atbm_printk_always("delta_gain9_5g:%d\n", efuse_save->delta_gain9_5g);
		atbm_printk_always("delta_gain10_5g :%d\n", efuse_save->delta_gain10_5g);
	}
...
int atbm_save_efuse(struct atbm_common *hw_priv,struct efuse_headr *efuse_save)
{
	...
	*/
	if(hw_priv->chip_id == HW_CHIP_VERSION_Oceanus_FM)
		efuseLen = sizeof(struct efuse_headr);
	else
		efuseLen = sizeof(struct efuse_headr_old);
	...
	if (iResult == 0)
	{
		...
		if(memcmp((void *)&efuse_bak, efuse_save, efuseLen) !=0)
		{
			atbm_printk_err("efuse_write:\n");
			print_efuse_details(hw_priv, efuse_save);
			atbm_printk_err("efuse_read:\n");
			print_efuse_details(hw_priv, &efuse_bak);
			iResult = -2;
		}else
		{
			iResult = 0;
