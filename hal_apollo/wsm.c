...
extern int atbm_get_rate_delta_gain(char *src_file, int *dst);
extern int atbm_cronus_get_rate_delta_gain(char *src_file, int *dst);

extern void atbm_set_power_by_mode(int wifi_mode, int ofdm_mode, int bw, int rateIndex, int delfault_power, int power_delta, int powerTarFlag);
extern void atbm_cronus_set_power_by_mode(int wifi_mode, int ofdm_mode, int bw, int rateIndex, int delfault_power, int power_delta, int powerTarFlag);
...
int wsm_set_rate_power(struct atbm_common *hw_priv)
{
	...
	file_5g = CONFIG_RATE_TXPOWER_5G;
#else
	file = "/tmp/set_rate_power.txt";
	file_5g = "/tmp/set_rate_power_5g.txt";
#endif
	...
	if(hw_priv->chip_id == HW_CHIP_VERSION_Cronus_Lite_FM)
	{
		...
		if((atbm_cronus_get_rate_delta_gain(file, &rate_txpower[0]) !=  0)){
			atbm_printk_always("==read file fail==\n");
			return -1;
		}
		for(i=1;i<=64;i++)
	...
	else
	{
		...
		if((atbm_get_rate_delta_gain(file, &rate_txpower[0]) !=  0)){
		...
		if((atbm_get_rate_delta_gain(file_5g, &rate_txpower[0]) !=  0)){
			atbm_printk_always("==read %s fail==\n", file_5g);
		}
		else
		{
...
int atbm_cronus_wsm_set_rate_power(struct atbm_common *hw_priv)
{
	...
	for(i=1;i<=44;i++)
	{
		...
		if(i<=2)
			atbm_cronus_set_power_by_mode(1, 0, 0, i, MCS_LUT_20M_Cronus[i-1], power, powerTarFlag);//rateIndex 0,1,2,3 --> 1:(0,1);2:(2,3)
		else if(i<=11)
		{
			if(i<=10)
				atbm_cronus_set_power_by_mode(0, 0, 0, (i-3), MCS_LUT_20M_Cronus[i-1], power, powerTarFlag);
			else
				atbm_cronus_set_power_by_mode(0, 1, 0, (i-4), MCS_LUT_20M_Cronus[i-1], power, powerTarFlag);
		}
		else if(i<=23)
			atbm_cronus_set_power_by_mode(0, 2, 0, (i-12), MCS_LUT_20M_Cronus[i-1], power, powerTarFlag);
		else if(i<=32)
		{
			if(i<=31)
				atbm_cronus_set_power_by_mode(0, 0, 1, (i-24), MCS_LUT_40M_Cronus[i-22], power, powerTarFlag);
			else
				atbm_cronus_set_power_by_mode(0, 1, 1, (i-25), MCS_LUT_40M_Cronus[i-22], power, powerTarFlag);
		}
		else if(i<=44)
			atbm_cronus_set_power_by_mode(0, 2, 1, (i-33), MCS_LUT_40M_Cronus[i-22], power, powerTarFlag);
	}
		...
		int if_id)
{
    ...
    WSM_PUT(buf, arg->rev,3);
    WSM_PUT(buf,&arg->supp_rate,sizeof(struct wsm_supp_rate));
...
//	printk("wsm_caps.firmwareCap %x firmware used %s-rate policy\n",hw_priv->wsm_caps.firmwareCap,hw_priv->wsm_caps.firmwareCap&CAPABILITIES_NEW_RATE_POLICY?"new":"old");
	...
	atbm_printk_init("EXCAPABILITIES_BLE_50		[%d]\n" ,!!(hw_priv->wsm_caps.firmwareExCap &EXCAPABILITIES_BLE_50)); 
	atbm_printk_init("EXCAPABILITIES_WIFI6	[%d]\n" ,!!(hw_priv->wsm_caps.firmwareExCap &EXCAPABILITIES_WIFI6)); 
...
static void wsm_fill_rx_cb_info(struct ieee80211_rx_status *hdr,struct wsm_rx *rx)
{
    ...
    if(rx->rxedRate >= 38){
        hdr->flag |= RX_FLAG_VHT;
		hdr->rate_idx =  rx->rxedRate - 38;
    }else if(rx->rxedRate >= 26){
		hdr->flag |= RX_FLAG_HE;
	...
	if(rx->flags & (WSM_RX_STATUS_GROUP|WSM_RX_STATUS_BROADCAST))
		hdr->flag |= RX_FLAG_MULTICAST;
...
int wsm_receive_indication_8023_frame(struct atbm_common *hw_priv,struct sk_buff* skb)
{
	...
	*/
	...
	atbm_tcpack_update_when_rx(hw_priv,skb);
#endif //CONFIG_TCP_ACK_OFFLOAD
				...
				int interface_link_id)
{
	...
	if(interface_link_id == 2 && hw_priv->cac_started){
		if(hw_priv->dfs_if_id < 2){
			interface_link_id = hw_priv->dfs_if_id;
		}else{
			goto underflow;
		}
	}
					...
					struct wsm_buf *buf)
{
	...
	if (hw_priv->wsm_cbc.scan_complete) {
		...
		if(hw_priv->scan.cca){
			...
			if(buf->data + sizeof(arg.radar_exist) <= buf->end){
				WSM_GET(buf, arg.radar_exist, sizeof(arg.radar_exist));
			}else{
				memset(arg.radar_exist, 0, sizeof(arg.radar_exist));
			}
		}
...
out:
    if(ret){
        print_hex_dump_bytes("wsm err-->",DUMP_PREFIX_NONE,wsm,wsm->len);
    }
	return ret;
}
static void wsm_build_80211_frame(struct wsm_tx_encap *wsm_encap)
{
    ...
    struct ieee80211_build_frame build;

    build.vif    = wsm_encap->priv->vif;
    build.hw_key = NULL;
    build.skb    = wsm_encap->skb;
    build.xmit_buff = wsm_encap->data + sizeof(struct wsm_tx);
    build.buff_len  = wsm_encap->buff_len - sizeof(struct wsm_tx);
    build.sta       = rcu_dereference(wsm_encap->priv->linked_sta[wsm_encap->txpriv->raw_link_id]);
	...
	*/
	...
	wsm_encap->len = ieee80211_build_80211_frame(&build);
    wsm_encap->hw_key = build.hw_key;
}
...
int wsm_get_tx(struct atbm_common *hw_priv, struct hif_tx_encap *encap)
{
	...
	if (hw_priv->wsm_cmd.ptr) {
        ...
        if(unlikely(encap->buff_len >= hw_priv->wsm_cmd.len)){
		    ++count;
            ...
            atbm_printk_err("wsm len err(%d)(%d)\n",encap->buff_len,hw_priv->wsm_cmd.len);
        }
	...
	{		
        ...
        wsm_encap.buff_len = encap->buff_len;
		...
		for (;;) {
            ...
            if(unlikely(encap->buff_len < sizeof(struct wsm_tx))){
                atbm_printk_err("%s:buff_len too short(%d)\n",__func__,encap->buff_len);
                break;
            }
...
*/
	WSM_PUT(buf, arg, sizeof(struct efuse_headr));
...
{
	int ret;
	ret = wsm_write_mib(hw_priv, WSM_MIB_ID_SET_TEMP_DCXO_TABLE, ptemp_dcxo_table, sizeof(struct atbm_temp_dcxo_table), if_id);
	return ret;
}
