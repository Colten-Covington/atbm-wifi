...
static void ble_npl_callout_timer_cb(struct timer_list *arg)
#elif(LINUX_VERSION_CODE >= KERNEL_VERSION(4, 9, 129))
static void ble_npl_callout_timer_cb(unsigned long arg)
#else
static void ble_npl_callout_timer_cb(atbm_void *arg)
#endif
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
...
void npl_atbmble_npl_callout_init(struct ble_npl_callout *co, struct ble_npl_eventq *evq,
                     ble_npl_event_fn *ev_cb, void *ev_arg)
{
    memset(co, 0, sizeof(*co));
	atbm_InitTimer(&co->handle, ble_npl_callout_timer_cb, co);
    co->evq = evq;
    ble_npl_event_init(&co->ev, ev_cb, ev_arg);
}

ble_npl_error_t npl_atbmble_npl_callout_reset(struct ble_npl_callout *co, ble_npl_time_t ticks)
{
    if (ticks < 0) {
        return BLE_NPL_INVALID_PARAM;
    }
