                        ...
                        uint16_t handle, char *uuid_buf, char *flags_buf)
{
    ...
    for (chr = svc->characteristics; chr && chr->uuid; ++chr) {
        iot_printf("characteristic\n");
        iot_printf("%" FIELD_INDENT "s %" FIELD_NAME_LEN "s "
                       "%s\n", " ", "uuid",
