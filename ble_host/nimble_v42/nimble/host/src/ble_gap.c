...
SRAM_CODE int
ble_gap_set_scan_chan_map(uint8_t mask)
{
	uint8_t buf[1];

	if((mask == 0) || (mask > 7)){
        iot_printf("invalid scan chan map\n");
		return BLE_ERR_INV_HCI_CMD_PARMS;
	}

	iot_printf("\n[ATBM lOG]set scan chan map(0x%X)\n", mask);
	buf[0] = mask;
    return ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_VENDOR, BLE_HCI_VS_SET_SCAN_CHAN_MAP),
        						buf, sizeof(buf), NULL, 0);
}
...
ble_gap_rx_le_pathloss_threshold(const struct ble_hci_ev_le_subev_path_loss_threshold *ev)
{
    ...
    ble_gap_event_listener_call(&event);
	ble_gap_call_conn_event_cb(&event, event.pathloss_threshold.conn_handle);
}
...
ble_gap_rx_transmit_power_report(const struct ble_hci_ev_le_subev_transmit_power_report *ev)
{
    ...
    ble_gap_event_listener_call(&event);
	ble_gap_call_conn_event_cb(&event, event.transmit_power.conn_handle);
}
#endif
...
ble_gap_rx_subrate_change(const struct ble_hci_ev_le_subev_subrate_change *ev)
{
    ...
    ble_gap_event_listener_call(&event);
	ble_gap_call_conn_event_cb(&event, event.subrate_change.conn_handle);
}
#endif
...
int
ble_gap_vs_enh_set_power(uint16_t conn_handle, int8_t tx_pwr)
{
#if MYNEWT_VAL(BLE_POWER_CONTROL)
    uint8_t buf[3];
	uint16_t opcode;

    opcode = BLE_HCI_OP(BLE_HCI_OGF_VENDOR, BLE_HCI_VS_SET_ENH_TX_PWR);

    buf[0] = (uint8_t)opcode;
	buf[1] = (uint8_t)(opcode >> 8);
	buf[2] = tx_pwr;

    return ble_hs_hci_cmd_tx(opcode, buf, 3, NULL, 0);
#else
    return BLE_HS_ENOTSUP;
#endif
}
