...
int ble_hs_rx_test(uint8_t channel, uint8_t phy_mode,uint8_t Modulation_Index);
int ble_hs_tx_test(uint8_t channel, uint8_t length, uint8_t Packet_Payload);
int ble_hs_dtm_tx_test(uint8_t channel, uint8_t length, uint8_t Packet_Payload, uint8_t phy_mode);
int ble_hs_dtm_rx_test(uint8_t channel, uint8_t phy_mode);
int ble_hs_dtm_end(void);
int ble_tone_tx_start(uint8_t channel, uint8_t power);
int ble_tone_tx_stop(void);
int ble_start_srrc(void);
int ble_stop_srrc(void);
int ble_hs_scan_set_name_filt(uint8_t name_len, uint8_t *name);
int ble_hs_afh_param_set(uint8_t threshold, uint8_t period);
int ble_hs_hci_set_pub_addr(uint8_t *addr);
int ble_hs_set_ce_test(uint8_t enable, int8_t rssi);


#define CMD_ADV_PDU_PAYLOAD_MAX_LEN                 (BLE_DEV_ADDR_LEN + BLE_HS_ADV_MAX_SZ)
#define CMD_SCAN_FILTER_BYTE_INDEX_MAX_VALUE        (CMD_ADV_PDU_PAYLOAD_MAX_LEN - 1)
#define CMD_FILTER_STRING_MAX_LENGTH                (29)
#define CMD_SCAN_FILT_SET_CMD_HEX_STRING            (0)
#define CMD_SCAN_FILT_SET_CMD_NORMAL_STRING         (1)
int ble_scan_filt_set_by_user(uint8_t start_byte_index, bool flag_hex_string, uint8_t *user_string);
int ble_hs_scan_filt_set(uint8_t start_byte_index, bool flag_hex_string, uint8_t *filter_string);
int ble_hs_scan_filt_clr(void);
