...
static struct ble_npl_callout update_params_co;
static struct gap_conn_param_update_cmd update_params;

static struct ble_npl_callout connected_ev_co;
...
static struct ble_npl_callout bttester_nrpa_rotate_timer;
#endif
...
static void rotate_nrpa_cb(struct os_event *ev)
{
    ...
    if (remaining_time > 0) {
        ...
        ble_npl_callout_reset(&bttester_nrpa_rotate_timer,
                         remaining_ticks);
    }
}
#endif
...
le_connected(uint16_t conn_handle, int status)
{
    ...
    ble_npl_callout_reset(&connected_ev_co,
...
le_disconnected(struct ble_gap_conn_desc *conn, int reason)
{
    ...
    if ((reason == BLE_HS_HCI_ERR(BLE_ERR_CONN_ESTABLISHMENT)) &&
        ble_npl_callout_queued(&connected_ev_co)) {
        if (connection_attempts < MYNEWT_VAL(BTTESTER_CONN_RETRY)) {
            ble_npl_callout_stop(&connected_ev_co);
    ...
    } else if (ble_npl_callout_queued(&connected_ev_co)) {
        ble_npl_callout_stop(&connected_ev_co);
        return;
    }
#endif
...
auth_passkey_oob_sc(uint16_t conn_handle)
{
    ...
    rc = ble_sm_inject_io(conn_handle, &pk);
    if (rc != 0) {
        iot_printf("error providing oob; rc=%d\n", rc);
    }
}
...
gap_event_cb(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_ADV_COMPLETE:
        iot_printf("advertising complete; reason=%d\n",
    ...
    case BLE_GAP_EVENT_REPEAT_PAIRING:
        iot_printf("repeat pairing event; conn_handle=%d "
    ...
    case BLE_GAP_EVENT_CONN_UPDATE:
        iot_printf("connection update event; status=%d ",
    ...
    case BLE_GAP_EVENT_CONN_UPDATE_REQ:
        iot_printf("connection update request event; "
    ...
    case BLE_GAP_EVENT_PARING_COMPLETE:
        iot_printf("received pairing complete: "
...
passkey_confirm(const uint8_t *data, uint16_t len)
{
    ...
    rc = ble_sm_inject_io(desc.conn_handle, &pk);
    if (rc) {
        iot_printf("sm inject io failed");
        status = BTP_STATUS_FAILED;
        goto rsp;
    }
...
{
    iot_printf("conn param update complete; conn_handle=%d status=%d\n",
                   conn_handle, status);
}
...
conn_param_update_async(const uint8_t *data, uint16_t len)
{
    ...
    ble_npl_callout_reset(&update_params_co, 0);
...
tester_init_gap_cb(int err)
{
    ...
    ble_npl_callout_init(&update_params_co, os_eventq_dflt_get(),
                    conn_param_update, NULL);

    ble_npl_callout_init(&connected_ev_co, os_eventq_dflt_get(),
...
#endif
#if MYNEWT_VAL(BTTESTER_PRIVACY_MODE) && MYNEWT_VAL(BTTESTER_USE_NRPA)
    ble_npl_callout_init(&bttester_nrpa_rotate_timer, os_eventq_dflt_get(),
                    rotate_nrpa_cb, NULL);
#endif
