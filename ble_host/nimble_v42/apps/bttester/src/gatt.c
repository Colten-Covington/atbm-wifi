...
static struct ble_npl_callout notify_tx_timer;
...
{
    ble_npl_callout_stop(&notify_tx_timer);
}
...
notify_test_reset(void)
{
    ...
    rc = ble_npl_callout_reset(&notify_tx_timer, OS_TICKS_PER_SEC);
    assert(rc == 0);
}
...
{
    ble_npl_callout_init(&notify_tx_timer, os_eventq_dflt_get(),
                    notify_test, NULL);
