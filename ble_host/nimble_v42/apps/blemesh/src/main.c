              ...
              uint8_t *fault_count)
{
    ...
    iot_printf("fault_get_cur() has_reg_fault %u\n", has_reg_fault);
              ...
              uint8_t *fault_count)
{
    ...
    iot_printf("fault_get_reg() has_reg_fault %u\n", has_reg_fault);
                             ...
                             struct bt_mesh_msg_ctx *ctx)
{
    ...
    *status = gen_on_off_state;
    ...
    if (rc) {
        iot_printf("#mesh-onoff STATUS: send status failed\n");
    }
...
static int gen_onoff_set_unack(struct bt_mesh_model *model,
                ...
                struct os_mbuf *buf)
{
    iot_printf("#mesh-onoff SET-UNACK\n");
                             ...
                             struct bt_mesh_msg_ctx *ctx)
{
    ...
    rc = bt_mesh_model_send(model, ctx, msg, NULL, NULL);
    if (rc) {
        iot_printf("#mesh-level STATUS: send status failed\n");
    }
              ...
              struct os_mbuf *buf)
{
    ...
    iot_printf("#mesh-level SET: level=%d\n", level);
                ...
                struct os_mbuf *buf)
{
    ...
    level = (int16_t) net_buf_simple_pull_le16(buf);
    iot_printf("#mesh-level SET-UNACK: level=%d\n", level);
              ...
              struct os_mbuf *buf)
{
    ...
    iot_printf("#mesh-level DELTA-SET: delta_level=%d\n", delta_level);
                           ...
                           struct os_mbuf *buf)
{
    ...
    iot_printf("#vendor-model-recv\n");

    iot_printf("data:%s len:%d\n", bt_hex(buf->om_data, buf->om_len),
                   buf->om_len);
    ...
    if (bt_mesh_model_send(model, ctx, msg, NULL, NULL)) {
        iot_printf("#vendor-model-recv: send rsp failed\n");
    }
...
{
    iot_printf("Local node provisioned, primary address 0x%04x\n", addr);
}
...
blemesh_on_sync(void)
{
    ...
    iot_printf("Bluetooth initialized\n");
    ...
    if (err) {
        iot_printf("Initializing mesh failed (err %d)\n", err);
        return;
    }
