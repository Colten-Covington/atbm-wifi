...
rx_stress_adv_start(uint8_t instance)
{
    ...
    assert (rc == 0 || rc == 2);
    BLE_HS_LOG(INFO, "Instance %d started; rc: %d\n", instance, rc);
    return rc;
}
...
rx_stress_simple_adv(struct rx_stress_adv_set *adv_set)
{
    ...
    if (rc != 0) {
        BLE_HS_LOG(ERROR, "\033[0;31mError determining own address type; "
                           "rc=%d\033[0m\n", rc);
        return;
    }
    ...
    BLE_HS_LOG(INFO, "instance %u started\n", adv_set->instance);
}
...
rx_stress_0_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            ...
            BLE_HS_LOG(INFO, "Connection failed; status=%d ",
                        event->connect.status);
        }
        return 0;
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        iot_printf("\033[0;32mReceived signal to switch test\033[0m\n");
...
rx_stress_4_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        /* A new connection was established or a connection attempt failed. */
        if (event->connect.status == 0) {
            BLE_HS_LOG(INFO, "Connection established; status=%d; num=%d",
                        event->connect.status,
...
{
    BLE_HS_LOG(INFO, "Event occurs=%d\n", event->type);
    return 0;
}
...
{
    if (status == 0) {
        BLE_HS_LOG(INFO, "L2CAP params updated\n");
    } else {
        BLE_HS_LOG(INFO, "L2CAP params update failed; rc=%d\n", status);
        assert(0);
    }
}
...
rx_stress_10_l2cap_event(struct ble_l2cap_event *event, void *arg)
{
    ...
    case BLE_L2CAP_EVENT_COC_CONNECTED:
        if (event->connect.status) {
            BLE_HS_LOG(INFO, "LE COC error: %d\n", event->connect.status);
            return 0;
        }
    ...
    case BLE_L2CAP_EVENT_COC_DISCONNECTED:
        BLE_HS_LOG(INFO, "LE CoC disconnected, chan: 0x%08lx\n",
    ...
    case BLE_L2CAP_EVENT_COC_DATA_RECEIVED:
        ...
        BLE_HS_LOG(INFO, "L2CAP server received data; num=%d\n",
    ...
    case BLE_L2CAP_EVENT_COC_TX_UNSTALLED:
        BLE_HS_LOG(INFO, "L2CAP unstalled event\n");
    ...
    default:
        BLE_HS_LOG(INFO, "Other L2CAP event occurs: %d\n", event->type);
        return 0;
    }
    ...
    BLE_HS_LOG(INFO, "Return code=%d\n", rc);
    if (rc) {
        if (rc == BLE_HS_ESTALLED) {
            BLE_HS_LOG(INFO, "L2CAP stalled - waiting\n");
            stalled = true;
        } else {
            BLE_HS_LOG(INFO, "Sending data via L2CAP failed with error "
                        "code %d\n", rc);
        }
    }
...
rx_stress_12_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        BLE_HS_LOG(INFO, "Average time: %d us\n",
    ...
    case BLE_GAP_EVENT_NOTIFY_TX:
        ...
        BLE_HS_LOG(INFO, "Notify TX event\n");
        ...
        iot_printf("Indication time: %lld\n", us);
...
rx_stress_13_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_NOTIFY_TX:
        BLE_HS_LOG(INFO, "Notify TX event; num=%d\n",
                    ++rx_stress_ctx->send_num);
...
rx_stress_14_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        BLE_HS_LOG(INFO, "Subscribe event\n");

        if (event->subscribe.cur_notify) {
            BLE_HS_LOG(INFO, "Notification subscribed\n");
            ...
            BLE_HS_LOG(INFO, "Notification unsubscribed\n");
        } else {
            BLE_HS_LOG(INFO, "Other subscription\n");
        }
        return 0;
...
rx_stress_main_task_fn(void *arg)
{
    ...
    iot_printf("\033[1;36mRX device\033[0m\n");
