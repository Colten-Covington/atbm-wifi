                      ...
                      struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    case STRESS_GATT_READ_UUID:
        BLE_HS_LOG(INFO, "GATT Read event\n");
        rc = os_mbuf_append(ctxt->om, &chr_value, sizeof(chr_value));
        assert(rc == 0);
        //return rc == 0 ? 0 : BLE_ATT_ERR_INSUFFICIENT_RES;
        return 0;
    case STRESS_GATT_WRITE_UUID:
        BLE_HS_LOG(INFO, "GATT Write event\n");
        print_mbuf(ctxt->om);
        return 0;
    case STRESS_GATT_NOTIFY_UUID:
        BLE_HS_LOG(INFO, "GATT Notify event\n");
        return 0;
    case STRESS_GATT_INDICATE_UUID:
        BLE_HS_LOG(INFO, "GATT Indicate event\n");
        return 0;
    default:
        BLE_HS_LOG(ERROR, "GATT UUID does not exist\n");
        assert(0);
