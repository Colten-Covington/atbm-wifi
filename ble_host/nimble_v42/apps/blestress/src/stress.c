...
com_stress_print_report(const struct com_stress_test_ctx *test_ctxs)
{
    ...
    iot_printf("\033[0;33mUse case 6 - Stress Scan: \n\033[0m");
    iot_printf("Received first packets = %d\n",
                   test_ctxs->s6_rcv_adv_first);
    iot_printf("Received all packets = %d\n", test_ctxs->s6_rcv_adv_suc);

    iot_printf("\033[0;33mUse case 7 - Stress PHY Update (TX): \n\033[0m");
    iot_printf("PHY updates = %d\n", test_ctxs->con_stat[7].phy_upd_num);

    iot_printf("\033[0;33mUse case 8 - Stress PHY Update (RX): \n\033[0m");
    iot_printf("PHY updates = %d\n", test_ctxs->con_stat[8].phy_upd_num);

    iot_printf(
        "\033[0;33mUse case 9 - Stress multi connection: \n\033[0m");
    iot_printf("Max reached num of connections = %d\n",
                   test_ctxs->con_stat[9].max_num);

    iot_printf("\033[0;33mUse case 10 - Stress L2CAP send: \n\033[0m");
    iot_printf("Average bit rate = %d\n", test_ctxs->s10_bit_rate);
    iot_printf("Max received MTU = %lld\n", test_ctxs->s10_max_mtu);

    iot_printf("\033[0;33mUse case 11 - "
                   "Stress Advertise/Connect/Continue adv \n\033[0m");
//    iot_printf(" = %d\n",);

    iot_printf("\033[0;33mUse case 12 - "
                   "Stress GATT indication: \n\033[0m");
    iot_printf("Average bit rate = %d\n", test_ctxs->s12_notif_time);

    iot_printf("\033[0;33mUse case 13 - "
                   "Stress GATT notification: \n\033[0m");
    iot_printf("Average time = %d\n", test_ctxs->s13_notif_time);

    iot_printf("\033[0;33mUse case 14 - "
                   "Stress GATT Subscribe/Notify/Unsubscribe: \n\033[0m");
    iot_printf("Average time = %d\n", test_ctxs->s14_notif_time);

    iot_printf("\033[0;33mUse case 15 - "
                   "Stress Connect/Send/Disconnect: \n\033[0m");
    iot_printf("Con num = %d\n", test_ctxs->con_stat[15].num);
}
...
stress_start_timer(uint32_t timeout_ms, os_event_fn *ev_cb)
{
    ...
    ble_npl_callout_stop(&stress_timer_callout);

    ble_npl_callout_init(&stress_timer_callout, os_eventq_dflt_get(), ev_cb, NULL);

    rc = ble_npl_callout_reset(&stress_timer_callout,
                   ...
                   void *arg)
{
    ...
    if (error->status == 0) {
        if (!ble_uuid_cmp(&dsc->uuid.u, &search_ctx->dsc_uuid.u) && !found) {
            BLE_HS_LOG(INFO, "Found chr descriptor\n");
            search_ctx->dsc_handle = dsc->handle;
            BLE_HS_LOG(INFO, "uuid=%#06x; handle=%#06x", dsc->uuid.u16.value,
                        dsc->handle);
            found = true;
        }
        return 0;
    ...
    if (error->status == BLE_HS_EDONE) {
        BLE_HS_LOG(INFO, "Done descriptor discovery\n");
        ...
        BLE_HS_LOG(ERROR, "\033[0;31mDid not find particular descriptor"
                           "\033[0m\n");
        return 0;
    }

    BLE_HS_LOG(ERROR, "\033[0;31mError during descriptor discovery"
                       "\033[0m\n");
    assert(0);
    return 0;
}
                   ...
                   const struct ble_gatt_chr *chr, void *arg)
{
    ...
    if (error->status == 0) {
        BLE_HS_LOG(INFO, "Found characteristic\n");
    ...
    if (error->status == BLE_HS_EDONE) {
        BLE_HS_LOG(INFO, "Done characteristic discovery\n");
        ...
        BLE_HS_LOG(ERROR, "\033[0;31mDid not find particular "
                   ...
                   const struct ble_gatt_svc *service, void *arg)
{
    ...
    if (error->status == BLE_HS_EDONE) {
        BLE_HS_LOG(INFO, "Done service discovery\n");
    ...
    BLE_HS_LOG(ERROR, "\033[0;31mError during service discovery\033[0m\n");
    assert(0);
    return 0;
}
