...
tx_stress_simple_scan(ble_gap_event_fn *cb, uint16_t duration)
{
    ...
    if (rc != 0) {
        iot_printf("\033[0;31mError determining own address type; "
                       "rc=%d\033[0m\n", rc);
        assert(0);
    }
    ...
    if (rc != 0) {
        iot_printf("\033[0;31mError initiating GAP discovery procedure"
                       "; rc=%d\033[0m\n", rc);
    }
}
...
tx_stress_simple_connect(ble_gap_event_fn *cb, int test_num)
{
    ...
    rc = ble_hs_id_infer_auto(0, &own_addr_type);
    if (rc != 0) {
        BLE_HS_LOG(INFO, "\033[0;31mError determining own address type; "
                          "rc=%d\033[0m\n", rc);
        return rc;
    }

    BLE_HS_LOG(INFO, "Connection attempt: %d\n",
    ...
    if (rc != 0) {
        BLE_HS_LOG(INFO, "\033[0;31mError during connection; rc=%d\033[0m\n",
                    rc);
    }
...
tx_stress_switcher_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            BLE_HS_LOG(INFO, "Success to connect to device\n");
            return 0;
        } else if (event->connect.status == BLE_HS_ETIMEOUT_HCI) {
            BLE_HS_LOG(INFO, "Connection timeout\n");
        } else {
            BLE_HS_LOG(INFO, "Error: connection attempt failed; status=%d\n",
    ...
    case BLE_GAP_EVENT_DISC_COMPLETE:
        BLE_HS_LOG(INFO, "Discovery complete; reason=%d\n",
                    event->disc_complete.reason);
        return 0;

    default:
        BLE_HS_LOG(INFO, "Other event occurs=%d\n", event->type);
        return 0;
    }
}
...
tx_stress_1_test()
{
    ...
    while (tx_stress_ctx->con_stat[1].attempts_num <
        ...
        BLE_HS_LOG(INFO, "Connection attempt; num=%d\n",
        ...
        if (rc != 0) {
            BLE_HS_LOG(INFO, "\033[0;31mConnection error; rc=%d\033[0m\n",
                        rc);
            os_sem_release(&tx_stress_main_sem);
            return;
        }

        BLE_HS_LOG(INFO, "Connect cancel\n");
        ble_gap_conn_cancel();
        iot_printf("\033[0;32m>\033[0m");
    }
    ...
    while (tx_stress_ctx->con_stat[1].attempts_num <
        ...
        BLE_HS_LOG(INFO, "Time to delay=%d\n", delay_time);
...
tx_stress_2_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            ...
            BLE_HS_LOG(INFO, "\033[0;31mError: connection attempt failed; "
                              "status=%d\033[0m\n", event->connect.status);
            os_sem_release(&tx_stress_main_sem);
        }
        return 0;
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        BLE_HS_LOG(INFO, "Disconnect; reason=%d \n",
...
tx_stress_4_con_update(void)
{
    ...
    if (rc == BLE_HS_ENOTCONN) {
        BLE_HS_LOG(INFO, "Device disconnected. Connection update failed\n");
        assert(0);
    }

    if (rc != 0) {
        BLE_HS_LOG(ERROR, "\033[0;31mError during connection update; "
                           "rc=%d\033[0m\n", rc);
        assert(0);
    }
...
tx_stress_4_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONN_UPDATE:
        if (event->conn_update.status != 0) {
            BLE_HS_LOG(INFO, "Connection update failed\n");
        } else {
            BLE_HS_LOG(INFO, "Connection updated; num=%d\n",
        ...
        if (tx_stress_ctx->con_stat[4].prms_upd_num >=
            ...
            if (rc != 0) {
                BLE_HS_LOG(INFO, "\033[0;31mError: update fail; "
                                  "rc=%d\033[0m\n", rc);
                os_sem_release(&tx_stress_main_sem);
            }
        }
    ...
    case BLE_GAP_EVENT_CONN_UPDATE_REQ:
        BLE_HS_LOG(INFO, "Connection update request\n");
        return 0;
...
tx_stress_5_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            ...
            iot_printf("\033[0;31mError: Update fail; "
                           "status=%d\033[0m\n", event->connect.status);
            os_sem_release(&tx_stress_main_sem);
        }
        return 0;
...
tx_stress_6_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_EXT_DISC:
        ...
        if (event->ext_disc.data_status ==
            ...
            BLE_HS_LOG(INFO, "Got all packets of advert. num=%d\n",
                        tx_stress_ctx->s6_rcv_adv_suc);
            iot_printf("\033[0;32m>\033[0m");
            start_id = 0;
...
tx_stress_7_phy_update(void)
{
    ...
    if (rc != 0) {
        BLE_HS_LOG(ERROR, "\033[0;31mError during PHY update; "
                           "rc=%d\033[0m\n", rc);
    }
...
tx_stress_7_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONN_UPDATE:
        BLE_HS_LOG(INFO, "Connection updated\n");
        return 0;
    ...
    case BLE_GAP_EVENT_PHY_UPDATE_COMPLETE:
        if (event->phy_updated.status != 0) {
            BLE_HS_LOG(INFO, "PHY update failed\n");
        } else {
            BLE_HS_LOG(INFO, "PHY updated; num=%d; rx:%d, tx:%d\n",
        ...
        if (tx_stress_ctx->con_stat[7].phy_upd_num >=
            ...
            if (rc != 0) {
                iot_printf("\033[0;31mError: PHPY update fail; "
...
tx_stress_8_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_PHY_UPDATE_COMPLETE:
        if (event->phy_updated.status != 0) {
            ...
            BLE_HS_LOG(INFO, "PHY updated; num=%d\n",
...
tx_stress_9_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISC_COMPLETE:
        if (event->disc_complete.reason == 0 && !tx_stress_ctx->completed[9]) {
            iot_printf("\033[0;31mScanning timeout\033[0m");
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            BLE_HS_LOG(INFO, "Success to connect to device\n");
            BLE_HS_LOG(INFO, "Connections num: %d\n",
...
void tx_stress_10_timer_ev_cb(struct os_event *ev)
{
    ...
    BLE_HS_LOG(INFO, "L2CAP receiving timeout\n");
    ble_gap_terminate(tx_stress_ctx->conn_handle,
                      BLE_ERR_REM_USER_CONN_TERM);
}
...
tx_stress_10_l2cap_send_req()
{
    ...
    BLE_HS_LOG(INFO, "L2CAP sending request\n");
...
tx_stress_10_l2cap_event(struct ble_l2cap_event *event, void *arg)
{
    ...
    case BLE_L2CAP_EVENT_COC_CONNECTED:
        ...
        if (event->connect.status == 0) {
            BLE_HS_LOG(INFO, "Established L2CAP connection\n");
    ...
    case BLE_L2CAP_EVENT_COC_DISCONNECTED:
        BLE_HS_LOG(INFO, "Remote device disconnected from L2CAP server\n");
        return 0;
    ...
    case BLE_L2CAP_EVENT_COC_DATA_RECEIVED:
        ...
        BLE_HS_LOG(INFO, "Bit rate: %d B/s\n", tx_stress_ctx->s10_bit_rate);
        ...
        iot_printf("\033[0;32m>\033[0m");
        BLE_HS_LOG(INFO, "Loop nr: %d\n", ++i);

        tx_stress_10_l2cap_send_req();
        return 0;

    case BLE_L2CAP_EVENT_COC_TX_UNSTALLED:
        BLE_HS_LOG(INFO, "L2CAP event unstalled\n");
        return 0;

    default:
        BLE_HS_LOG(INFO, "Other L2CAP event occurs; rc=%d\n", event->type);
        return 0;
    }
}
...
tx_stress_10_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        /* A new connection was established or a connection attempt failed. */
        if (event->connect.status == 0) {
            BLE_HS_LOG(INFO, "Success to connect to device; num: %d\n",
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        BLE_HS_LOG(INFO, "Average bit rate: %d B/s\n",
                    tx_stress_ctx->s10_bit_rate);
...
tx_stress_12_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_NOTIFY_RX:
        /* Received indication */
        BLE_HS_LOG(INFO, "Notify RX event\n");
        iot_printf("\033[0;32m>\033[0m");
        return 0;
...
tx_stress_14_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        BLE_HS_LOG(INFO, "Average notification time: %d\n",
    ...
    case BLE_GAP_EVENT_NOTIFY_RX:
        ...
        /* Time of data sending */
        us = tx_stress_ctx->end_us - tx_stress_ctx->begin_us;
        BLE_HS_LOG(INFO, "Notification time: %lld\n us", us);
...
scan_for_test_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISC_COMPLETE:
        ...
        iot_printf("\033[1;36mDiscover complete\033[0m\n");
        os_sem_release(&tx_stress_main_sem);
        return 0;
...
//    }
    ...
    iot_printf("\033[1;36mStart test num %d - ", test_num);
    ...
    case 0:
        return;
    case 1:
        iot_printf("Stress Connect -> Connect Cancel\033[0m\n");
        tx_stress_1_test();
        break;
    case 2:
        iot_printf("Stress Connect/Disconnect legacy\033[0m\n");
        tx_stress_simple_connect(&tx_stress_2_gap_event, 2);
        break;
    case 3:
        iot_printf("Stress Connect/Disconnect ext adv\033[0m\n");
        tx_stress_simple_connect(&tx_stress_3_gap_event, 3);
        break;
    case 4:
        iot_printf("Stress connection params update (TX)\033[0m\n");
        tx_stress_simple_connect(&tx_stress_4_gap_event, 4);
        break;
    case 5:
        iot_printf("Stress connection params update (RX)\033[0m\n");
        tx_stress_simple_connect(&tx_stress_5_gap_event, 5);
        break;
    case 6:
        iot_printf("Stress Scan\033[0m\n");
        tx_stress_6_perform();
        break;
    case 7:
        iot_printf("Stress PHY Update (TX)\033[0m\n");
        tx_stress_simple_connect(&tx_stress_7_gap_event, 7);
        break;
    case 8:
        iot_printf("Stress PHY Update (RX)\033[0m\n");
        tx_stress_simple_connect(&tx_stress_8_gap_event, 8);
        break;
    case 9:
        iot_printf("Stress multi connection\033[0m\n");
        tx_stress_9_perform();
        break;
    case 10:
        iot_printf("Stress L2CAP send\033[0m\n");
        tx_stress_simple_connect(&tx_stress_10_gap_event, 10);
        break;
    case 11:
        iot_printf("Stress Advertise/Connect/Disconnect\033[0m\n");
        tx_stress_simple_connect(&tx_stress_11_gap_event, 11);
        break;
    case 12:
        iot_printf("Stress GATT indication\033[0m\n");
        tx_stress_simple_connect(&tx_stress_12_gap_event, 12);
        break;
    case 13:
        iot_printf("Stress GATT notification\033[0m\n");
        tx_stress_simple_connect(&tx_stress_13_gap_event, 13);
        break;
    case 14:
        iot_printf("Stress GATT Subscribe/Notify/Unsubscribe\033[0m\n");
        tx_stress_simple_connect(&tx_stress_14_gap_event, 14);
        break;
    case 15:
        iot_printf("Stress Connect/Send/Disconnect\033[0m\n");
        tx_stress_simple_connect(&tx_stress_15_gap_event, 15);
        break;
    default:
        iot_printf("\033[0;31mFound test, but do not know how to perform."
                       "\033[0m\n");
        assert(0);
    }
...
tx_stress_main_task_fn(void *arg)
{
    ...
    iot_printf("\033[1;36mTX device\033[0m\n");
    iot_printf("Press ENTER to start: \n");
    ...
    while (1) {
        iot_printf("\033[0;36mStart scan for test\033[0m\n");
