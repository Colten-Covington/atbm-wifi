                ...
                const struct ble_gatt_dsc *gatt_dsc)
{
    ...
    if (dsc == NULL) {
        iot_printf("OOM WHILE DISCOVERING DESCRIPTOR\n");
        return NULL;
    }
...
{
    iot_printf("full discovery complete; rc=%d\n", rc);
...
btshell_disc_full_dscs(uint16_t conn_handle)
{
    ...
    conn = btshell_conn_find(conn_handle);
    if (conn == NULL) {
        BLE_HS_LOG(DEBUG, "Failed to discover descriptors for conn=%d; "
                           "not connected\n", conn_handle);
...
btshell_disc_full_chrs(uint16_t conn_handle)
{
    ...
    conn = btshell_conn_find(conn_handle);
    if (conn == NULL) {
        BLE_HS_LOG(DEBUG, "Failed to discover characteristics for conn=%d; "
                           "not connected\n", conn_handle);
        btshell_full_disc_complete(BLE_HS_ENOTCONN);
        return;
    }
                  ...
                  const struct ble_gatt_svc *service, void *arg)
{
    ...
    case BLE_HS_EDONE:
        iot_printf("service discovery successful\n");
                  ...
                  void *arg)
{
    ...
    case BLE_HS_EDONE:
        iot_printf("descriptor discovery successful\n");
        if (btshell_full_disc_prev_chr_val > 0) {
            btshell_disc_full_dscs(conn_handle);
        }
        break;
                ...
                struct ble_gatt_attr *attr, void *arg)
{
    ...
    case BLE_HS_EDONE:
        iot_printf("characteristic read complete\n");
        break;
                          ...
                          void *arg)
{
    ...
    switch (error->status) {
    case 0:
        iot_printf("characteristic write reliable complete; "
                       "conn_handle=%d", conn_handle);

        for (i = 0; i < num_attrs; i++) {
            iot_printf(" attr_handle=%d len=%d value=", attrs[i].handle,
...
btshell_decode_adv_data(const uint8_t *adv_data, uint8_t adv_data_len, void *arg)
{
    ...
    iot_printf(" data_length=%d data=", adv_data_len);
...
btshell_decode_event_type(struct ble_gap_ext_disc_desc *desc, void *arg)
{
    ...
    if (desc->props & BLE_HCI_ADV_LEGACY_MASK) {
        ...
        iot_printf("Legacy PDU type %d", desc->legacy_event_type);
    ...
    case BLE_GAP_EXT_ADV_DATA_STATUS_INCOMPLETE:
        iot_printf("incomplete");
        break;
    case BLE_GAP_EXT_ADV_DATA_STATUS_TRUNCATED:
        iot_printf("truncated");
        break;
    default:
        iot_printf("reserved %d", desc->data_status);
        break;
    }

common_data:
    iot_printf(" rssi=%d txpower=%d, pphy=%d, sphy=%d, sid=%d,"
    ...
    if (directed) {
        iot_printf(" init_addr_type=%d inita=", desc->direct_addr.type);
...
btshell_sync_stats(uint16_t handle)
{
    ...
    if (!psync->established) {
        iot_printf("Sync not established\n");
        return;
    }

    iot_printf("completed=%u truncated=%u\n",
...
handle_periodic_report(struct ble_gap_event *event)
{
    ...
    if (psync->changed ||
        ...
        if (!psync->changed) {
            iot_printf("Sync data changed, completed=%u, truncated=%u\n",
                           psync->complete, psync->truncated);
        }

        psync->changed = true;

        iot_printf("Sync report handle=%u status=", handle);
        ...
        case BLE_HCI_PERIODIC_DATA_STATUS_TRUNCATED:
            ...
            break;
        default:
            iot_printf("reserved 0x%x", event->periodic_report.data_status);
            break;
        }
...
btshell_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        iot_printf("disconnect; reason=%d ", event->disconnect.reason);
    ...
    case BLE_GAP_EVENT_PERIODIC_SYNC:
        if (event->periodic_sync.status) {
            iot_printf("Periodic Sync Establishment Failed; status=%u\n",
                           event->periodic_sync.status);
        } else {
            iot_printf("Periodic Sync Established; sync_handle=%u sid=%u "
            ...
            if (event->periodic_sync.sync_handle >= MYNEWT_VAL(BLE_MAX_PERIODIC_SYNCS)) {
                iot_printf("Unable to prepare cache for sync data\n");
            } else {
    ...
    case BLE_GAP_EVENT_PERIODIC_SYNC_LOST:
        ...
        if (event->periodic_sync_lost.sync_handle >= MYNEWT_VAL(BLE_MAX_PERIODIC_SYNCS)) {
            iot_printf("Periodic Sync Lost; sync_handle=%d reason=%d\n",
            ...
            iot_printf("Periodic Sync Lost; sync_handle=%d reason=%d completed=%u truncated=%u\n",
                           event->periodic_sync_lost.sync_handle,
                           event->periodic_sync_lost.reason,
                           psync->complete, psync->truncated);
    ...
    case BLE_GAP_EVENT_PERIODIC_TRANSFER:
        iot_printf("Periodic Sync Transfer Received on conn=%u; status=%u,"
	...
	case BLE_GAP_EVENT_PATHLOSS_THRESHOLD:
		iot_printf("LE Path Loss Report, handle=%d current_path_loss=%d zone_entered:%d\n",
	...
	case BLE_GAP_EVENT_TRANSMIT_POWER:
		if (event->transmit_power.status) {
            iot_printf("Transmit Power Report failed; status=%u\n", event->transmit_power.status);			
		}
		else{
			iot_printf("Transmit Power Report, handle=%d phy=%u reason=%d pwr=%d(dbm), pwr_flag:%d, delta:%d\n",
						event->transmit_power.conn_handle, event->transmit_power.phy, event->transmit_power.reason,
						event->transmit_power.transmit_power_level, event->transmit_power.transmit_power_level_flag,
						event->transmit_power.delta);
		}
		return 0;

	case BLE_GAP_EVENT_SUBRATE_CHANGE:
		if(event->subrate_change.status){
			 iot_printf("LE Subrate Change event failed; status=%u\n", event->subrate_change.status);	
		}
		else{
			iot_printf("LE Subrate Change event, handle=%d subrate_factor=%d periph_latency=%d cont_num=%d supervision_tmo=%d\n",
						event->subrate_change.conn_handle, event->subrate_change.subrate_factor, event->subrate_change.periph_latency,
						event->subrate_change.cont_num, event->subrate_change.supervision_tmo);
		}
		return 0;
#endif
...
{
    iot_printf("l2cap update complete; conn_handle=%d status=%d\n",
                   conn_handle, status);
}
...
btshell_read(uint16_t conn_handle, uint16_t attr_handle)
{
    ...
    if (conn_handle == BLE_HS_CONN_HANDLE_NONE) {
        ...
        if (rc == 0) {
            iot_printf("read local; attr_handle=%d len=%d value=",
...
{
#if !MYNEWT_VAL(BLE_EXT_ADV)
    iot_printf("BLE extended advertising not supported.");
    iot_printf(" Configure nimble host to enable it\n");
    return 0;
#else
    int rc;
    uint8_t phy_mask = 0;
...
btshell_tx_start(uint16_t conn_handle, uint16_t len, uint16_t rate, uint16_t num)
{
    ...
    if (!btshell_tx_data.conn) {
        iot_printf("Could not find ble_hs_conn for handle: %d\n",
                       conn_handle);
        return -1;
    }
...
{
    iot_printf("Error: Resetting state; reason=%d\n", reason);
}
...
btshell_on_sync(void)
{
    ...
    if (ble_hs_util_ensure_addr(0) != 0) {
        iot_printf("Failed to set identity address\n");
    }
    ...
    if (rc) {
        iot_printf("Error: generating oob data; reason=%d\n", rc);
        return;
    }
#endif

    iot_printf("Host and controller synced\n");
}
...
{
    iot_printf("LE CoC SDU received, chan: 0x%08lx, data len %d\n",
                   (uint32_t)(uintptr_t)chan, OS_MBUF_PKTLEN(sdu));
                         ...
                         struct ble_l2cap_chan *chan)
{
    ...
    iot_printf("LE CoC accepting, chan: 0x%08lx, peer_mtu %d\n",
                   (uint32_t)(uintptr_t)chan, peer_mtu);

    for (i = 0; i < MYNEWT_VAL(BLE_L2CAP_COC_SDU_BUFF_COUNT); i++) {
...
btshell_l2cap_event(struct ble_l2cap_event *event, void *arg)
{
    ...
    switch(event->type) {
        case BLE_L2CAP_EVENT_COC_CONNECTED:
            ...
            iot_printf("LE COC connected, conn: %d, chan: %p, psm: 0x%02x, scid: 0x%04x, "
        ...
        case BLE_L2CAP_EVENT_COC_DISCONNECTED:
            iot_printf("LE CoC disconnected, chan: %p\n",
        ...
        case BLE_L2CAP_EVENT_COC_PEER_RECONFIGURED:
            ...
            iot_printf("LE CoC peer reconfigured status 0x%02x," \
                            "chan: %p\n",
                            event->reconfigured.status,
                            event->reconfigured.chan);

            if (event->reconfigured.status == 0) {
                iot_printf("\t peer_mps: %d peer_mtu %d\n", chan_info.peer_l2cap_mtu, chan_info.peer_coc_mtu);
            }

            return 0;
        case BLE_L2CAP_EVENT_COC_TX_UNSTALLED:
            iot_printf("L2CAP CoC channel %p unstalled, last sdu sent with err=0x%02x\n",
...
{
#if MYNEWT_VAL(BLE_L2CAP_COC_MAX_NUM) == 0
    iot_printf("BLE L2CAP LE COC not supported.");
    iot_printf(" Configure nimble host to enable it\n");
    return 0;
#else
...
btshell_l2cap_connect(uint16_t conn_handle, uint16_t psm, uint16_t mtu, uint8_t num)
{
    ...
    iot_printf("L2CAP CoC MTU: %d, max available %d\n", mtu, BTSHELL_COC_MTU);
...
btshell_l2cap_disconnect(uint16_t conn_handle, uint16_t idx)
{
    ...
    if (rc) {
        iot_printf("Could not disconnect channel rc=%d\n", rc);
    }
                       ...
                       uint8_t num, uint8_t idxs[])
{
    ...
    conn = btshell_conn_find(conn_handle);
    if (conn == NULL) {
        iot_printf("conn=%d does not exist\n", conn_handle);
        return 0;
    }
...
btshell_l2cap_send(uint16_t conn_handle, uint16_t idx, uint16_t bytes)
{
    ...
    iot_printf("conn=%d, idx=%d, bytes=%d\n", conn_handle, idx, bytes);
    ...
    if (coc == NULL) {
        iot_printf("Are you sure your channel exist?\n");
        return 0;
    }

    if (coc->stalled) {
        iot_printf("Channel is stalled, wait ...\n");
        return 0;
    }

    sdu_tx = os_mbuf_get_pkthdr(&sdu_os_mbuf_pool, 0);
    if (sdu_tx == NULL) {
        iot_printf("No memory in the test sdu pool\n");
        return 0;
    }
    ...
    for (i = 0; i < bytes / sizeof(b); i++) {
        ...
        if (rc) {
            iot_printf("Cannot append data %i !\n", i);
    ...
    if (rc) {
        if (rc == BLE_HS_ESTALLED) {
          iot_printf("CoC module is stalled with data. Wait for unstalled \n");
          coc->stalled = true;
        } else {
            iot_printf("Could not send data rc=%d\n", rc);
        }
        os_mbuf_free_chain(sdu_tx);
    }
 ...
 */
SRAM_CODE int btshell_init(void)
{
    int rc;
...
SRAM_CODE int btshell_entry(void)
{
