...
cmd_l2cap_create_server(int argc, char **argv)
{
    ...
    if (rc) {
        iot_printf("Server create error: 0x%02x\n", rc);
        return rc;
    }

    iot_printf("Server created successfully\n");
    return 0;
}
...
cmd_l2cap_reconfig(int argc, char **argv)
{
    ...
    if (rc != 0) {
       iot_printf("invalid 'idxs' parameter\n");
       return rc;
    }

    return btshell_l2cap_reconfig(conn, mtu, num, idxs);
#else
    iot_printf("To enable this features set BLE_L2CAP_ENHANCED_COC\n");
    return ENOTSUP;
#endif
}
