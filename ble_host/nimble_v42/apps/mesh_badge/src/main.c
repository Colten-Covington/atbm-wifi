...
void print_addr(const void *addr)
{
	...
	u8p = addr;
	BLE_HS_LOG(INFO, "%02x:%02x:%02x:%02x:%02x:%02x",
...
{
	BLE_HS_LOG(INFO, "handle=%d our_ota_addr_type=%d our_ota_addr=",
		    desc->conn_handle, desc->our_ota_addr.type);
	print_addr(desc->our_ota_addr.val);
	BLE_HS_LOG(INFO, " our_id_addr_type=%d our_id_addr=",
		    desc->our_id_addr.type);
	print_addr(desc->our_id_addr.val);
	BLE_HS_LOG(INFO, " peer_ota_addr_type=%d peer_ota_addr=",
		    desc->peer_ota_addr.type);
	print_addr(desc->peer_ota_addr.val);
	BLE_HS_LOG(INFO, " peer_id_addr_type=%d peer_id_addr=",
		    desc->peer_id_addr.type);
	print_addr(desc->peer_id_addr.val);
	BLE_HS_LOG(INFO, " conn_itvl=%d conn_latency=%d supervision_timeout=%d "
...
static void advertise(void)
{
	...
	if (rc != 0) {
		BLE_HS_LOG(ERROR, "error setting advertisement data; rc=%d\n", rc);
		return;
	}
	...
	if (rc != 0) {
		BLE_HS_LOG(ERROR, "error enabling advertisement; rc=%d\n", rc);
		return;
	}
}
...
static int gap_event(struct ble_gap_event *event, void *arg)
{
	...
	switch (event->type) {
		...
		case BLE_GAP_EVENT_CONN_UPDATE:
			...
			BLE_HS_LOG(INFO, "connection updated; status=%d ",
		...
		case BLE_GAP_EVENT_PASSKEY_ACTION:
			BLE_HS_LOG(INFO, "passkey action event; conn_handle=%d action=%d numcmp=%d\n",
