...
{
    iot_printf("Received net packet: ttl 0x%02x ctl 0x%02x src 0x%04x "
...
static void model_bound_cb(uint16_t addr, struct bt_mesh_model *model,
                           uint16_t key_idx)
{
    iot_printf("Model bound: remote addr 0x%04x key_idx 0x%04x model %p\n",
                   addr, key_idx, model);
}
...
static void model_unbound_cb(uint16_t addr, struct bt_mesh_model *model,
                             uint16_t key_idx)
{
    iot_printf("Model unbound: remote addr 0x%04x key_idx 0x%04x "
                   "model %p\n", addr, key_idx, model);
}

static void invalid_bearer_cb(uint8_t opcode)
{
    iot_printf("Invalid bearer: opcode 0x%02x\n", opcode);
}

static void incomp_timer_exp_cb(void)
{
    iot_printf("Incomplete timer expired\n");
}
