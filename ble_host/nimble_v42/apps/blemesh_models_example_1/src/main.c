...
static void button_pressed(struct os_event *ev)
{
    ...
    if (button_press_cnt == 0) {
        ble_npl_callout_reset(&sw.button_timer, os_time_ms_to_ticks32(K_SECONDS(1)));
    }
...
blemesh_on_sync(void)
{
    ...
    if (bt_mesh_is_provisioned()) {
        iot_printf("Mesh network restored from flash\n");
    }
...
int main(void)
{
    ...
    ble_npl_callout_init(&sw.button_work, os_eventq_dflt_get(),
                    button_pressed_worker, &sw);

    /* Initialize button count timer */
    ble_npl_callout_init(&sw.button_timer, os_eventq_dflt_get(),
                    button_cnt_timer, &sw);
