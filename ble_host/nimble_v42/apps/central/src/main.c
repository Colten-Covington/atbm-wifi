...
conn_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        if (event->connect.status == 0) {
            BLE_HS_LOG(INFO,"Connection was established\n");
            ble_gap_terminate(event->connect.conn_handle, 0x13);
        } else {
            BLE_HS_LOG(INFO,"Connection failed, error code: %i\n",
                event->connect.status);
        }
        break;
    case BLE_GAP_EVENT_DISCONNECT:
        BLE_HS_LOG(INFO,"Disconnected, reason code: %i\n",
        event->disconnect.reason);
        scan();
        break;
    case BLE_GAP_EVENT_CONN_UPDATE_REQ:
        BLE_HS_LOG(INFO,"Connection update request received\n");
        break;
    case BLE_GAP_EVENT_CONN_UPDATE:
        if (event->conn_update.status == 0) {
            BLE_HS_LOG(INFO,"Connection update successful\n");
        } else {
            BLE_HS_LOG(INFO,"Connection update failed; reson: %d\n",
                        event->conn_update.status);
        }
        break;
    default:
        BLE_HS_LOG(INFO,"Connection event type not supported, %d\n",
                    event->type);
        break;
    }
    return 0;
...
scan_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISC:
        BLE_HS_LOG(INFO, "Advertising report received! Checking UUID...\n");
        ...
        if (uuid_cmp_result) {
            BLE_HS_LOG(INFO, "UUID doesn't fit\n");
        } else {
            BLE_HS_LOG(INFO, "UUID fits, connecting...\n");
            ble_gap_disc_cancel();
    ...
    case BLE_GAP_EVENT_DISC_COMPLETE:
        BLE_HS_LOG(INFO,"Discovery completed, reason: %d\n",
