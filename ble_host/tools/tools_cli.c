...
int main(int argc, char *argv[])
{
	...
	if (argc <= 1)
	{
		fprintf(stdout,"atbm_iot_cli need at least 1 argument\n"
			   "refer to atbm_iot_supplicant command use(help) or input 'quit' to exit cli and supplicant process.\n");
		return -1;
	}

	memset(command, 0, sizeof(command));
	snprintf(command, sizeof(command), "%s", argv[tmp_argc++]);

	while (tmp_argc < argc)
	{	memcpy(temp, command, CMD_LINE_LEN);
		sprintf(command, "%s %s", temp, argv[tmp_argc++]);
	}
